...
exports.SageMakerEmbeddingProvider = exports.SageMakerCompletionProvider = void 0;
...
const SUPPORTED_MODEL_TYPES = ['openai', 'llama', 'huggingface', 'jumpstart', 'custom'];
/**
 * Zod schema for validating SageMaker options
 */
const SageMakerConfigSchema = zod_1.default
    .object({
    // AWS credentials options
    accessKeyId: zod_1.default.string().optional(),
    profile: zod_1.default.string().optional(),
    region: zod_1.default.string().optional(),
    secretAccessKey: zod_1.default.string().optional(),
    sessionToken: zod_1.default.string().optional(),
    // SageMaker specific options
    endpoint: zod_1.default.string().optional(),
    contentType: zod_1.default.string().optional(),
    acceptType: zod_1.default.string().optional(),
    // Model parameters
    maxTokens: zod_1.default.number().optional(),
    temperature: zod_1.default.number().optional(),
    topP: zod_1.default.number().optional(),
    stopSequences: zod_1.default.array(zod_1.default.string()).optional(),
    // Provider behavior options
    delay: zod_1.default.number().optional(), // Delay between API calls in milliseconds
    transform: zod_1.default.string().optional(), // Transform function or file path to transform prompts
    ...
    modelType: zod_1.default.enum(SUPPORTED_MODEL_TYPES).optional(),
    // Response format options
    responseFormat: zod_1.default
        .object({
        type: zod_1.default.string().optional(),
        path: zod_1.default.string().optional(), // JavaScript expression to extract content (formerly JSONPath)
    })
        .optional(),
    basePath: zod_1.default.string().optional(),
})
    .strict();
/**
...
class SageMakerGenericProvider {
    constructor(endpointName, options) {
        ...
        try {
            ...
            logger_1.default.warn(`Error validating SageMaker config\nConfig: ${JSON.stringify(config)}\n${(0, zod_validation_error_1.fromError)(error).message}`);
        }
        this.config = config ?? {};
...
class SageMakerCompletionProvider extends SageMakerGenericProvider {
    ...
    parseModelType(modelType) {
        ...
        if (match) {
            ...
            if (SUPPORTED_MODEL_TYPES.includes(modelTypeFromId)) {
                return modelTypeFromId;
            }
            else {
                throw new Error(`Invalid model type "${modelTypeFromId}" in provider ID. Valid types are: ${SUPPORTED_MODEL_TYPES.join(', ')}`);
            }
        }
        ...
        if (modelType) {
            if (SUPPORTED_MODEL_TYPES.includes(modelType)) {
                ...
                throw new Error(`Invalid model type "${modelType}" in \`config.modelType\`. Valid types are: ${SUPPORTED_MODEL_TYPES.join(', ')}`);
            }
        }
    ...
    formatPayload(prompt) {
        ...
        const temperature = typeof this.config.temperature === 'number'
            ? this.config.temperature
            : ((0, envars_1.getEnvFloat)('AWS_SAGEMAKER_TEMPERATURE') ?? 0.7);
        const topP = typeof this.config.topP === 'number'
            ? this.config.topP
            : ((0, envars_1.getEnvFloat)('AWS_SAGEMAKER_TOP_P') ?? 1.0);
        const stopSequences = this.config.stopSequences || [];
        let payload;
        logger_1.default.debug(`Formatting payload for model type: ${this.modelType}`);
    ...
    async parseResponse(responseBody) {
        let responseJson;
        logger_1.default.debug(`Parsing response for model type: ${this.modelType}`);
        try {
    ...
    getCacheKey(prompt) {
        ...
        const promptHash = crypto_1.default.createHash('sha256').update(prompt).digest('hex').substring(0, 16);
        const configHash = crypto_1.default.createHash('sha256').update(configStr).digest('hex').substring(0, 8);
    ...
    async callApi(prompt, context, _options) {
        ...
        if (isCacheEnabled() && !bustCache) {
            ...
            const cache = getCache ? getCache() : await Promise.resolve().then(() => __importStar(require('../cache'))).then((m) => m.getCache());
...
class SageMakerEmbeddingProvider extends SageMakerGenericProvider {
    ...
    getCacheKey(text) {
        ...
        const configStr = JSON.stringify(configForKey);
        // Generate shorter, more efficient hashed keys
        const textHash = crypto_1.default.createHash('sha256').update(text).digest('hex').substring(0, 16);
        const configHash = crypto_1.default.createHash('sha256').update(configStr).digest('hex').substring(0, 8);
