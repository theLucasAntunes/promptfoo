...
class OllamaCompletionProvider {
    ...
    async callApi(prompt) {
        ...
        logger_1.default.debug('Calling Ollama API', { params });
        let response;
        try {
            response = await (0, cache_1.fetchWithCache)(`${(0, envars_1.getEnvString)('OLLAMA_BASE_URL') || 'http://localhost:11434'}/api/generate`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    ...((0, envars_1.getEnvString)('OLLAMA_API_KEY')
                        ? { Authorization: `Bearer ${(0, envars_1.getEnvString)('OLLAMA_API_KEY')}` }
                        : {}),
                },
...
class OllamaChatProvider {
    ...
    async callApi(prompt, context) {
        ...
        if (this.config.tools) {
            params.tools = (0, index_1.maybeLoadToolsFromExternalFile)(this.config.tools, context?.vars);
        }
        logger_1.default.debug('Calling Ollama API', { params });
        let response;
        try {
            response = await (0, cache_1.fetchWithCache)(`${(0, envars_1.getEnvString)('OLLAMA_BASE_URL') || 'http://localhost:11434'}/api/chat`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    ...((0, envars_1.getEnvString)('OLLAMA_API_KEY')
                ...
                body: JSON.stringify(params),
            }, shared_1.REQUEST_TIMEOUT_MS, 'text', context?.bustCache ?? context?.debug);
        }
        catch (err) {
            return {
                error: `API call error: ${String(err)}. Output:\n${response?.data}`,
...
class OllamaEmbeddingProvider extends OllamaCompletionProvider {
    async callEmbeddingApi(text) {
        ...
        let response;
        try {
            response = await (0, cache_1.fetchWithCache)(`${(0, envars_1.getEnvString)('OLLAMA_BASE_URL') || 'http://localhost:11434'}/api/embeddings`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    ...((0, envars_1.getEnvString)('OLLAMA_API_KEY')
