...
exports.processAnthropicTools = processAnthropicTools;
...
function calculateAnthropicCost(modelName, config, promptTokens, completionTokens) {
    ...
    if (isSonnet45 &&
        ...
        const inputCost = config.cost ?? (promptTokens > 200000 ? 6 / 1e6 : 3 / 1e6);
        const outputCost = config.cost ?? (promptTokens > 200000 ? 22.5 / 1e6 : 15 / 1e6);
    ...
    for (const tool of tools) {
        if ('type' in tool) {
            ...
            if (tool.type === 'web_fetch_20250910') {
                ...
                processedTools.push(transformWebSearchTool(tool));
