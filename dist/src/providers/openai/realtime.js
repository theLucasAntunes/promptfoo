...
exports.OpenAiRealtimeProvider = void 0;
 ...
 */
function convertPcm16ToWav(pcmData, sampleRate = 24000) {
...
class OpenAiRealtimeProvider extends _1.OpenAiGenericProvider {
    ...
    getRealtimeSessionBody() {
        ...
        if (this.config.tools && this.config.tools.length > 0) {
            body.tools = (0, index_1.maybeLoadToolsFromExternalFile)(this.config.tools);
                    ...
                    switch (message.type) {
                        ...
                        case 'conversation.item.created':
                            if (message.item.role === 'user') {
                                ...
                                if (this.config.tools && this.config.tools.length > 0) {
                                    responseEvent.response.tools = (0, index_1.maybeLoadToolsFromExternalFile)(this.config.tools);
                        ...
                        case 'response.audio.delta':
                            ...
                            logger_1.default.debug(`Received audio data chunk: delta field exists=${!!message.delta}, length=${message.delta ? message.delta.length : 0}`);
                            if (audioData && audioData.length > 0) {
                                ...
                                try {
                                    ...
                                    logger_1.default.debug(`Successfully processed audio chunk: ${audioBuffer.length} bytes, total chunks: ${audioContent.length}`);
                                }
                                catch (error) {
                                ...
                                logger_1.default.debug(`Audio delta received but no audio data present. Message fields: ${Object.keys(message).join(', ')}`);
                            }
                            break;
                        ...
                        case 'response.done':
                            ...
                            if (usage?.output_token_details?.audio_tokens &&
                                ...
                                audioFormat = 'wav';
                                logger_1.default.debug(`Audio detected from usage tokens: ${usage.output_token_details.audio_tokens} audio tokens, converting PCM16 to WAV format`);
                            }
                            ...
                            if (hasAudioContent && audioContent.length > 0) {
                                try {
                                    ...
                                    logger_1.default.debug(`Audio conversion: PCM16 ${rawPcmData.length} bytes -> WAV ${wavData.length} bytes`);
                                }
                                catch (error) {
                                    logger_1.default.error(`Error converting audio data to WAV format: ${error}`);
                                    // Still set hasAudioContent to false if conversion fails
                                    hasAudioContent = false;
                                }
                            }
                            logger_1.default.debug(`AUDIO TRACE: Before resolve - hasAudioContent=${hasAudioContent}, audioContent.length=${audioContent.length}, finalAudioData.length=${finalAudioData?.length || 0}`);
                            logger_1.default.debug(`AUDIO TRACE: audioFormat=${audioFormat}, responseText.length=${responseText.length}`);
                            resolve({
                                output: responseText,
                                tokenUsage: {
    ...
    async callApi(prompt, context, _callApiOptions) {
        ...
        try {
            ...
            if (result.metadata?.audio) {
                ...
                logger_1.default.debug(`AUDIO TRACE: Main callApi - Found result.metadata.audio, data.length=${audioDataBase64?.length || 0}, format=${result.metadata.audio.format}`);
            }
            else {
                logger_1.default.debug(`AUDIO TRACE: Main callApi - No result.metadata.audio found. result.metadata keys: ${Object.keys(result.metadata || {}).join(', ')}`);
            }
            return {
                output: finalOutput,
                ...
                sendEvent({
                    ...
                    session: {
                        ...
                        ...(this.config.tools &&
                            ...
                            tools: (0, index_1.maybeLoadToolsFromExternalFile)(this.config.tools),
                ...
                try {
                    ...
                    switch (message.type) {
                        ...
                        case 'session.updated':
                            logger_1.default.debug('Session updated on WebSocket');
                            break;
                        case 'conversation.item.created':
                            if (message.item.role === 'user') {
                                // User message was created, now create a response
...
OpenAiRealtimeProvider.OPENAI_REALTIME_MODELS = util_1.OPENAI_REALTIME_MODELS;
OpenAiRealtimeProvider.OPENAI_REALTIME_MODEL_NAMES = util_1.OPENAI_REALTIME_MODELS.map((model) => model.id);
