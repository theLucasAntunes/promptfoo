...
exports.AdalineGatewayChatProvider = exports.AdalineGatewayEmbeddingProvider = exports.AdalineGatewayCachePlugin = void 0;
    ...
    async callEmbeddingApi(text) {
        try {
            ...
            if (this.providerName === 'openai') {
                ...
                const parentClass = new vertex_2.VertexEmbeddingProvider(this.modelName, {
                    config: this.config,
                    env: this.env,
                });
            ...
            logger_1.default.debug('Adaline Gateway Embedding API Request', { request: gatewayRequest });
...
class AdalineGatewayChatProvider extends AdalineGatewayGenericProvider {
    ...
    async callApi(prompt, context, callApiOptions) {
        ...
        try {
            ...
            if (formatType === 'openai') {
                ...
                gatewayTools = _config.tools
                    ? (0, index_1.maybeLoadToolsFromExternalFile)(_config.tools)
                    : undefined;
            }
            if (this.providerName === 'openai') {
                const provider = new openai_1.OpenAI();
                ...
                const provider = new vertex_1.Vertex();
                const parentClass = new vertex_2.VertexChatProvider(this.modelName, {
                    config: this.config,
                    env: this.env,
                });
                const client = await parentClass.getClientWithCredentials();
        ...
        logger_1.default.debug('Adaline Gateway Chat API request', { request: gatewayRequest });
        try {
            ...
            logger_1.default.error('Adaline Gateway Chat API response error', {
                error: String(error),
        ...
        try {
            ...
            if (response.response.messages[0].content.length === 1 &&
                ...
                if (formatType === 'openai') {
                    ...
                    if (response.response.messages[0].content.filter((content) => content.modality === 'text').length > 0) {
