"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProviderEnvOverridesSchema = void 0;
const zod_1 = require("zod");
exports.ProviderEnvOverridesSchema = zod_1.z.object({
    AI21_API_BASE_URL: zod_1.z.string().optional(),
    AI21_API_KEY: zod_1.z.string().optional(),
    AIML_API_KEY: zod_1.z.string().optional(),
    ANTHROPIC_API_KEY: zod_1.z.string().optional(),
    ANTHROPIC_BASE_URL: zod_1.z.string().optional(),
    AWS_BEDROCK_REGION: zod_1.z.string().optional(),
    AZURE_API_BASE_URL: zod_1.z.string().optional(),
    AZURE_API_HOST: zod_1.z.string().optional(),
    AZURE_API_KEY: zod_1.z.string().optional(),
    AZURE_AUTHORITY_HOST: zod_1.z.string().optional(),
    AZURE_CLIENT_ID: zod_1.z.string().optional(),
    AZURE_CLIENT_SECRET: zod_1.z.string().optional(),
    AZURE_DEPLOYMENT_NAME: zod_1.z.string().optional(),
    AZURE_EMBEDDING_DEPLOYMENT_NAME: zod_1.z.string().optional(),
    AZURE_OPENAI_API_BASE_URL: zod_1.z.string().optional(),
    AZURE_OPENAI_API_HOST: zod_1.z.string().optional(),
    AZURE_OPENAI_API_KEY: zod_1.z.string().optional(),
    AZURE_OPENAI_BASE_URL: zod_1.z.string().optional(),
    AZURE_OPENAI_DEPLOYMENT_NAME: zod_1.z.string().optional(),
    AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME: zod_1.z.string().optional(),
    AZURE_TENANT_ID: zod_1.z.string().optional(),
    AZURE_TOKEN_SCOPE: zod_1.z.string().optional(),
    CLAUDE_CODE_USE_BEDROCK: zod_1.z.string().optional(),
    CLAUDE_CODE_USE_VERTEX: zod_1.z.string().optional(),
    CLOUDFLARE_ACCOUNT_ID: zod_1.z.string().optional(),
    CLOUDFLARE_API_KEY: zod_1.z.string().optional(),
    COMETAPI_KEY: zod_1.z.string().optional(),
    COHERE_API_KEY: zod_1.z.string().optional(),
    COHERE_CLIENT_NAME: zod_1.z.string().optional(),
    DATABRICKS_TOKEN: zod_1.z.string().optional(),
    DATABRICKS_WORKSPACE_URL: zod_1.z.string().optional(),
    DOCKER_MODEL_RUNNER_BASE_URL: zod_1.z.string().optional(),
    DOCKER_MODEL_RUNNER_API_KEY: zod_1.z.string().optional(),
    FAL_KEY: zod_1.z.string().optional(),
    GITHUB_TOKEN: zod_1.z.string().optional(),
    GOOGLE_API_HOST: zod_1.z.string().optional(),
    GOOGLE_API_BASE_URL: zod_1.z.string().optional(),
    GOOGLE_API_KEY: zod_1.z.string().optional(),
    GOOGLE_PROJECT_ID: zod_1.z.string().optional(),
    GOOGLE_LOCATION: zod_1.z.string().optional(),
    GOOGLE_GENERATIVE_AI_API_KEY: zod_1.z.string().optional(),
    GEMINI_API_KEY: zod_1.z.string().optional(),
    GROQ_API_KEY: zod_1.z.string().optional(),
    HELICONE_API_KEY: zod_1.z.string().optional(),
    HF_API_TOKEN: zod_1.z.string().optional(),
    HF_TOKEN: zod_1.z.string().optional(),
    HYPERBOLIC_API_KEY: zod_1.z.string().optional(),
    HUGGING_FACE_HUB_TOKEN: zod_1.z.string().optional(),
    JFROG_API_KEY: zod_1.z.string().optional(),
    LANGFUSE_HOST: zod_1.z.string().optional(),
    LANGFUSE_PUBLIC_KEY: zod_1.z.string().optional(),
    LANGFUSE_SECRET_KEY: zod_1.z.string().optional(),
    LITELLM_API_BASE: zod_1.z.string().optional(),
    LLAMA_BASE_URL: zod_1.z.string().optional(),
    LOCALAI_BASE_URL: zod_1.z.string().optional(),
    MISTRAL_API_BASE_URL: zod_1.z.string().optional(),
    MISTRAL_API_HOST: zod_1.z.string().optional(),
    MISTRAL_API_KEY: zod_1.z.string().optional(),
    NSCALE_SERVICE_TOKEN: zod_1.z.string().optional(),
    NSCALE_API_KEY: zod_1.z.string().optional(),
    OLLAMA_API_KEY: zod_1.z.string().optional(),
    OLLAMA_BASE_URL: zod_1.z.string().optional(),
    OPENAI_API_BASE_URL: zod_1.z.string().optional(),
    OPENAI_API_HOST: zod_1.z.string().optional(),
    OPENAI_API_KEY: zod_1.z.string().optional(),
    OPENAI_BASE_URL: zod_1.z.string().optional(),
    OPENAI_ORGANIZATION: zod_1.z.string().optional(),
    PALM_API_HOST: zod_1.z.string().optional(),
    PALM_API_KEY: zod_1.z.string().optional(),
    PORTKEY_API_KEY: zod_1.z.string().optional(),
    PROMPTFOO_CA_CERT_PATH: zod_1.z.string().optional(),
    PROMPTFOO_PFX_CERT_PATH: zod_1.z.string().optional(),
    PROMPTFOO_PFX_PASSWORD: zod_1.z.string().optional(),
    PROMPTFOO_JKS_CERT_PATH: zod_1.z.string().optional(),
    PROMPTFOO_JKS_PASSWORD: zod_1.z.string().optional(),
    PROMPTFOO_JKS_ALIAS: zod_1.z.string().optional(),
    PROMPTFOO_INSECURE_SSL: zod_1.z.string().optional(),
    REPLICATE_API_KEY: zod_1.z.string().optional(),
    REPLICATE_API_TOKEN: zod_1.z.string().optional(),
    VERTEX_API_HOST: zod_1.z.string().optional(),
    VERTEX_API_KEY: zod_1.z.string().optional(),
    VERTEX_API_VERSION: zod_1.z.string().optional(),
    VERTEX_PROJECT_ID: zod_1.z.string().optional(),
    VERTEX_PUBLISHER: zod_1.z.string().optional(),
    VERTEX_REGION: zod_1.z.string().optional(),
    VOYAGE_API_BASE_URL: zod_1.z.string().optional(),
    VOYAGE_API_KEY: zod_1.z.string().optional(),
    WATSONX_AI_APIKEY: zod_1.z.string().optional(),
    WATSONX_AI_AUTH_TYPE: zod_1.z.string().optional(),
    WATSONX_AI_BEARER_TOKEN: zod_1.z.string().optional(),
    WATSONX_AI_PROJECT_ID: zod_1.z.string().optional(),
    AZURE_CONTENT_SAFETY_ENDPOINT: zod_1.z.string().optional(),
    AZURE_CONTENT_SAFETY_API_KEY: zod_1.z.string().optional(),
    AZURE_CONTENT_SAFETY_API_VERSION: zod_1.z.string().optional(),
    AWS_REGION: zod_1.z.string().optional(),
    AWS_DEFAULT_REGION: zod_1.z.string().optional(),
    AWS_SAGEMAKER_MAX_TOKENS: zod_1.z.string().optional(),
    AWS_SAGEMAKER_TEMPERATURE: zod_1.z.string().optional(),
    AWS_SAGEMAKER_TOP_P: zod_1.z.string().optional(),
    AWS_SAGEMAKER_MAX_RETRIES: zod_1.z.string().optional(),
    PROMPTFOO_EVAL_TIMEOUT_MS: zod_1.z.string().optional(),
});
//# sourceMappingURL=env.js.map