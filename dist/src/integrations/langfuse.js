...
let langfuse;
async function getPrompt(id, vars, type, version, label) {
    let prompt;
    if (!langfuse) {
        try {
    ...
    try {
        ...
        if (label) {
            throw new Error(`Failed to fetch Langfuse prompt "${id}" with label "${label}": ${error.message || error}`);
        }
        else if (version === undefined) {
            throw new Error(`Failed to fetch Langfuse prompt "${id}": ${error.message || error}`);
        }
        else {
            throw new Error(`Failed to fetch Langfuse prompt "${id}" version ${version}: ${error.message || error}`);
        }
    }
