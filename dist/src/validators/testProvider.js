...
exports.testHTTPProviderConnectivity = testHTTPProviderConnectivity;
exports.testProviderSession = testProviderSession;
 ...
 */
async function testHTTPProviderConnectivity(provider, prompt = 'Hello World!') {
    const vars = {};
    ...
    try {
        ...
        // Run evaluation
        logger_1.default.debug('[testProviderConnectivity] Running evaluation', {
        ...
        logger_1.default.debug('[testProviderConnectivity] Evaluation completed', {
            result: (0, sanitizer_1.sanitizeObject)(result),
            providerId: provider.id,
        });
        // Extract session ID
        ...
        if ((0, remoteGeneration_1.neverGenerateRemote)()) {
            logger_1.default.debug('[testProviderConnectivity] Remote grading disabled, returning raw result');
            return {
        ...
        const HOST = cloud_1.cloudConfig.getApiHost();
        try {
            logger_1.default.debug('[testProviderConnectivity] Calling agent helper', {
                providerId: provider.id,
            });
            const testAnalyzerResponse = await (0, fetch_1.fetchWithProxy)(`${HOST}/api/v1/providers/test`, {
                method: 'POST',
                headers: {
            ...
            if (!testAnalyzerResponse.ok) {
                logger_1.default.error('[testProviderConnectivity] Error calling agent helper', {
            ...
            return {
                ...
                message: testAnalyzerResponseObj.message ||
        ...
        logger_1.default.error('[testProviderConnectivity] Error during evaluation', {
            error: errorMessage,
            providerId: provider.id,
        });
        return {
...
function validateAndConfigureSessions({ provider, sessionConfig, options, }) {
    const effectiveSessionSource = (0, util_1.determineEffectiveSessionSource)({ provider, sessionConfig });
    ...
    if (!options?.skipConfigValidation) {
        (0, util_1.validateSessionConfig)({
            provider,
...
function buildTroubleshootingAdvice({ sessionWorking, sessionSource, sessionConfig, providerConfig, initialSessionId, firstResult, secondResult, }) {
    ...
    const configuredHeaders = (0, util_1.formatConfigHeaders)({ headers: providerConfig?.headers });
    const configuredBody = (0, util_1.formatConfigBody)({ body: providerConfig?.body });
 ...
 */
async function testProviderSession(provider, sessionConfig, options) {
    try {
        ...
        const effectiveSessionSource = (0, util_1.determineEffectiveSessionSource)({
            provider,
            sessionConfig,
        });
        const initialSessionId = effectiveSessionSource === 'server' ? undefined : (0, uuid_1.v4)();
        const firstPrompt = 'What can you help me with?';
        const secondPrompt = 'What was the last thing I asked you?';
        // Make first request
        logger_1.default.debug('[testProviderSession] Making first request', {
            prompt: firstPrompt,
        ...
        logger_1.default.debug('[testProviderSession] First request completed', {
            response: (0, sanitizer_1.sanitizeObject)(firstResponse),
            providerId: provider.id,
        });
        ...
        logger_1.default.debug('[testProviderSession] Session ID extracted', {
            extractedSessionId,
            providerId: provider.id,
        });
        ...
        logger_1.default.debug('[testProviderSession] Making second request', {
            prompt: secondPrompt,
            sessionId: extractedSessionId,
            providerId: provider.id,
        });
        ...
        logger_1.default.debug('[testProviderSession] Second request completed', {
            response: (0, sanitizer_1.sanitizeObject)(secondResponse),
            providerId: provider.id,
        });
        ...
        logger_1.default.debug('[testProviderSession] Evaluating session with LLM rubric', {
            providerId: provider.id,
        });
        let sessionWorking = false;
        let judgeReason = 'Session memory test completed';
        try {
            const gradingResult = await (0, remoteGrading_1.doRemoteGrading)({
                task: 'llm-rubric',
                rubric: (0, dedent_1.default) `
      ...
      `,
            ...
            logger_1.default.warn('[testProviderSession] Failed to evaluate session with LLM rubric', {
        ...
        logger_1.default.debug('[testProviderSession] Judge result', {
            pass: sessionWorking,
            reason: judgeReason,
            providerId: provider.id,
        });
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        logger_1.default.error('[testProviderSession] Error testing session', {
            error: errorMessage,
            providerId: provider.id,
        });
        return {
