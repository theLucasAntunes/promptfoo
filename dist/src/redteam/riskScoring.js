...
exports.getStrategyMetadata = getStrategyMetadata;
...
exports.calculatePluginRiskScore = calculatePluginRiskScore;
exports.calculateSystemRiskScore = calculateSystemRiskScore;
exports.prepareTestResultsFromStats = prepareTestResultsFromStats;
...
function getStrategyMetadata(strategy) {
 ...
 */
function calculateExploitabilityScore(metadata) {
 ...
 */
function calculateComplexityScore(metadata) {
...
function calculateStrategyRiskScore(severity, successRate, metadata) {
    // Impact Base Score (0-4 points)
    const impactBase = {
        [constants_1.Severity.Critical]: 4,
        [constants_1.Severity.High]: 3,
        [constants_1.Severity.Medium]: 2,
        [constants_1.Severity.Low]: 1,
    }[severity];
    ...
    if (metadata.humanExploitable) {
        const baseHumanFactor = metadata.humanComplexity === 'low' ? 1.5 : metadata.humanComplexity === 'medium' ? 1.0 : 0.5;
...
function calculatePluginRiskScore(pluginId, severity, testResults) {
    ...
    if (!worstStrategy) {
        console.warn(`No worst strategy found for plugin ${pluginId} with severity ${severity}. ${JSON.stringify(strategyScores)}`);
        return {
            pluginId,
            severity,
            score: 0,
    ...
    const exploitabilityScore = worstStrategy.successRate > 0 ? Math.min(4, 1.5 + 2.5 * worstStrategy.successRate) : 0;
    const humanFactor = worstStrategy.metadata.humanExploitable
        ...
        : 0;
    const strategyWeight = worstStrategy.metadata.humanComplexity === 'low' && worstStrategy.successRate > 0
...
function calculateSystemRiskScore(pluginScores) {
    ...
    const components = pluginScores.reduce((acc, p) => ({
 ...
 */
function prepareTestResultsFromStats(failuresByPlugin, passesByPlugin, subCategory, categoryStats, getStrategyId) {
    // Default strategy extraction function
    const extractStrategyId = getStrategyId ||
        ((test) => {
