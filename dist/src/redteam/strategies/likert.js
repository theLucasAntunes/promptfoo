...
exports.addLikertTestCases = addLikertTestCases;
...
async function generateLikertPrompts(testCases, injectVar, config) {
    ...
    try {
        ...
        await async_1.default.forEachOfLimit(testCases, concurrency, async (testCase, index) => {
            logger_1.default.debug(`[Likert] Processing test case: ${JSON.stringify(testCase)}`);
            (0, invariant_1.default)(testCase.vars, `Likert: testCase.vars is required, but got ${JSON.stringify(testCase)}`);
            const payload = {
            ...
            logger_1.default.debug(`Got Likert jailbreak generation result for case ${Number(index) + 1}: ${JSON.stringify(data)}`);
            if (data.error) {
                logger_1.default.error(`[jailbreak:likert] Error in Likert generation: ${data.error}}`);
                logger_1.default.debug(`[jailbreak:likert] Response: ${JSON.stringify(data)}`);
                return;
            }
        ...
        logger_1.default.error(`Error in Likert generation: ${error}`);
        return [];
    }
}
async function addLikertTestCases(testCases, injectVar, config) {
    ...
    if (likertTestCases.length === 0) {
        logger_1.default.warn('No Likert jailbreak test cases were generated');
    }
