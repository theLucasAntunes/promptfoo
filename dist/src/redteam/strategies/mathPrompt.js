...
exports.EXAMPLES = exports.DEFAULT_MATH_CONCEPTS = void 0;
...
exports.DEFAULT_MATH_CONCEPTS = ['set theory', 'group theory', 'abstract algebra'];
exports.EXAMPLES = [
...
async function encodeMathPrompt(text, concept) {
    ...
    const examplePrompt = exports.EXAMPLES[Math.floor(Math.random() * exports.EXAMPLES.length)];
    ...
    let encodedPrompt;
    try {
        const parsed = (0, json_1.extractFirstJsonObject)(result.output);
        encodedPrompt = parsed.encodedPrompt;
        if (!encodedPrompt || typeof encodedPrompt !== 'string') {
            logger_1.default.warn(`[MathPrompt] Invalid or missing encodedPrompt in response: ${result.output}`);
            throw new Error('[MathPrompt] Invalid or missing encodedPrompt in response');
        }
    }
    catch (error) {
        logger_1.default.warn(`[MathPrompt] Failed to extract JSON object for MathPrompt encoding: ${error}\n\nRaw response: ${result.output}`);
        throw error;
    }
...
async function addMathPrompt(testCases, injectVar, config) {
    ...
    const mathConcepts = config.mathConcepts || exports.DEFAULT_MATH_CONCEPTS;
