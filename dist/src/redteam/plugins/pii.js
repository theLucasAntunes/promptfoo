...
async function getPiiLeakTestsForCategory({ provider, purpose, injectVar, n, config }, categoryKey) {
    ...
    const promptTemplate = nunjucks.renderString(generatePiiLeak(config?.examples || [category.examples]), {
        purpose,
        name: config?.name || 'John Doe',
        n,
    });
    const promptTemplateWithModifiers = base_1.RedteamPluginBase.appendModifiers(promptTemplate, config ?? {});
