...
exports.PolicyViolationGrader = exports.PolicyPlugin = void 0;
...
class PolicyPlugin extends base_1.RedteamPluginBase {
    constructor(provider, purpose, injectVar, config) {
        ...
        if ((0, utils_1.isValidPolicyObject)(config.policy)) {
            ...
            if (!config.policy.text) {
                throw new Error(`Policy with ID ${config.policy.id} has not been resolved. Please ensure policies are loaded from cloud.`);
            }
            ...
            this.policyId = (0, utils_1.makeInlinePolicyId)(this.policy); // Generate a unique ID for the inline policy
        }
        ...
        // error message which is meaningful to a developer debugging the issue.
            ...
            (0, invariant_1.default)(false, 'This state can only be reached if `createPluginFactory#validate` is not validating the config.');
        }
    }
    async getTemplate() {
    ...
    getAssertions(_prompt) {
        return [
            {
                type: PLUGIN_ID,
                metric: `${constants_1.POLICY_METRIC_PREFIX}:${this.policyId}`,
            },
        ];
    }
...
class PolicyViolationGrader extends base_1.RedteamGraderBase {
  ...
  `;
    ...
    async getResult(prompt, llmOutput, test, provider) {
        ...
        }, provider, undefined, undefined, true);
    }
}
