...
async function filterTestsByResults(testSuite, pathOrId, filterFn) {
    ...
    try {
        if (pathOrId.endsWith('.json')) {
            const output = await (0, index_1.readOutput)(pathOrId);
    ...
    return [...testSuite.tests].filter((test) => filteredResults.some((result) => (0, index_1.resultIsForTestCase)(result, test)));
}
