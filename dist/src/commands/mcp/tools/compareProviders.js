...
exports.registerCompareProvidersTool = registerCompareProvidersTool;
...
function registerCompareProvidersTool(server) {
    server.tool('compare_providers', {
        ...
        try {
            // Load all providers
            const apiProviders = await (0, index_1.loadApiProviders)(providers.map((p) => ({ id: p })));
            if (apiProviders.length !== providers.length) {
                return (0, utils_1.createToolResponse)('compare_providers', false, { requestedProviders: providers }, `Failed to load all providers. Loaded ${apiProviders.length} out of ${providers.length}`);
            }
            ...
            const results = await Promise.allSettled(apiProviders.map(async (provider) => {
                ...
                try {
                    const response = await (0, utils_2.withTimeout)(provider.callApi(testPrompt), timeoutMs, `Provider ${provider.id} test`);
            ...
            return (0, utils_1.createToolResponse)('compare_providers', true, {
            ...
            if (errorMessage.includes('provider')) {
                return (0, utils_1.createToolResponse)('compare_providers', false, {
                    originalError: errorMessage,
                    suggestion: 'Check provider IDs and ensure all providers are properly configured',
                }, 'Failed to load one or more providers');
            }
            return (0, utils_1.createToolResponse)('compare_providers', false, undefined, `Failed to compare providers: ${errorMessage}`);
        }
    });
}
...
//# sourceMappingURL=compareProviders.js.map