...
async function downloadDirectory(dirPath, targetDir) {
    ...
    let response = await (0, index_1.fetchWithProxy)(url, {
        headers: {
    ...
    if (!response.ok) {
        ...
        response = await (0, index_1.fetchWithProxy)(mainUrl, {
            headers: {
                Accept: 'application/vnd.github.v3+json',
                'User-Agent': 'promptfoo-cli',
            },
...
async function getExamplesList() {
    try {
        const response = await (0, index_1.fetchWithProxy)(`${GITHUB_API_BASE}/repos/promptfoo/promptfoo/contents/examples?ref=${constants_1.VERSION}`, {
            headers: {
                Accept: 'application/vnd.github.v3+json',
                'User-Agent': 'promptfoo-cli',
            },
