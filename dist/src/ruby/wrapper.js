...
exports.runRubyCode = runRubyCode;
const fs_1 = __importDefault(require("fs"));
const os_1 = __importDefault(require("os"));
const path_1 = __importDefault(require("path"));
const logger_1 = __importDefault(require("../logger"));
const rubyUtils_1 = require("./rubyUtils");
/**
...
async function runRubyCode(code, method, args) {
    const tempFilePath = path_1.default.join(os_1.default.tmpdir(), `temp-ruby-code-${Date.now()}-${Math.random().toString(16).slice(2)}.rb`);
    try {
        fs_1.default.writeFileSync(tempFilePath, code);
        // Necessary to await so temp file doesn't get deleted.
        const result = await (0, rubyUtils_1.runRuby)(tempFilePath, method, args);
        return result;
    }
    catch (error) {
        logger_1.default.error(`Error executing Ruby code: ${error}`);
        throw error;
    }
    finally {
        try {
