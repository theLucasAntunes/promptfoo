...
const { dynamicImport } = require('./dynamic-import.cjs');
...
async function importModule(modulePath, functionName) {
    logger_1.default.debug(`Attempting to import module: ${JSON.stringify({ resolvedPath: (0, pathUtils_1.safeResolve)(modulePath), moduleId: modulePath })}`);
    try {
        ...
        const resolvedPath = (0, node_url_1.pathToFileURL)((0, pathUtils_1.safeResolve)(modulePath));
        const resolvedPathStr = resolvedPath.toString();
        logger_1.default.debug(`Attempting ESM import from: ${resolvedPathStr}`);
        ...
        try {
            const importedModule = require((0, pathUtils_1.safeResolve)(modulePath));
            const mod = importedModule?.default?.default || importedModule?.default || importedModule;
            logger_1.default.debug(`Successfully required module: ${JSON.stringify({ resolvedPath: (0, pathUtils_1.safeResolve)(modulePath), moduleId: modulePath })}`);
            if (functionName) {
                logger_1.default.debug(`Returning named export: ${functionName}`);
                return mod[functionName];
            }
