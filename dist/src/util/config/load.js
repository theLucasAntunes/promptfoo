...
const index_4 = require("../../types/index");
const index_5 = require("../../util/index");
const promptfooCommand_1 = require("../promptfooCommand");
...
async function readConfig(configPath) {
    ...
    if (ext === '.json' || ext === '.yaml' || ext === '.yml') {
        ...
        const UnifiedConfigSchemaWithoutPrompts = index_4.UnifiedConfigSchema.innerType()
            .innerType()
            .extend({ prompts: index_4.UnifiedConfigSchema.innerType().innerType().shape.prompts.optional() });
        ...
        const validationResult = index_4.UnifiedConfigSchema.safeParse(imported);
        if (!validationResult.success) {
            logger_1.default.warn(`Invalid configuration file ${configPath}:\n${(0, zod_validation_error_1.fromError)(validationResult.error).message}`);
        }
        ret = imported;
...
async function combineConfigs(configPaths) {
    ...
    const makeAbsolute = (configPath, relativePath) => {
        ...
        else if (prompts_1.PromptSchema.safeParse(relativePath).success) {
            return relativePath;
        }
        else {
    ...
    const addSeenPrompt = (prompt) => {
        ...
        else if (prompts_1.PromptSchema.safeParse(prompt).success) {
            seenPrompts.add(prompt);
        }
        else {
...
async function resolveConfigs(cmdObj, _defaultConfig, type) {
    ...
    if (cmdObj.assertions) {
        ...
        const assertions = await (0, index_1.readAssertions)(cmdObj.assertions);
    ...
    if (typeof defaultTestRaw === 'string' && defaultTestRaw.startsWith('file://')) {
        ...
        cliState_1.default.basePath = basePath;
        const loaded = await (0, file_1.maybeLoadFromExternalFile)(defaultTestRaw);
    ...
    const config = {
        ...
        defaultTest: processedDefaultTest
            ? await (0, testCaseReader_1.readTest)(processedDefaultTest, basePath, true)
            : undefined,
    ...
    if (!hasConfigFile && !hasPrompts && !hasProviders && !(0, envars_1.isCI)()) {
      ...
      ${chalk_1.default.cyan(`${(0, promptfooCommand_1.promptfooCommand)('')} eval -c ${chalk_1.default.bold('path/to/promptfooconfig.yaml')}`)}

      ${chalk_1.default.white('Or create a config with:')}

      ${chalk_1.default.green((0, promptfooCommand_1.promptfooCommand)('init'))}
    `);
        process_1.default.exit(1);
    }
    if (!hasPrompts) {
    ...
    const parsedPrompts = await (0, index_2.readPrompts)(config.prompts, cmdObj.prompts ? undefined : basePath);
    const parsedProviders = await (0, index_3.loadApiProviders)(config.providers, {
        env: config.env,
        basePath,
    });
    ...
    const parsedProviderPromptMap = (0, index_2.readProviderPromptMap)(config, parsedPrompts);
    ...
    const testSuite = {
        ...
        nunjucksFilters: await (0, index_5.readFilters)(fileConfig.nunjucksFilters || defaultConfig.nunjucksFilters || {}, basePath),
