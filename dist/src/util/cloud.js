...
exports.ConfigPermissionError = void 0;
...
exports.getDefaultTeam = getDefaultTeam;
exports.getTeamById = getTeamById;
exports.resolveTeamFromIdentifier = resolveTeamFromIdentifier;
exports.resolveTeamId = resolveTeamId;
exports.checkCloudPermissions = checkCloudPermissions;
exports.canCreateTargets = canCreateTargets;
exports.getPoliciesFromCloud = getPoliciesFromCloud;
...
async function resolveTeamId(teamIdentifier, fallbackToDefault = true) {
    ...
    if (teamIdentifier) {
        logger_1.default.debug(`[Team Resolution] Using explicit team identifier: ${teamIdentifier}`);
    ...
    if (currentTeamId) {
        try {
            logger_1.default.debug(`[Team Resolution] Using stored team ID: ${currentTeamId}`);
            return await getTeamById(currentTeamId);
        }
        catch (_error) {
            logger_1.default.warn(`[Team Resolution] Stored team ${currentTeamId} no longer accessible, falling back`);
        }
    }
    // 3. Fall back to server default (oldest team)
    if (fallbackToDefault) {
        logger_1.default.debug(`[Team Resolution] Using server default team`);
        const defaultTeam = await getDefaultTeam();
        // Store the default team for future use (scoped to organization)
        cloud_1.cloudConfig.setCurrentTeamId(defaultTeam.id, defaultTeam.organizationId);
        logger_1.default.info(`Using team: ${defaultTeam.name} (use 'promptfoo auth teams set <name>' to change)`);
        return defaultTeam;
    }
...
async function checkCloudPermissions(config) {
    ...
    if (!config.providers) {
        logger_1.default.warn('No providers specified. Skipping permission check.');
        return;
    }
    try {
        const hasPermissionCheckServerFeature = await (0, server_1.checkServerFeatureSupport)(PERMISSION_CHECK_SERVER_FEATURE_NAME, PERMISSION_CHECK_SERVER_FEATURE_DATE);
        if (!hasPermissionCheckServerFeature) {
            logger_1.default.debug(`[Config Permission Check] Server feature ${PERMISSION_CHECK_SERVER_FEATURE_NAME} is not supported. Skipping permission check.`);
            return;
        }
        ...
        if (!response.ok) {
            ...
            if (response.status === 403) {
                throw new ConfigPermissionError(`Permission denied: ${convertErrorsToReadableMessage(errors)}`);
            }
            // For other errors, log and continue (existing behavior)
            logger_1.default.warn(`Error checking permissions: ${convertErrorsToReadableMessage(errors)}. Continuing anyway.`);
            return;
        }
        const result = await response.json();
        if (result.errors && result.errors.length > 0) {
            throw new ConfigPermissionError(`Not able to continue with config: ${convertErrorsToReadableMessage(result.errors)}`);
        }
        logger_1.default.debug('Permission check passed');
    }
    catch (error) {
        ...
        logger_1.default.warn(`Error checking permissions: ${error}. Continuing anyway.`);
    }
    return;
}
/**
 ...
 */
async function canCreateTargets(teamId) {
    if (!cloud_1.cloudConfig.isEnabled()) {
        logger_1.default.debug('[canCreateTargets] Cloud config is not enabled, create providers is not relevant.');
        return true;
    }
    if (!teamId) {
        const team = await getDefaultTeam();
        teamId = team.id;
        logger_1.default.debug(`[canCreateTargets] No team id provided, using default team ${team.name} (${teamId})`);
    }
    ...
    const body = await response.json();
    return body.some((ability) => ability.action === 'create' && ability.subject === 'Provider');
}
/**
...
async function getPoliciesFromCloud(ids, teamId) {
    if (!cloud_1.cloudConfig.isEnabled()) {
        throw new Error(`Could not fetch policies from cloud. Cloud config is not enabled. Please run \`promptfoo auth login\` to login.`);
    }
    try {
        ...
        logger_1.default.error(`Failed to fetch policies from cloud.`);
        logger_1.default.error(String(e));
