...
exports.deleteEvals = deleteEvals;
...
function getPromptsForTestCasesHash(testCasesSha256, limit = constants_1.DEFAULT_QUERY_LIMIT) {
...
function getPrompts(limit = constants_1.DEFAULT_QUERY_LIMIT) {
    return getPromptsWithPredicate(() => true, limit);
}
async function getTestCases(limit = constants_1.DEFAULT_QUERY_LIMIT) {
...
async function getEvals(limit = constants_1.DEFAULT_QUERY_LIMIT) {
...
function deleteEvals(ids) {
    const db = (0, index_1.getDb)();
    db.transaction(() => {
        db.delete(tables_1.evalsToPromptsTable).where((0, drizzle_orm_1.inArray)(tables_1.evalsToPromptsTable.evalId, ids)).run();
        db.delete(tables_1.evalsToDatasetsTable).where((0, drizzle_orm_1.inArray)(tables_1.evalsToDatasetsTable.evalId, ids)).run();
        db.delete(tables_1.evalsToTagsTable).where((0, drizzle_orm_1.inArray)(tables_1.evalsToTagsTable.evalId, ids)).run();
        db.delete(tables_1.evalResultsTable).where((0, drizzle_orm_1.inArray)(tables_1.evalResultsTable.evalId, ids)).run();
        db.delete(tables_1.evalsTable).where((0, drizzle_orm_1.inArray)(tables_1.evalsTable.id, ids)).run();
    });
}
/**
...
async function getStandaloneEvals({ limit = constants_1.DEFAULT_QUERY_LIMIT, tag, description, } = {}) {
    ...
    const standaloneEvals = results.flatMap((result) => {
        ...
        (0, invariant_1.default)(evalInfo, `Eval with ID ${evalId} not found in map`);
