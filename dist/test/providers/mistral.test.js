...
const mistral_1 = require("../../src/providers/mistral");
jest.mock('../../src/cache', () => ({
...
describe('Mistral', () => {
    ...
    describe('MistralChatCompletionProvider', () => {
        let provider;
        beforeEach(() => {
            provider = new mistral_1.MistralChatCompletionProvider('mistral-tiny');
        ...
        it('should create a provider with custom options', () => {
            const customProvider = new mistral_1.MistralChatCompletionProvider('mistral-medium', {
        ...
        it('should support new Magistral reasoning models', () => {
            const magistralSmallProvider = new mistral_1.MistralChatCompletionProvider('magistral-small-2506');
            expect(magistralSmallProvider.modelName).toBe('magistral-small-2506');
            expect(magistralSmallProvider.config).toEqual({});
            const magistralMediumProvider = new mistral_1.MistralChatCompletionProvider('magistral-medium-latest', {
        ...
        it('should support Pixtral multimodal model', () => {
            const pixtralProvider = new mistral_1.MistralChatCompletionProvider('pixtral-12b', {
        ...
        it('should call Mistral API and return output with correct structure', async () => {
            ...
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith(expect.stringContaining('/chat/completions'), expect.objectContaining({
                method: 'POST',
        ...
        it('should not use cache when disabled', async () => {
            ...
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith(expect.stringContaining('/chat/completions'), expect.any(Object), expect.any(Number));
        });
        ...
        it('should use custom API base URL if provided', async () => {
            const customProvider = new mistral_1.MistralChatCompletionProvider('mistral-tiny', {
            ...
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith('https://custom.mistral.ai/v1/chat/completions', expect.any(Object), expect.any(Number));
        });
    ...
    describe('MistralEmbeddingProvider', () => {
        let provider;
        beforeEach(() => {
            provider = new mistral_1.MistralEmbeddingProvider();
            jest.spyOn(provider, 'getApiKey').mockReturnValue('fake-api-key');
        });
        it('should create a provider with default options', () => {
        ...
        it('should call Mistral Embedding API and return embedding with correct structure', async () => {
            ...
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith(expect.stringContaining('/embeddings'), expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
                    'Content-Type': 'application/json',
                    Authorization: expect.stringContaining('Bearer '),
        ...
        it('should use cache for embedding when enabled', async () => {
            ...
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith(expect.stringContaining('/embeddings'), expect.any(Object), expect.any(Number));
        });
        ...
        it('should use custom API base URL if provided', async () => {
            const customProvider = new mistral_1.MistralEmbeddingProvider({
                config: { apiBaseUrl: 'https://custom.mistral.ai/v1' },
            });
            jest.spyOn(customProvider, 'getApiKey').mockReturnValue('fake-api-key');
            const mockResponse = {
            ...
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith('https://custom.mistral.ai/v1/embeddings', expect.any(Object), expect.any(Number));
        });
    });
});
//# sourceMappingURL=mistral.test.js.map