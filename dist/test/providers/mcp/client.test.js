...
describe('MCPClient', () => {
    ...
    describe('initialize', () => {
        it('should not initialize if disabled', async () => {
            mcpClient = new client_1.MCPClient({ enabled: false });
        ...
        it('should initialize with single server config', async () => {
            ...
            mcpClient = new client_1.MCPClient({
                enabled: true,
                server: {
                    command: 'npm',
                    args: ['start'],
            ...
            await mcpClient.initialize();
            expect(stdio_js_1.StdioClientTransport).toHaveBeenCalledWith({
                command: 'npm',
                args: ['start'],
                env: process.env,
            });
        ...
        it('should initialize with multiple servers', async () => {
            ...
            await mcpClient.initialize();
            expect(stdio_js_1.StdioClientTransport).toHaveBeenCalledTimes(2);
        ...
        it('should throw error for unsupported file type', async () => {
            ...
            await expect(mcpClient.initialize()).rejects.toThrow('Local server must be a .js or .py file');
        });
        it('should initialize with remote server using StreamableHTTPClientTransport', async () => {
            ...
            await mcpClient.initialize();
            expect(streamableHttp_js_1.StreamableHTTPClientTransport).toHaveBeenCalledWith(expect.any(URL), undefined);
        ...
        it('should initialize with remote server using StreamableHTTPClientTransport with headers', async () => {
            ...
            await mcpClient.initialize();
            expect(streamableHttp_js_1.StreamableHTTPClientTransport).toHaveBeenCalledWith(expect.any(URL), expect.objectContaining({
        ...
        it('should fall back to SSEClientTransport if StreamableHTTPClientTransport fails', async () => {
            ...
            await mcpClient.initialize();
            expect(streamableHttp_js_1.StreamableHTTPClientTransport).toHaveBeenCalledWith(expect.any(URL), undefined);
            expect(sse_js_1.SSEClientTransport).toHaveBeenCalledWith(expect.any(URL), undefined);
            expect(mockClient.connect).toHaveBeenCalledTimes(2);
        });
        it('should fall back to SSEClientTransport with headers if StreamableHTTPClientTransport fails', async () => {
            ...
            expect(sse_js_1.SSEClientTransport).toHaveBeenCalledWith(expect.any(URL), expect.objectContaining({
                requestInit: expect.objectContaining({
                    headers: expect.objectContaining(customHeaders),
                }),
            }));
        ...
        it('should initialize with correct client name', async () => {
            ...
            await mcpClient.initialize();
            expect(index_js_1.Client).toHaveBeenCalledWith({ name: 'promptfoo-MCP', version: '1.0.0' });
        });
    });
    describe('callTool', () => {
    ...
    describe('getAllTools', () => {
        ...
        it('should return empty array if no tools', () => {
            mcpClient = new client_1.MCPClient({ enabled: true });
