...
describe('AnthropicMessagesProvider', () => {
    ...
    beforeEach(() => {
        ...
        jest.mocked(client_1.MCPClient).mockImplementation(() => mockMCPClient);
    });
    afterEach(async () => {
        jest.clearAllMocks();
    ...
    describe('callApi', () => {
        ...
        const provider = new messages_1.AnthropicMessagesProvider('claude-3-5-sonnet-20241022', {
            config: { tools },
        });
        ...
        it('should handle APIError with error details', async () => {
            const provider = new messages_1.AnthropicMessagesProvider('claude-3-5-sonnet-20241022');
            const mockApiError = Object.create(sdk_1.APIError.prototype);
        ...
        it('should handle thinking configuration', async () => {
            const provider = new messages_1.AnthropicMessagesProvider('claude-3-7-sonnet-20250219', {
                config: {
                    thinking: {
                        type: 'enabled',
                        budget_tokens: 2048,
            ...
            expect(result.output).toBe('Thinking: Let me analyze this step by step...\nSignature: test-signature\n\nFinal answer');
        });
        ...
        it('should handle API errors for thinking configuration', async () => {
            ...
            expect(result.error).toBe('API call error: Thinking budget must be at least 1024 tokens when enabled, status 400, type invalid_request_error');
            // Test budget exceeding max_tokens
            const providerWithMaxTokens = new messages_1.AnthropicMessagesProvider('claude-3-7-sonnet-20250219', {
                config: {
                    max_tokens: 2048,
                },
            });
            const mockMaxTokensError = Object.create(sdk_1.APIError.prototype);
            ...
            const result2 = await providerWithMaxTokens.callApi(JSON.stringify([
                {
                    role: 'user',
                    content: 'test',
                    thinking: {
            ...
            expect(result2.error).toBe('API call error: Thinking budget must be less than max_tokens, status 400, type invalid_request_error');
        });
        ...
        describe('finish reason handling', () => {
            ...
            it('should handle cached responses with finishReason', async () => {
                ...
                await (0, cache_1.getCache)().set(cacheKey, JSON.stringify({
                ...
                jest
                    ...
                    .mockImplementation()
                    .mockResolvedValue({});
                const specificCacheKey = 'anthropic:' +
                    JSON.stringify({
                        model: 'claude-3-5-sonnet-20241022',
                        max_tokens: 1024,
                ...
                await (0, cache_1.getCache)().set(specificCacheKey, JSON.stringify({
                    content: [{ type: 'text', text: 'Cached response' }],
                    stop_reason: 'end_turn',
                    usage: { input_tokens: 5, output_tokens: 5, server_tool_use: null },
                }));
    ...
    describe('cleanup', () => {
        it('should await initialization before cleanup', async () => {
            provider = new messages_1.AnthropicMessagesProvider('claude-3-5-sonnet-latest', {
                config: {
                    mcp: {
                        enabled: true,
                        server: {
