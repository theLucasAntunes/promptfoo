...
const mockLogger = jest.mocked(logger_1.default);
...
describe('OpenAI Provider', () => {
    ...
    describe('OpenAiChatCompletionProvider', () => {
        ...
        it('constructor should handle config correctly', async () => {
            ...
            const provider = new chat_1.OpenAiChatCompletionProvider('gpt-4o-mini', { config });
            ...
            const call = mockFetchWithCache.mock.calls[0];
        ...
        it('should handle DeepSeek reasoning model content correctly', async () => {
            ...
            mockFetchWithCache.mockResolvedValue(mockResponse);
            const provider = new chat_1.OpenAiChatCompletionProvider('deepseek-reasoner');
            const result = await provider.callApi(JSON.stringify([{ role: 'user', content: '9.11 and 9.8, which is greater?' }]));
            expect(mockFetchWithCache).toHaveBeenCalledTimes(1);
...
Therefore, 9.11 is greater than 9.8.\n\nThe final answer is 9.11 is greater than 9.8.`;
        ...
        it('should handle multi-round conversations with DeepSeek reasoning model', async () => {
            ...
            const provider = new chat_1.OpenAiChatCompletionProvider('deepseek-reasoner');
            // First round
            const result1 = await provider.callApi(JSON.stringify([{ role: 'user', content: '9.11 and 9.8, which is greater?' }]));
...
Therefore, 9.11 is greater than 9.8.\n\nThe final answer is 9.11 is greater than 9.8.`;
            ...
            const result2 = await provider.callApi(JSON.stringify([
...
Therefore, there are 2 occurrences of the letter "r" in "strawberry".\n\nThere are 2 "r"s in the word "strawberry".`;
        ...
        describe('External Function Callbacks', () => {
            beforeEach(() => {
                cliState_1.default.basePath = '/test/base/path';
                jest.clearAllMocks();
            });
            afterEach(() => {
            ...
            it('should load and execute external function callbacks from file', async () => {
                ...
                expect(mockImportModule).toHaveBeenCalledWith(path_1.default.resolve('/test/base/path', 'test/callbacks.js'), 'testFunction');
            ...
            it('should handle errors in external function loading gracefully', async () => {
                ...
                expect(mockImportModule).toHaveBeenCalledWith(path_1.default.resolve('/test/base/path', 'nonexistent/module.js'), 'errorFunction');
            ...
            it('should handle errors in external function execution gracefully', async () => {
                ...
                expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Function callback failed for failing_function'));
            });
            it('should handle file reference parsing correctly', async () => {
                ...
                expect(mockImportModule).toHaveBeenCalledWith(path_1.default.resolve('/test/base/path', 'deep/path/module.js'), 'deepExport.nestedFunction');
            ...
            it('should handle mixed inline and external function callbacks', async () => {
                ...
                expect(mockImportModule).toHaveBeenCalledWith(path_1.default.resolve('/test/base/path', 'mixed/callbacks.js'), 'externalFunc');
        ...
        it('should identify reasoning models correctly', () => {
            const regularProvider = new chat_1.OpenAiChatCompletionProvider('gpt-4');
            const o1Provider = new chat_1.OpenAiChatCompletionProvider('o1-mini');
            const o3Provider = new chat_1.OpenAiChatCompletionProvider('o3-mini');
            const o1PreviewProvider = new chat_1.OpenAiChatCompletionProvider('o1-preview');
            const o3StandardProvider = new chat_1.OpenAiChatCompletionProvider('o3');
            const o4MiniProvider = new chat_1.OpenAiChatCompletionProvider('o4-mini');
        ...
        it('should handle temperature support correctly', () => {
            ...
            const o1PreviewProvider = new chat_1.OpenAiChatCompletionProvider('o1-preview');
            const o4MiniProvider = new chat_1.OpenAiChatCompletionProvider('o4-mini');
            const gpt41Provider = new chat_1.OpenAiChatCompletionProvider('gpt-4.1');
        ...
        it('should respect temperature settings based on model type', async () => {
            ...
            const regularCall = mockFetchWithCache.mock.calls[0];
            ...
            const o1Call = mockFetchWithCache.mock.calls[0];
        ...
        it('should handle max tokens settings based on model type', async () => {
            ...
            mockFetchWithCache.mockClear();
            const gpt5Provider = new chat_1.OpenAiChatCompletionProvider('gpt-5', {
                config: { max_tokens: 300 },
            });
            await gpt5Provider.callApi('Test prompt');
            const gpt5Call = mockFetchWithCache.mock.calls[0];
        ...
        it('should handle o4-mini with reasoning_effort and service_tier', async () => {
            ...
            const o4Provider = new chat_1.OpenAiChatCompletionProvider('o4-mini', {
                config: {
        ...
        it('should handle user, metadata, and store parameters', async () => {
            ...
            mockFetchWithCache.mockResolvedValue(mockResponse);
            const provider = new chat_1.OpenAiChatCompletionProvider('gpt-4o', {
                config: {
                    user: 'user-123',
                    metadata: {
        ...
        it('should use generic error message with fallback when apiKeyEnvar is undefined', async () => {
            ...
            try {
                const provider = new chat_1.OpenAiChatCompletionProvider('gpt-4o-mini');
                await expect(provider.callApi('Test prompt')).rejects.toThrow('API key is not set. Set the OPENAI_API_KEY environment variable or add `apiKey` to the provider config.');
            }
            finally {
        ...
        it('should use custom apiKeyEnvar in error message when provided', async () => {
            ...
            try {
                ...
                await expect(provider.callApi('Test prompt')).rejects.toThrow('API key is not set. Set the CUSTOM_API_KEY environment variable or add `apiKey` to the provider config.');
            }
            finally {
                // Restore original environment
                if (originalEnv) {
        ...
        it('should demonstrate improved logging for inherited classes', async () => {
            ...
            class CustomProvider extends chat_1.OpenAiChatCompletionProvider {
            ...
            try {
                ...
                await expect(provider.callApi('Test prompt')).rejects.toThrow('API key is not set. Set the CUSTOM_PROVIDER_API_KEY environment variable or add `apiKey` to the provider config.');
        ...
        it('should log generic message for unknown chat models', () => {
            new chat_1.OpenAiChatCompletionProvider('unknown-model');
        ...
        it('should not log unknown model message for known OpenAI models', () => {
            new chat_1.OpenAiChatCompletionProvider('gpt-4o-mini');
        ...
        it('should support legacy model IDs', () => {
            ...
            const gpt4LegacyProvider = new chat_1.OpenAiChatCompletionProvider('gpt-4-0314');
            expect(gpt4LegacyProvider.modelName).toBe('gpt-4-0314');
            const gpt4_32kProvider = new chat_1.OpenAiChatCompletionProvider('gpt-4-32k-0314');
            expect(gpt4_32kProvider.modelName).toBe('gpt-4-32k-0314');
            const gpt4VisionProvider = new chat_1.OpenAiChatCompletionProvider('gpt-4-vision-preview');
            expect(gpt4VisionProvider.modelName).toBe('gpt-4-vision-preview');
            // Test legacy GPT-3.5 models
            const gpt35LegacyProvider = new chat_1.OpenAiChatCompletionProvider('gpt-3.5-turbo-0301');
            expect(gpt35LegacyProvider.modelName).toBe('gpt-3.5-turbo-0301');
            const gpt35_16kProvider = new chat_1.OpenAiChatCompletionProvider('gpt-3.5-turbo-16k');
            expect(gpt35_16kProvider.modelName).toBe('gpt-3.5-turbo-16k');
            // Test latest audio model
            const audioModelProvider = new chat_1.OpenAiChatCompletionProvider('gpt-4o-audio-preview-2025-06-03');
