...
describe('OpenAiModerationProvider', () => {
    ...
    beforeEach(() => {
        jest.resetAllMocks();
        jest.mocked(cache_1.isCacheEnabled).mockReturnValue(false);
    ...
    const createProvider = (modelName = 'text-moderation-latest') => {
        return new moderation_1.OpenAiModerationProvider(modelName, {
    ...
    describe('Basic functionality', () => {
        it('should moderate content and detect harmful content', async () => {
            ...
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith(expect.stringContaining('/moderations'), expect.objectContaining({
                method: 'POST',
            ...
            }), expect.any(Number), 'json', false, undefined);
        });
    ...
    describe('Caching', () => {
        it('should use cache when enabled', async () => {
            ...
            jest.mocked(cache_1.getCache).mockReturnValue(mockCache);
            const result = await provider.callModerationApi('user input', 'assistant response');
            expect(result).toEqual(mockResponse);
            expect(mockCache.get).toHaveBeenCalledWith(expect.stringContaining('openai:moderation:text-moderation-latest:'));
        ...
        it('should store results in cache when caching is enabled', async () => {
            ...
            expect(mockCache.set).toHaveBeenCalledWith(expect.stringContaining('openai:moderation:'), expect.stringContaining('{"flags":[{"code":"hate"'));
        });
    });
    ...
    describe('Model configuration', () => {
        it('should warn about unknown models', () => {
            ...
            new moderation_1.OpenAiModerationProvider('unknown-model', {
                config: { apiKey: 'test-key' },
            });
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('unknown OpenAI moderation model'));
        });
        it('should accept custom API headers', async () => {
            const provider = new moderation_1.OpenAiModerationProvider('text-moderation-latest', {
                config: {
                    apiKey: 'test-key',
                    headers: {
...
describe('Moderation Utility Functions', () => {
    describe('Input Type Guards', () => {
        it('should correctly identify text inputs', () => {
            const textInput = { type: 'text', text: 'test content' };
            const imageInput = {
                type: 'image_url',
                image_url: { url: 'https://example.com/image.jpg' },
            };
            expect((0, moderation_1.isTextInput)(textInput)).toBe(true);
            expect((0, moderation_1.isTextInput)(imageInput)).toBe(false);
        });
        it('should correctly identify image inputs', () => {
            ...
            expect((0, moderation_1.isImageInput)(imageInput)).toBe(true);
            expect((0, moderation_1.isImageInput)(textInput)).toBe(false);
        });
    });
    describe('supportsImageInput', () => {
        it('should return true for multi-modal models', () => {
            expect((0, moderation_1.supportsImageInput)('omni-moderation-latest')).toBe(true);
            expect((0, moderation_1.supportsImageInput)('omni-moderation-2024-09-26')).toBe(true);
        });
        it('should return false for text-only models', () => {
            expect((0, moderation_1.supportsImageInput)('text-moderation-latest')).toBe(false);
            expect((0, moderation_1.supportsImageInput)('text-moderation-stable')).toBe(false);
            expect((0, moderation_1.supportsImageInput)('text-moderation-007')).toBe(false);
        });
        it('should return false for unknown models', () => {
            expect((0, moderation_1.supportsImageInput)('nonexistent-model')).toBe(false);
        });
    });
    describe('formatModerationInput', () => {
        it('should return string as-is for text-only models', () => {
            const input = 'test content';
            expect((0, moderation_1.formatModerationInput)(input, false)).toBe(input);
        });
        it('should wrap string in TextInput object for multi-modal models', () => {
            const input = 'test content';
            const expected = [{ type: 'text', text: input }];
            expect((0, moderation_1.formatModerationInput)(input, true)).toEqual(expected);
        });
        it('should return array inputs as-is for multi-modal models', () => {
            ...
            expect((0, moderation_1.formatModerationInput)(input, true)).toEqual(input);
        });
        it('should filter out image inputs for text-only models', () => {
            ...
            expect((0, moderation_1.formatModerationInput)(input, false)).toBe('text content');
        });
        it('should join multiple text inputs when filtering for text-only models', () => {
            ...
            expect((0, moderation_1.formatModerationInput)(input, false)).toBe('first text second text');
        });
    });
});
