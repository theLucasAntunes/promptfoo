...
const realtime_1 = require("../../../src/providers/openai/realtime");
// Mock WebSocket
jest.mock('ws');
...
describe('OpenAI Realtime Provider', () => {
    ...
    describe('Basic Functionality', () => {
        it('should initialize with correct model and config', () => {
            ...
            const provider = new realtime_1.OpenAiRealtimeProvider('gpt-4o-realtime-preview', { config });
        ...
        it('should initialize with gpt-realtime model and new voices', () => {
            const config = {
                ...
                instructions: 'Test instructions',
                voice: 'cedar', // New voice for gpt-realtime
            };
            const provider = new realtime_1.OpenAiRealtimeProvider('gpt-realtime', { config });
        ...
        it('should support marin voice for gpt-realtime model', () => {
            const config = {
                modalities: ['text', 'audio'],
                instructions: 'Test instructions',
                voice: 'marin', // New voice for gpt-realtime
            };
            const provider = new realtime_1.OpenAiRealtimeProvider('gpt-realtime', { config });
            expect(provider.modelName).toBe('gpt-realtime');
        ...
        it('should log warning for unknown model', () => {
            new realtime_1.OpenAiRealtimeProvider('unknown-model');
            expect(logger_1.default.debug).toHaveBeenCalledWith('Using unknown OpenAI realtime model: unknown-model');
        });
        ...
        it('should handle basic text response with persistent connection', async () => {
            ...
            lastHandler(Buffer.from(JSON.stringify({
        ...
        it('should maintain conversation context across multiple messages', async () => {
            ...
            const simulateMessageSequence = async (messageId, assistantId, responseId, responseText) => {
                const messageHandlers = mockHandlers.message;
                const lastHandler = messageHandlers[messageHandlers.length - 1];
                // User message
                await Promise.resolve(lastHandler(Buffer.from(JSON.stringify({
                    type: 'conversation.item.created',
        ...
        it('should reuse existing connection for subsequent requests', async () => {
            ...
            const provider = new realtime_1.OpenAiRealtimeProvider('gpt-4o-realtime-preview', {
    ...
    describe('WebSocket URL configuration', () => {
        ...
        it('uses default OpenAI base for direct WebSocket', async () => {
            ...
            simulateMinimalFlow();
            await promise;
            const constructedUrl = MockWebSocket.mock.calls[0][0];
            expect(constructedUrl).toBe('wss://api.openai.com/v1/realtime?model=' + encodeURIComponent('gpt-4o-realtime-preview'));
        });
        it('converts custom https apiBaseUrl to wss for direct WebSocket', async () => {
            ...
            await promise;
            const constructedUrl = MockWebSocket.mock.calls[0][0];
            const wsOptions = MockWebSocket.mock.calls[0][1];
            expect(constructedUrl).toBe('wss://my-custom-api.com/v1/realtime?model=' +
        ...
        it('converts custom http apiBaseUrl to ws for direct WebSocket', async () => {
            ...
            const constructedUrl = MockWebSocket.mock.calls[0][0];
            const wsOptions = MockWebSocket.mock.calls[0][1];
            expect(constructedUrl).toBe('ws://localhost:8080/v1/realtime?model=' + encodeURIComponent('gpt-4o-realtime-preview'));
        ...
        it('uses apiBaseUrl for client-secret socket URL', async () => {
            ...
            const constructedUrl = MockWebSocket.mock.calls[0][0];
            const wsOptions = MockWebSocket.mock.calls[0][1];
            expect(constructedUrl).toBe('wss://my-custom-api.com/v1/realtime/socket?client_secret=' +
