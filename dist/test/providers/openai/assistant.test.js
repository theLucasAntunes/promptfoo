...
describe('OpenAI Provider', () => {
    ...
    describe('OpenAiAssistantProvider', () => {
        ...
        it('should handle API errors', async () => {
            const error = new openai_1.default.APIError(500, {}, 'API Error', new Headers());
    ...
    describe('Function Callbacks with Context', () => {
        ...
        it('should pass context to function callbacks', async () => {
            ...
            const provider = new assistant_1.OpenAiAssistantProvider('asst_test', {
                config: {
                    apiKey: 'test-key',
                    functionToolCallbacks: {
        ...
        it('should work with callbacks that do not use context', async () => {
            ...
            expect(oldStyleCallback).toHaveBeenCalledWith({ param: 'value' }, expect.objectContaining({
                threadId: 'thread_test',
                runId: 'run_test',
                assistantId: 'asst_test',
                provider: 'openai',
        ...
        it('should handle string-based function callbacks', async () => {
            const provider = new assistant_1.OpenAiAssistantProvider('asst_test', {
                config: {
                    apiKey: 'test-key',
                    functionToolCallbacks: {
                        string_function: '(args, context) => { return `received: ${JSON.stringify(args)} with context: ${JSON.stringify(context)}`; }',
                    },
                },
            });
            const mockRun = {
        ...
        it('should handle callbacks that access context properties', async () => {
            ...
            expect(contextAwareCallback).toHaveBeenCalledWith({ user_id: '123' }, expect.objectContaining({
                threadId: 'thread_test',
                runId: 'run_test',
                assistantId: 'asst_test',
                provider: 'openai',
