...
const fal_1 = require("../../src/providers/fal");
// Mock the fal client
...
describe('Fal Provider', () => {
    ...
    describe('FalImageGenerationProvider', () => {
        let provider;
        beforeEach(() => {
            provider = new fal_1.FalImageGenerationProvider('fal-ai/flux/schnell', {
        ...
        describe('constructor and configuration', () => {
            ...
            it('should create provider with custom configuration options', () => {
                const customProvider = new fal_1.FalImageGenerationProvider('fal-ai/fast-sdxl', {
                    config: {
                        apiKey: 'custom-key',
                        seed: 12345,
            ...
            it('should use environment variable for API key when not provided in config', () => {
                jest.mocked(jest.mocked(envars_1.getEnvString)).mockReturnValue('env-api-key');
                const envProvider = new fal_1.FalImageGenerationProvider('fal-ai/flux/schnell');
                expect(envProvider.apiKey).toBe('env-api-key');
                expect(envars_1.getEnvString).toHaveBeenCalledWith('FAL_KEY');
            });
        ...
        describe('API key validation', () => {
            it('should throw error when API key is not set', async () => {
                jest.mocked(jest.mocked(envars_1.getEnvString)).mockReturnValue(undefined);
                const noKeyProvider = new fal_1.FalImageGenerationProvider('fal-ai/flux/schnell');
                await expect(noKeyProvider.callApi('test prompt')).rejects.toThrow('fal.ai API key is not set. Set the FAL_KEY environment variable or or add `apiKey` to the provider config.');
            });
        });
        describe('API calls and image generation', () => {
            ...
            it('should merge provider config with prompt context config', async () => {
                const providerWithConfig = new fal_1.FalImageGenerationProvider('fal-ai/flux/schnell', {
                    config: {
                        apiKey: 'test-api-key',
                        seed: 12345,
                        num_inference_steps: 8,
        ...
        describe('caching behavior', () => {
            it('should use cached response when cache is enabled and available', async () => {
                ...
                expect(mockCache.get).toHaveBeenCalledWith(expect.stringContaining('fal:fal-ai/flux/schnell:'));
            });
            it('should set cache when enabled and response is fresh', async () => {
                ...
                expect(mockCache.set).toHaveBeenCalledWith(expect.stringContaining('fal:fal-ai/flux/schnell:'), JSON.stringify('![test prompt](https://example.com/image.png)'));
            });
        ...
        describe('client initialization', () => {
            it('should lazy load the fal client', async () => {
                jest.clearAllMocks();
                const newProvider = new fal_1.FalImageGenerationProvider('fal-ai/flux/schnell', {
                    config: { apiKey: 'test-api-key' },
                });
                const mockResponse = {
                    data: {
