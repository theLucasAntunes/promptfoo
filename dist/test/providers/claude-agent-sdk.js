...
const claude_agent_sdk_1 = require("../../src/providers/claude-agent-sdk");
...
const mockTransformMCPConfigToClaudeCode = jest.mocked(transform_1.transformMCPConfigToClaudeCode);
// Helper to create a complete NonNullableUsage object
const createMockUsage = (input = 0, output = 0) => ({
    input_tokens: input,
...
const createMockResponse = (result, usage, cost = 0.001, sessionId = 'test-session-123') => {
...
const createMockErrorResponse = (errorSubtype) => {
    return createMockQuery({
        type: 'result',
...
describe('ClaudeCodeSDKProvider', () => {
    ...
    beforeEach(() => {
        ...
        // File system mocks
        tempDirSpy = jest.spyOn(fs_1.default, 'mkdtempSync').mockReturnValue('/tmp/test-temp-dir');
        statSyncSpy = jest.spyOn(fs_1.default, 'statSync').mockReturnValue({
            isDirectory: () => true,
        });
        rmSyncSpy = jest.spyOn(fs_1.default, 'rmSync').mockImplementation();
    });
    afterEach(async () => {
    ...
    describe('constructor', () => {
        it('should initialize with default config', () => {
            const provider = new claude_agent_sdk_1.ClaudeCodeSDKProvider();
        ...
        it('should initialize with custom config', () => {
            ...
            const provider = new claude_agent_sdk_1.ClaudeCodeSDKProvider({ config });
            expect(provider.config).toEqual(config);
            expect(provider.apiKey).toBe('test-key');
        });
        it('should use custom id when provided', () => {
            const provider = new claude_agent_sdk_1.ClaudeCodeSDKProvider({ id: 'custom-provider-id' });
            expect(provider.id()).toBe('custom-provider-id');
        });
        it('should warn about unknown model', () => {
            const warnSpy = jest.spyOn(logger_1.default, 'warn').mockImplementation();
            new claude_agent_sdk_1.ClaudeCodeSDKProvider({ config: { model: 'unknown-model' } });
            expect(warnSpy).toHaveBeenCalledWith('Using unknown model for Claude Agent SDK: unknown-model');
            warnSpy.mockRestore();
        });
        it('should warn about unknown fallback model', () => {
            const warnSpy = jest.spyOn(logger_1.default, 'warn').mockImplementation();
            new claude_agent_sdk_1.ClaudeCodeSDKProvider({ config: { fallback_model: 'unknown-fallback' } });
            expect(warnSpy).toHaveBeenCalledWith('Using unknown model for Claude Agent SDK fallback: unknown-fallback');
            warnSpy.mockRestore();
        });
        it('should not warn about Claude Agent SDK model aliases', () => {
            const warnSpy = jest.spyOn(logger_1.default, 'warn').mockImplementation();
            // Test all Claude Agent SDK aliases
            claude_agent_sdk_1.CLAUDE_CODE_MODEL_ALIASES.forEach((alias) => {
                new claude_agent_sdk_1.ClaudeCodeSDKProvider({ config: { model: alias } });
                new claude_agent_sdk_1.ClaudeCodeSDKProvider({ config: { fallback_model: alias } });
            });
        ...
        it('should not warn about known Anthropic models', () => {
            const warnSpy = jest.spyOn(logger_1.default, 'warn').mockImplementation();
            new claude_agent_sdk_1.ClaudeCodeSDKProvider({ config: { model: 'claude-3-5-sonnet-20241022' } });
            new claude_agent_sdk_1.ClaudeCodeSDKProvider({ config: { fallback_model: 'claude-3-5-haiku-20241022' } });
            expect(warnSpy).not.toHaveBeenCalled();
            warnSpy.mockRestore();
        });
    });
    describe('callApi', () => {
        describe('basic functionality', () => {
            it('should successfully call API with simple prompt', async () => {
                mockQuery.mockReturnValue(createMockResponse('Test response', { input_tokens: 10, output_tokens: 20 }, 0.002));
                const provider = new claude_agent_sdk_1.ClaudeCodeSDKProvider({
            ...
            it('should handle SDK exceptions', async () => {
                const errorSpy = jest.spyOn(logger_1.default, 'error').mockImplementation();
                ...
                expect(errorSpy).toHaveBeenCalledWith('Error calling Claude Agent SDK: Error: Network error');
            ...
            it('should return error when API key is missing', async () => {
                ...
                const provider = new claude_agent_sdk_1.ClaudeCodeSDKProvider();
                await expect(provider.callApi('Test prompt')).rejects.toThrow(/Anthropic API key is not set/);
            });
        ...
        describe('working directory management', () => {
            ...
            it('should error when working_dir does not exist', async () => {
                ...
                await expect(provider.callApi('Test prompt')).rejects.toThrow(/Working dir \/nonexistent\/dir does not exist/);
            });
            it('should error when working_dir is not a directory', async () => {
                ...
                await expect(provider.callApi('Test prompt')).rejects.toThrow('Working dir /path/to/file.txt is not a directory');
            });
        ...
        describe('tool configuration', () => {
            ...
            it('should use read-only tools by default when working directory is provided', async () => {
                ...
                expect(mockQuery).toHaveBeenCalledWith({
                    prompt: 'Test prompt',
                    options: expect.objectContaining({
                        allowedTools: claude_agent_sdk_1.FS_READONLY_ALLOWED_TOOLS,
                    }),
                });
            });
            it('should handle allowed tools configuration', async () => {
                ...
                const provider1 = new claude_agent_sdk_1.ClaudeCodeSDKProvider({
                    config: { working_dir: './test-dir' },
                    env: { ANTHROPIC_API_KEY: 'test-api-key' },
                });
                ...
                mockQuery.mockClear();
                const provider2 = new claude_agent_sdk_1.ClaudeCodeSDKProvider({
                ...
                mockQuery.mockClear();
                const provider3 = new claude_agent_sdk_1.ClaudeCodeSDKProvider({
                    config: {
                ...
                expect(mockQuery).toHaveBeenCalledWith({
                    prompt: 'Test prompt',
                    options: expect.objectContaining({
                        allowedTools: [...claude_agent_sdk_1.FS_READONLY_ALLOWED_TOOLS, 'Write'],
                    }),
                });
            });
            it('should handle disallowed tools and conflicting configurations', async () => {
                ...
                await expect(provider2.callApi('Test prompt')).rejects.toThrow('Cannot specify both custom_allowed_tools and append_allowed_tools');
            });
        });
        ...
        describe('caching behavior', () => {
            it('should cache responses', async () => {
                mockQuery.mockImplementation(() => createMockResponse('Cached response', { input_tokens: 10, output_tokens: 20 }));
                const provider = new claude_agent_sdk_1.ClaudeCodeSDKProvider({
                    env: { ANTHROPIC_API_KEY: 'test-api-key' },
                });
            ...
            it('should respect bustCache parameter', async () => {
                mockQuery.mockReturnValue(createMockResponse('Response v1', { input_tokens: 10, output_tokens: 20 }));
                const provider = new claude_agent_sdk_1.ClaudeCodeSDKProvider({
                    env: { ANTHROPIC_API_KEY: 'test-api-key' },
                });
                ...
                mockQuery.mockReturnValue(createMockResponse('Response v2', { input_tokens: 15, output_tokens: 25 }));
