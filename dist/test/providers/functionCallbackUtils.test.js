...
const functionCallbackUtils_1 = require("../../src/providers/functionCallbackUtils");
// Mock dependencies
...
describe('FunctionCallbackHandler', () => {
    let handler;
    beforeEach(() => {
        handler = new functionCallbackUtils_1.FunctionCallbackHandler();
        jest.clearAllMocks();
    });
    describe('processCall', () => {
        ...
        it('should return original call on callback error', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith('Function callback failed for testFunction: Error: Callback failed');
        });
    ...
    describe('executeCallback', () => {
        ...
        it('should load external file-based callbacks', async () => {
            ...
            expect(mockImportModule).toHaveBeenCalledWith(path_1.default.resolve('/test/basePath', 'path/to/function.js'));
        ...
        it('should throw error for invalid callback configuration', async () => {
            const callbacks = {
                testFunction: 123, // Invalid callback type
            };
            const call = { name: 'testFunction', arguments: '{}' };
            const result = await handler.processCall(call, callbacks);
        ...
        it('should handle errors when loading external functions', async () => {
            ...
            expect(result.isError).toBe(true);
            expect(result.output).toBe(JSON.stringify(call));
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Function callback failed for testFunction'));
        });
    ...
    describe('clearCache', () => {
        it('should clear cached callbacks', async () => {
            ...
            const result = await handler.processCall({ name: 'testFunction', arguments: '{}' }, newCallbacks);
    ...
    describe('MCP Integration', () => {
        ...
        beforeEach(() => {
            ...
            handler = new functionCallbackUtils_1.FunctionCallbackHandler(mockMCPClient);
        });
        ...
        it('should fall back to function callbacks when tool is not an MCP tool', async () => {
            ...
            const callbacks = {
                regular_function: async (args) => 'callback result',
            };
        ...
        it('should prioritize MCP tools over function callbacks when both exist', async () => {
            ...
            const callbacks = {
                shared_name: async (args) => 'callback result',
            };
        ...
        it('should work without MCP client (backwards compatibility)', async () => {
            const handlerWithoutMCP = new functionCallbackUtils_1.FunctionCallbackHandler();
            const callbacks = {
                test_function: async (args) => 'no MCP result',
            };
...
//# sourceMappingURL=functionCallbackUtils.test.js.map