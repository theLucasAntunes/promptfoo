...
const mockedGetEnvString = jest.mocked(envars_1.getEnvString);
describe('GitHub Models Default Providers', () => {
    ...
    it('should use GitHub token for github: model when only GITHUB_TOKEN is available', async () => {
        ...
        mockedGetEnvString.mockImplementation((key, defaultValue = '') => {
            if (key === 'GITHUB_TOKEN') {
    ...
    it('should use GitHub with env overrides', async () => {
        mockedGetEnvString.mockImplementation((key, defaultValue = '') => defaultValue);
        const providers = await (0, defaults_1.getDefaultProviders)({
