...
describe('CloudflareAi Provider', () => {
    ...
    describe('CloudflareAiChatCompletionProvider', () => {
        ...
        it('Can be invoked with custom configuration', async () => {
            ...
            expect(providers[0]).toBeInstanceOf(cloudflare_ai_1.CloudflareAiChatCompletionProvider);
        ...
        it('Should use custom API base URL when provided', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith('https://custom-cloudflare-api.example.com/v1/chat/completions', expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
        ...
        it('requires API key', () => {
            expect(() => new cloudflare_ai_1.CloudflareAiChatCompletionProvider(testModelName, {
        ...
        it('throws error when no configuration and no environment variables', () => {
            ...
            delete process.env.CLOUDFLARE_ACCOUNT_ID;
            delete process.env.CLOUDFLARE_API_KEY;
            expect(() => new cloudflare_ai_1.CloudflareAiChatCompletionProvider(testModelName, {})).toThrow('Cloudflare API token required');
        });
    });
    describe('CloudflareAiCompletionProvider', () => {
        it('Should handle completion provider', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('completions'), expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
                    'Content-Type': 'application/json',
    ...
    describe('CloudflareAiEmbeddingProvider', () => {
        ...
        it('Should handle passthrough parameters correctly', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('embeddings'), expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer test-key',
    ...
    describe('createCloudflareAiProvider', () => {
        it('throws an error if no model name is provided', () => {
            expect(() => (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:chat:')).toThrow('Model name is required');
        });
        it('creates correct provider types', () => {
            const chatProvider = (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:chat:llama-3', {
                config: cloudflareMinimumConfig,
            });
            expect(chatProvider).toBeInstanceOf(cloudflare_ai_1.CloudflareAiChatCompletionProvider);
            const completionProvider = (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:completion:llama-3', {
                config: cloudflareMinimumConfig,
            });
            expect(completionProvider).toBeInstanceOf(cloudflare_ai_1.CloudflareAiCompletionProvider);
            const embeddingProvider = (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:embedding:bge-base', {
                config: cloudflareMinimumConfig,
            });
            expect(embeddingProvider).toBeInstanceOf(cloudflare_ai_1.CloudflareAiEmbeddingProvider);
            const embeddingsProvider = (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:embeddings:bge-base', {
                config: cloudflareMinimumConfig,
            });
            expect(embeddingsProvider).toBeInstanceOf(cloudflare_ai_1.CloudflareAiEmbeddingProvider);
        });
        it('throws error for unknown model types', () => {
            expect(() => (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:unknown:model', {
                config: cloudflareMinimumConfig,
        ...
        it('handles model names with multiple colons correctly', () => {
            const provider = (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:chat:@cf/meta/llama-3:latest', {
                config: cloudflareMinimumConfig,
            });
            expect(provider).toBeInstanceOf(cloudflare_ai_1.CloudflareAiChatCompletionProvider);
        ...
        it('works with undefined options when environment variables are set', () => {
            process.env.CLOUDFLARE_ACCOUNT_ID = 'env-account';
            process.env.CLOUDFLARE_API_KEY = 'env-key';
            const provider = (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:chat:test-model', undefined);
            expect(provider).toBeInstanceOf(cloudflare_ai_1.CloudflareAiChatCompletionProvider);
            // Clean up
            delete process.env.CLOUDFLARE_ACCOUNT_ID;
            delete process.env.CLOUDFLARE_API_KEY;
        ...
        it('works with empty options object when environment variables are set', () => {
            process.env.CLOUDFLARE_ACCOUNT_ID = 'env-account';
            process.env.CLOUDFLARE_API_KEY = 'env-key';
            const provider = (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:chat:test-model', {});
            expect(provider).toBeInstanceOf(cloudflare_ai_1.CloudflareAiChatCompletionProvider);
            // Clean up
            delete process.env.CLOUDFLARE_ACCOUNT_ID;
            delete process.env.CLOUDFLARE_API_KEY;
        ...
        it('passes through provider options correctly', () => {
            ...
            const provider = (0, cloudflare_ai_1.createCloudflareAiProvider)('cloudflare-ai:chat:test-model', customOptions);
            expect(provider).toBeInstanceOf(cloudflare_ai_1.CloudflareAiChatCompletionProvider);
...
//# sourceMappingURL=cloudflare-ai.test.js.map