...
const responses_1 = require("../../../src/providers/azure/responses");
...
jest.mock('fs');
const mockFetchWithCache = cache_1.fetchWithCache;
const mockMaybeLoadFromExternalFile = file_1.maybeLoadFromExternalFile;
...
describe('AzureResponsesProvider', () => {
    ...
    describe('constructor', () => {
        it('should create an instance with deployment name', () => {
            const provider = new responses_1.AzureResponsesProvider('gpt-4.1-test');
            expect(provider).toBeInstanceOf(responses_1.AzureResponsesProvider);
            expect(provider.deploymentName).toBe('gpt-4.1-test');
        });
    });
    describe('isReasoningModel', () => {
        it('should identify o1 models as reasoning models', () => {
            const provider = new responses_1.AzureResponsesProvider('o1-preview');
            expect(provider.isReasoningModel()).toBe(true);
        });
        it('should identify o3 models as reasoning models', () => {
            const provider = new responses_1.AzureResponsesProvider('o3-mini');
            expect(provider.isReasoningModel()).toBe(true);
        });
        it('should identify gpt-5 models as reasoning models', () => {
            const provider = new responses_1.AzureResponsesProvider('gpt-5');
            expect(provider.isReasoningModel()).toBe(true);
        });
        it('should not identify gpt-4.1 as reasoning model', () => {
            const provider = new responses_1.AzureResponsesProvider('gpt-4.1');
    ...
    describe('callApi', () => {
        beforeEach(() => {
            ...
            responses_1.AzureResponsesProvider.prototype.ensureInitialized = jest.fn().mockResolvedValue(void 0);
            responses_1.AzureResponsesProvider.prototype.getApiBaseUrl = jest
                .fn()
                .mockReturnValue('https://test.openai.azure.com');
            Object.defineProperty(responses_1.AzureResponsesProvider.prototype, 'authHeaders', {
        ...
        it('should provide clear error for missing API host', async () => {
            ...
            await expect(provider.callApi('test')).rejects.toThrow(/Azure API configuration missing.*AZURE_API_HOST/);
        });
        it('should provide clear error for missing authentication', async () => {
            ...
            jest.spyOn(provider, 'ensureInitialized').mockImplementation(async () => {
                provider.authHeaders = {}; // Set empty auth headers
            });
            await expect(provider.callApi('test')).rejects.toThrow(/Azure API authentication failed.*AZURE_API_KEY/);
        });
        it('should validate external response_format files', async () => {
            ...
            await expect(provider.callApi('test')).rejects.toThrow(/Failed to load response_format file.*missing\.json/);
        });
        it('should make successful API call', async () => {
            ...
            expect(mockFetchWithCache).toHaveBeenCalledWith('https://test.openai.azure.com/openai/v1/responses?api-version=preview', expect.objectContaining({
                method: 'POST',
        ...
        it('should handle deep research model timeout', async () => {
            const provider = new responses_1.AzureResponsesProvider('o3-deep-research-test');
            mockFetchWithCache.mockResolvedValue({
                data: {
                    output: [
                        {
            ...
            expect(mockFetchWithCache).toHaveBeenCalledWith(expect.any(String), expect.any(Object), 600000, // 10 minutes timeout for deep research
            'json', undefined);
        });
        it('should construct correct Azure URL format', async () => {
            ...
            expect(mockFetchWithCache).toHaveBeenCalledWith('https://test.openai.azure.com/openai/v1/responses?api-version=preview', expect.any(Object), expect.any(Number), 'json', undefined);
        });
    });
});
