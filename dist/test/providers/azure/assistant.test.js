...
const assistant_1 = require("../../../src/providers/azure/assistant");
...
describe('Azure Assistant Provider', () => {
    let provider;
    const mockSleep = jest.mocked(time_1.sleep);
    const originalFunction = global.Function;
    beforeEach(() => {
        jest.clearAllMocks();
        provider = new assistant_1.AzureAssistantProvider('test-deployment', {
            config: {
                apiKey: 'test-key',
        ...
        jest.spyOn(provider, 'makeRequest').mockImplementation(jest.fn());
        jest.spyOn(provider, 'getHeaders').mockResolvedValue({
            'Content-Type': 'application/json',
            'api-key': 'test-key',
        });
        jest.spyOn(provider, 'getApiKey').mockReturnValue('test-key');
        jest.spyOn(provider, 'getApiBaseUrl').mockReturnValue('https://test.azure.com');
        jest.spyOn(provider, 'ensureInitialized').mockResolvedValue(undefined);
    ...
    describe('basic functionality', () => {
        it('should be instantiable', () => {
            const provider = new assistant_1.AzureAssistantProvider('test-deployment');
        ...
        it('should store config options', () => {
            ...
            const provider = new assistant_1.AzureAssistantProvider('test-deployment', options);
            expect(provider.deploymentName).toBe('test-deployment');
    ...
    describe('callApi', () => {
        it('should throw an error if API key is not set', async () => {
            jest.spyOn(provider, 'getApiKey').mockReturnValue(null);
        ...
        it('should throw an error if API host is not set', async () => {
            jest.spyOn(provider, 'getApiBaseUrl').mockReturnValue(null);
        ...
        it('should create a thread, add a message, and run an assistant', async () => {
            const testProvider = new assistant_1.AzureAssistantProvider('test-deployment', {
                config: {
                    apiKey: 'test-key',
                    apiHost: 'test.azure.com',
                },
    ...
    describe('error handling', () => {
        it('should handle rate limit errors', async () => {
            provider.makeRequest.mockRejectedValueOnce(new Error('rate limit exceeded'));
            const result = await provider.callApi('test prompt');
            expect(result.error).toContain('Rate limit exceeded');
        });
        it('should handle service errors', async () => {
            provider.makeRequest.mockRejectedValueOnce(new Error('Service unavailable'));
            const result = await provider.callApi('test prompt');
            expect(result.error).toContain('Service error');
        });
        it('should handle server errors', async () => {
            provider.makeRequest.mockRejectedValueOnce(new Error('500 Server Error'));
            const result = await provider.callApi('test prompt');
            expect(result.error).toContain('Error in Azure Assistant API call');
        });
        it('should handle thread with run in progress errors', async () => {
            provider.makeRequest.mockRejectedValueOnce(new Error("Can't add messages to thread while a run is in progress"));
            const result = await provider.callApi('test prompt');
            expect(result.error).toContain('Error in Azure Assistant API call');
        });
        it('should handle content filter errors during API call', async () => {
            provider.makeRequest.mockRejectedValueOnce(new Error('Content filter triggered: The input contained inappropriate content'));
        ...
        it('should handle guardrail errors during API call', async () => {
            provider.makeRequest.mockRejectedValueOnce(new Error('guardrail violation detected in response'));
            const result = await provider.callApi('test prompt');
            expect(result).toEqual({
                output: "The generated content was filtered due to triggering Azure OpenAI Service's content filtering system.",
                guardrails: {
    ...
    describe('pollRun', () => {
        it('should poll until run is completed', async () => {
            ...
            jest.spyOn(provider, 'pollRun').mockImplementation(async () => {
                // Simulate sleep call to verify it was made
                await mockSleep(1000);
                return completedResponse;
            });
            // Call the mocked method directly
            const result = await provider.pollRun('https://test.azure.com', '2024-04-01-preview', 'thread-123', 'run-123');
        ...
        it('should throw error when polling times out', async () => {
            ...
            await expect(provider.pollRun('https://test.azure.com', '2024-04-01-preview', 'thread-123', 'run-123')).rejects.toThrow('Run polling timed out');
        });
        it('should increase polling interval after 30 seconds', async () => {
            ...
            jest.spyOn(provider, 'pollRun').mockImplementation(simulatePolling);
            await provider.pollRun('https://test.azure.com', '2024-04-01-preview', 'thread-123', 'run-123');
            // Verify sleep calls
    ...
    describe('function tool handling', () => {
        it('should handle function tool calls and submit outputs', async () => {
            ...
            jest.spyOn(provider, 'getApiBaseUrl').mockReturnValue('https://test.azure.com');
            jest
                .spyOn(provider, 'processCompletedRun')
            ...
            expect(functionCallbacks.testFunction).toHaveBeenCalledWith('{"param": "value"}', expect.objectContaining({
                threadId: 'thread-123',
                runId: 'run-123',
            ...
            expect(provider.makeRequest).toHaveBeenCalledTimes(6);
            // Verify the tool outputs were submitted correctly
            const submitToolOutputsRequest = provider.makeRequest.mock.calls[4];
            ...
            expect(provider.processCompletedRun).toHaveBeenCalledWith('https://test.azure.com', '2024-04-01-preview', 'thread-123', 'run-123');
        });
        it('should handle missing callbacks gracefully', async () => {
            ...
            expect(provider.makeRequest).toHaveBeenCalledTimes(4); // Only 4 calls because it returns error before completion
        });
        it('should handle string-based function callbacks', async () => {
            ...
            expect(provider.makeRequest).toHaveBeenCalledTimes(6);
            expect(provider.makeRequest.mock.calls[4][0]).toContain('submit_tool_outputs');
            expect(JSON.parse(provider.makeRequest.mock.calls[4][1].body)).toEqual({
                tool_outputs: [
                    {
                        tool_call_id: 'call-123',
    ...
    describe('processCompletedRun', () => {
        it('should process text messages from the assistant', async () => {
            ...
            const result = await provider.processCompletedRun('https://test.azure.com', '2024-04-01-preview', 'thread-123', 'run-123');
            expect(result).toEqual({
        ...
        it('should handle errors during run processing', async () => {
            provider.makeRequest.mockRejectedValueOnce(new Error('Processing error'));
            const result = await provider.processCompletedRun('https://test.azure.com', '2024-04-01-preview', 'thread-123', 'run-123');
    ...
    describe('makeRequest', () => {
        beforeEach(() => {
            provider.makeRequest = assistant_1.AzureAssistantProvider.prototype['makeRequest'];
        ...
        it('should make a successful request', async () => {
            ...
            const result = await provider.makeRequest('https://test.url', {
                method: 'POST',
                body: JSON.stringify({ test: true }),
            });
            expect(result).toEqual(mockResponseData);
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('https://test.url', {
                method: 'POST',
                body: JSON.stringify({ test: true }),
            }, expect.any(Number), 'json', expect.any(Boolean), expect.any(Number));
        });
        it('should throw an error for non-200 responses', async () => {
            ...
            await expect(provider.makeRequest('https://test.url', {})).rejects.toThrow(/API error: 400 Bad Request/);
        });
        it('should handle JSON parsing errors', async () => {
            jest
                .mocked(cache_1.fetchWithCache)
                .mockRejectedValueOnce(new Error('Failed to parse response as JSON: Invalid JSON'));
            await expect(provider.makeRequest('https://test.url', {})).rejects.toThrow('Failed to parse response as JSON');
        });
    });
    describe('error detection methods', () => {
        it('should identify content filter errors', () => {
            expect(provider.isContentFilterError('content_filter triggered')).toBe(true);
            expect(provider.isContentFilterError('content filter violation')).toBe(true);
            expect(provider.isContentFilterError('Content filter blocked this')).toBe(true);
            expect(provider.isContentFilterError('filtered due to policy')).toBe(true);
            expect(provider.isContentFilterError('content filtering system')).toBe(true);
            expect(provider.isContentFilterError('inappropriate content detected')).toBe(true);
            expect(provider.isContentFilterError('safety guidelines violation')).toBe(true);
            expect(provider.isContentFilterError('guardrail triggered')).toBe(true);
            expect(provider.isContentFilterError('some other error')).toBe(false);
        });
        it('should identify rate limit errors', () => {
            expect(provider.isRateLimitError('rate limit exceeded')).toBe(true);
            expect(provider.isRateLimitError('Rate limit reached')).toBe(true);
            expect(provider.isRateLimitError('HTTP 429 Too Many Requests')).toBe(true);
            expect(provider.isRateLimitError('some other error')).toBe(false);
        });
        it('should identify service errors', () => {
            expect(provider.isServiceError('Service unavailable')).toBe(true);
            expect(provider.isServiceError('Bad gateway')).toBe(true);
            expect(provider.isServiceError('Gateway timeout')).toBe(true);
            expect(provider.isServiceError('Server is busy')).toBe(true);
            expect(provider.isServiceError('Sorry, something went wrong')).toBe(true);
            expect(provider.isServiceError('some other error')).toBe(false);
        });
        it('should identify server errors', () => {
            expect(provider.isServerError('500 Internal Server Error')).toBe(true);
            expect(provider.isServerError('502 Bad Gateway')).toBe(true);
            expect(provider.isServerError('503 Service Unavailable')).toBe(true);
            expect(provider.isServerError('504 Gateway Timeout')).toBe(true);
            expect(provider.isServerError('some other error')).toBe(false);
        });
        it('should determine if an error is retryable', () => {
            // Direct code check
            expect(provider.isRetryableError('rate_limit_exceeded')).toBe(true);
            // Message checks
            expect(provider.isRetryableError(undefined, 'rate limit exceeded')).toBe(true);
            expect(provider.isRetryableError(undefined, 'Service unavailable')).toBe(true);
            expect(provider.isRetryableError(undefined, '500 Internal Server Error')).toBe(true);
            // Not retryable
            expect(provider.isRetryableError(undefined, 'Invalid request')).toBe(false);
            expect(provider.isRetryableError('invalid_request')).toBe(false);
            expect(provider.isRetryableError()).toBe(false);
        });
    });
    describe('content filter error handling', () => {
        it('should format content filter errors with proper guardrail response', () => {
            ...
            const result = provider.formatError(contentFilterError);
            expect(result).toEqual({
                output: "The generated content was filtered due to triggering Azure OpenAI Service's content filtering system.",
                guardrails: {
                    flagged: true,
        ...
        it('should detect input filtering in content filter errors', () => {
            ...
            const result = provider.formatError(inputFilterError);
            expect(result).toEqual({
                output: "The generated content was filtered due to triggering Azure OpenAI Service's content filtering system.",
                guardrails: {
                    flagged: true,
        ...
        it('should detect output filtering in content filter errors', () => {
            ...
            const result = provider.formatError(outputFilterError);
            expect(result).toEqual({
                output: "The generated content was filtered due to triggering Azure OpenAI Service's content filtering system.",
                guardrails: {
                    flagged: true,
        ...
        it('should handle content filter errors in makeRequest', async () => {
            ...
            await expect(provider.makeRequest('https://test.url', {})).rejects.toThrow('Content filter triggered: Content was filtered due to policy violation');
        });
    ...
    describe('guardrail response consistency', () => {
        it('should return consistent guardrail responses from different error paths', async () => {
            ...
            const formatErrorResult = provider.formatError(new Error('content_filter detected'));
            expect(formatErrorResult).toEqual(expectedGuardrailResponse);
            // Test 2: Content filter error during API call
            provider.makeRequest.mockRejectedValueOnce(new Error('content filter triggered'));
        ...
        it('should distinguish between input and output filtering correctly', () => {
            ...
            const inputResult = provider.formatError(inputError);
            ...
            const outputResult = provider.formatError(outputError);
            ...
            const generalResult = provider.formatError(generalError);
        ...
        it('should never flag both input and output simultaneously', () => {
            ...
            const promptResult = provider.formatError(promptError);
            ...
            testCases.forEach(({ message, expectedInput, expectedOutput }) => {
                ...
                const result = provider.formatError(error);
