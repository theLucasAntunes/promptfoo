...
describe('AzureChatCompletionProvider', () => {
    describe('config merging', () => {
        ...
        it('should use provider config when no prompt config exists', () => {
            ...
            expect(provider.getOpenAiBody('test prompt', context).body).toMatchObject({
                functions: [{ name: 'provider_func', parameters: {} }],
                max_tokens: 100,
                temperature: 0.5,
            });
        ...
        it('should handle json_schema response format', () => {
            ...
            const { body } = provider.getOpenAiBody('test prompt', context);
    ...
    describe('response handling', () => {
        ...
        it('should pass custom headers from config to fetchWithCache', async () => {
            ...
            const provider = new chat_1.AzureChatCompletionProvider('test-deployment', {
                config: {
                    apiHost: 'test.azure.com',
                    apiKey: 'test-key',
    ...
    describe('structured outputs', () => {
        ...
        it('should use correct API URL based on legacy dataSources config from prompt', async () => {
            ...
            expect(jest.mocked(cache_1.fetchWithCache).mock.calls[0][0]).toContain('/extensions/chat/completions?api-version=2024-custom');
        });
        it('should use correct API URL based on data_sources config from prompt', async () => {
            ...
            expect(jest.mocked(cache_1.fetchWithCache).mock.calls[0][0]).not.toContain('extensions');
            expect(jest.mocked(cache_1.fetchWithCache).mock.calls[0][0]).toContain('/chat/completions?api-version=2024-custom');
        });
    });
    describe('reasoning models', () => {
        ...
        it('should use max_completion_tokens for reasoning models', () => {
            ...
            const body = provider.getOpenAiBody('test prompt').body;
        ...
        it('should support variable rendering in reasoning_effort', () => {
            ...
            const body = provider.getOpenAiBody('test prompt', context).body;
            expect(body).toHaveProperty('reasoning_effort', 'high');
        });
    });
    describe('guardrails and content filtering', () => {
        ...
        it('should detect input content filtering', async () => {
            ...
            expect(result.output).toBe("The response was filtered due to the prompt triggering Azure OpenAI's content management policy. Please modify your prompt and retry. To learn more about our content filtering policies please read our documentation: https://go.microsoft.com/fwlink/?linkid=2198766");
            expect(result.finishReason).toBe('content_filter');
        });
        it('should handle content filtering system errors', async () => {
            ...
            expect(logger_1.default.warn).toHaveBeenCalledWith('Content filtering system is down or otherwise unable to complete the request in time: content_filter_error The contents are not filtered');
        });
    ...
    describe('Function Tool Callbacks', () => {
        it('should execute function callbacks and return the result', async () => {
            ...
            const providerWithCallbacks = new chat_1.AzureChatCompletionProvider('test-deployment', {
                config: {
                    apiKey: 'test-key',
                    apiHost: 'test.azure.com',
