...
const foundry_agent_1 = require("../../../src/providers/azure/foundry-agent");
jest.mock('../../../src/cache', () => ({
    getCache: jest.fn(),
...
describe('Azure Foundry Agent Provider', () => {
    beforeEach(() => {
        ...
        jest
            .spyOn(foundry_agent_1.AzureFoundryAgentProvider.prototype, 'initializeClient')
            .mockResolvedValue(mockClient);
    });
    describe('instantiation', () => {
        it('should create provider with minimal config', () => {
            const provider = new foundry_agent_1.AzureFoundryAgentProvider('test-agent', {
                config: {
        ...
        it('should throw error if projectUrl is not provided', () => {
            expect(() => {
                new foundry_agent_1.AzureFoundryAgentProvider('test-agent', {
                    config: {},
                });
    ...
    describe('run options parameters', () => {
        it('should pass max_tokens to run options', async () => {
            ...
            expect(mockClient.agents.runs.create).toHaveBeenCalledWith(mockThread.id, mockAgent.id, expect.objectContaining({
                max_tokens: 150,
            }));
        });
    ...
    describe('basic functionality', () => {
        it('should create thread, message, and run', async () => {
            ...
            expect(mockClient.agents.messages.create).toHaveBeenCalledWith(mockThread.id, 'user', 'test prompt');
            expect(mockClient.agents.runs.create).toHaveBeenCalledWith(mockThread.id, mockAgent.id, expect.any(Object));
        });
...
//# sourceMappingURL=foundry-agent.test.js.map