...
describe('AzureCompletionProvider', () => {
    ...
    it('should handle basic completion with caching', async () => {
        ...
        provider.authHeaders = { 'api-key': 'test-key' };
    ...
    it('should pass custom headers from config to fetchWithCache', async () => {
        ...
        const provider = new completion_1.AzureCompletionProvider('test-deployment', {
            config: {
    ...
    it('should handle content filter response', async () => {
        ...
        expect(result.output).toBe("The generated content was filtered due to triggering Azure OpenAI Service's content filtering system.");
    });
    ...
    it('should handle missing API host', async () => {
        jest.mocked(cache_1.fetchWithCache).mockImplementationOnce(() => {
            throw new Error('Azure API host must be set.');
        });
        const provider = new completion_1.AzureCompletionProvider('test', { config: {} });
    ...
    it('should handle invalid OPENAI_STOP env var', async () => {
        ...
        provider.authHeaders = { 'api-key': 'test-key' };
        await expect(provider.callApi('test')).rejects.toThrow(/OPENAI_STOP is not a valid JSON string/);
    ...
    it('should pass passthrough config fields in body', async () => {
        ...
        const body = JSON.parse(actualCall[1]?.body);
