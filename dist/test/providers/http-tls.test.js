...
describe('HttpProvider with TLS Configuration', () => {
    ...
    describe('TLS certificate configuration', () => {
        it('should create HTTPS agent when TLS config is provided', async () => {
            ...
            const provider = new http_1.HttpProvider('https://api.example.com', {
                config: {
                    method: 'POST',
            ...
            expect(mockFetchWithCache).toHaveBeenCalledWith('https://api.example.com', expect.objectContaining({
                method: 'POST',
                headers: { 'content-type': 'application/json' },
            ...
            expect(fs_1.default.readFileSync).toHaveBeenCalledTimes(3);
        });
        it('should support inline certificates', async () => {
            ...
            }), expect.any(Number), expect.any(String), undefined, undefined);
        });
        it('should support PFX certificates', async () => {
            // Mock PFX file read (returns Buffer)
            fs_1.default.readFileSync.mockReturnValue(Buffer.from('PFX_CONTENT'));
            const provider = new http_1.HttpProvider('https://api.example.com', {
                config: {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
            ...
            expect(fs_1.default.readFileSync).toHaveBeenCalledWith(expect.stringContaining('cert.pfx'));
            // Verify dispatcher was included
            expect(mockFetchWithCache).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                dispatcher: expect.objectContaining({
                    dispatcher: 'mock-agent',
        ...
        it('should validate cert/key pairs', () => {
            ...
            expect(() => {
                new http_1.HttpProvider('https://api.example.com', {
                    config: {
                        method: 'POST',
                        tls: {
                            certPath: '/path/to/cert.pem',
        ...
        it('should work with raw request and TLS', async () => {
            fs_1.default.readFileSync.mockReturnValue('CA_CERT_CONTENT');
            const provider = new http_1.HttpProvider('https://api.example.com', {
                config: {
            ...
            expect(mockFetchWithCache).toHaveBeenCalledWith(expect.stringContaining('api.example.com'), expect.objectContaining({
                dispatcher: expect.objectContaining({
                    dispatcher: 'mock-agent',
                }),
            }), expect.any(Number), 'text', undefined, undefined);
    ...
    describe('TLS with signature auth', () => {
        it('should support both TLS and signature authentication', async () => {
            ...
            jest.spyOn(require('crypto'), 'createSign').mockReturnValue(mockSign);
            fs_1.default.readFileSync.mockReturnValue('CA_CERT_CONTENT');
            const provider = new http_1.HttpProvider('https://api.example.com', {
                config: {
                    method: 'POST',
...
//# sourceMappingURL=http-tls.test.js.map