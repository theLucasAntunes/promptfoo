...
describe('OpenRouter', () => {
    ...
    describe('OpenRouterProvider', () => {
        const provider = new openrouter_1.OpenRouterProvider('google/gemini-2.5-pro', {});
        ...
        describe('Thinking tokens handling', () => {
            ...
            it('should hide reasoning when showThinking is false', async () => {
                const providerWithoutThinking = new openrouter_1.OpenRouterProvider('google/gemini-2.5-pro', {
            ...
            it('should handle models with reasoning field', async () => {
                const nonGeminiProvider = new openrouter_1.OpenRouterProvider('anthropic/claude-3.5-sonnet', {});
                const mockResponse = {
                    choices: [
                        {
                            message: {
                ...
                const expectedOutput = 'Thinking: Thinking about the best way to respond to this query\n\nRegular response with reasoning';
                expect(result.output).toBe(expectedOutput);
                expect(result.tokenUsage).toEqual({ total: 30, prompt: 10, completion: 20 });
            });
            it('should handle models without reasoning field', async () => {
                const provider = new openrouter_1.OpenRouterProvider('anthropic/claude-3.5-sonnet', {});
                const mockResponse = {
                    choices: [
                        {
                            message: {
            ...
            it('should handle tool calls without including reasoning when showThinking is false', async () => {
                    ...
                    choices: [
                        {
                            message: {
                                ...
                                reasoning: 'I need to check the weather for San Francisco to answer the user query.',
                            },
                        },
                    ],
                ...
                mockedFetchWithRetries.mockResolvedValueOnce(response);
                const result = await providerWithoutThinking.callApi('What is the weather in San Francisco?');
                    ...
                    choices: [
                        {
                            message: {
                                ...
                                reasoning: 'The user wants to know the current time, I should call the time function.',
                            },
                        },
                    ],
                    ...
                    choices: [
                        {
                            message: {
                                content: null,
                                tool_calls: [mockToolCall],
                                reasoning: 'I need to search the database for the latest sales data to provide accurate information.',
                            },
                        },
                    ],
            ...
            it('should prioritize tool calls over content+reasoning when all three are present (fixes Qwen thinking models)', async () => {
                const providerWithoutThinking = new openrouter_1.OpenRouterProvider('qwen/qwen3-235b-a22b-thinking-2507', {
                    config: { showThinking: false },
                });
                const mockToolCall = {
                    ...
                    choices: [
                        {
                            message: {
                                ...
                                reasoning: 'The user is asking for weather information. I should use the get_weather function with San Francisco as the location parameter.',
                                tool_calls: [mockToolCall],
                            },
                        },
                    ],
                    ...
                    choices: [
                        {
                            message: {
                                ...
                                reasoning: 'The user wants to find information in the database. I should call the search function.',
                                tool_calls: [mockToolCall],
                            },
                        },
                    ],
            ...
            it('should handle responses with empty content and reasoning when showThinking is false', async () => {
                const providerWithoutThinking = new openrouter_1.OpenRouterProvider('some/thinking-model', {
                    config: { showThinking: false },
                });
                const mockResponse = {
                    choices: [
            ...
            it('should handle responses with only reasoning and no content/tools when showThinking is false', async () => {
                const providerWithoutThinking = new openrouter_1.OpenRouterProvider('some/reasoning-model', {
                    config: { showThinking: false },
                });
                const mockResponse = {
                    choices: [
            ...
            it('should pass through OpenRouter-specific options', async () => {
                const providerWithOptions = new openrouter_1.OpenRouterProvider('google/gemini-2.5-pro', {
                    config: {
        ...
        describe('JSON schema response format handling', () => {
            ...
            it('should parse JSON output when response_format.type is json_schema', async () => {
                const providerWithJsonSchema = new openrouter_1.OpenRouterProvider('google/gemini-2.5-pro', {
                    config: {
                        response_format: {
                            type: 'json_schema',
                            json_schema: {
            ...
            it('should not parse JSON when response_format.type is not json_schema', async () => {
                const regularProvider = new openrouter_1.OpenRouterProvider('google/gemini-2.5-pro', {});
                const mockResponse = {
                    choices: [
                        {
                            message: {
