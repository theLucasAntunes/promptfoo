...
jest.mock('../../src/cache');
const mockedFetchWithCache = jest.mocked(cache_1.fetchWithCache);
describe('ReplicateImageProvider Demonstration', () => {
    ...
    it('demonstrates FLUX 1.1 Pro Ultra image generation', async () => {
        ...
        const provider = new replicate_1.ReplicateImageProvider('black-forest-labs/flux-1.1-pro-ultra', {
            config: {
                apiKey: mockApiKey,
                width: 1024,
                height: 1024,
        ...
        expect(result.output).toBe('![A majestic mountain landscape at golden hour](https://replicate.delivery/pbxt/flux-ultra-example/beautiful-landscape.webp)');
        expect(result.error).toBeUndefined();
        // Verify the API was called correctly
        expect(mockedFetchWithCache).toHaveBeenCalledWith('https://api.replicate.com/v1/models/black-forest-labs/flux-1.1-pro-ultra/predictions', expect.objectContaining({
            method: 'POST',
    ...
    it('demonstrates multiple image outputs handling', async () => {
        ...
        const provider = new replicate_1.ReplicateImageProvider('test-model', {
            config: { apiKey: mockApiKey },
        });
        const result = await provider.callApi('Generate variations');
        // Only the first image is returned for simplicity
        expect(result.output).toBe('![Generate variations](https://replicate.delivery/pbxt/example/image1.png)');
    });
    it('demonstrates raw mode for FLUX 1.1 Pro Ultra', async () => {
        ...
        expect(result.output).toBe('![Professional headshot, natural lighting](https://replicate.delivery/pbxt/flux-raw/photorealistic.png)');
...
//# sourceMappingURL=replicate-image.test.js.map