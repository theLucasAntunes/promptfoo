...
const sanitizer_1 = require("../../src/util/sanitizer");
...
jest.mock('../../src/esm', () => ({
    importModule: jest.fn(async (_modulePath, functionName) => {
        const mockModule = {
...
describe('HttpProvider', () => {
    ...
    it('should call the API and return the response', async () => {
        provider = new http_1.HttpProvider(mockUrl, {
            config: {
                method: 'POST',
        ...
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith(mockUrl, expect.objectContaining({
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ key: 'test prompt' }),
        }), expect.any(Number), 'text', undefined, undefined);
    });
    it('should handle API call errors', async () => {
        ...
        jest.mocked(cache_1.fetchWithCache).mockRejectedValueOnce(mockError);
    ...
    it('should use custom method/headers/queryParams', async () => {
        ...
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith(`${mockUrl}?foo=bar`, expect.objectContaining({
            method: 'PATCH',
    ...
    it('should substitute variables in URL path parameters', async () => {
        ...
        provider = new http_1.HttpProvider(urlWithPathParam, {
            config: {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' },
            },
        ...
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith('http://example.com/users/12345/profile', expect.objectContaining({
            method: 'GET',
            headers: { 'content-type': 'application/json' },
        }), expect.any(Number), 'text', undefined, undefined);
    });
    const testCases = [
        { parser: (data) => data.custom, expected: 'parsed' },
    ...
    it('should correctly render Nunjucks templates in config', async () => {
        ...
        jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
        await provider.callApi('test prompt');
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith(mockUrl, {
            method: 'POST',
    ...
    it('should throw an error when creating HttpProvider with invalid config', () => {
        ...
        expect(() => {
            new http_1.HttpProvider(mockUrl, {
    ...
    it('should handle GET requests with query parameters', async () => {
        ...
        jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
        await provider.callApi('test prompt');
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith(`${mockUrl}?q=test+prompt&foo=bar`, expect.objectContaining({
            method: 'GET',
        }), expect.any(Number), 'text', undefined, undefined);
    });
    describe('raw request', () => {
        it('should handle a basic GET raw request', async () => {
            const rawRequest = (0, dedent_1.default) `
        GET /api/data HTTP/1.1
        Host: example.com
        User-Agent: TestAgent/1.0
      `;
            const provider = new http_1.HttpProvider('http', {
                config: {
                    request: rawRequest,
                    transformResponse: (data) => data,
                },
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
            const result = await provider.callApi('test prompt');
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('http://example.com/api/data', expect.objectContaining({
                method: 'GET',
        ...
        {"data": "{{prompt}}"}
      `;
            const provider = new http_1.HttpProvider('https', {
                config: {
                    request: rawRequest,
                    transformResponse: (data) => data,
                },
            ...
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('https://example.com/api/submit', {
                method: 'POST',
                headers: {
                    host: 'example.com',
                    'content-type': 'application/json',
      ...
      `;
            ...
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('https://example.com/api/users/12345/profile', expect.objectContaining({
                method: 'GET',
                headers: expect.objectContaining({
                    host: 'example.com',
                    accept: 'application/json',
        ...
        Host: example.com
      `;
            jest.mocked(file_1.maybeLoadFromExternalFile).mockReturnValueOnce(fileContent);
            const provider = new http_1.HttpProvider('https', {
                config: {
                    request: filePath,
                    transformResponse: (data) => data,
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
            const result = await provider.callApi('test prompt');
            expect(file_1.maybeLoadFromExternalFile).toHaveBeenCalledWith(filePath);
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('https://example.com/api/data', expect.objectContaining({
                method: 'GET',
                headers: expect.objectContaining({
                    host: 'example.com',
                }),
      ...
      `;
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
            await provider.callApi('test prompt');
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('https://example.com/api/submit', expect.objectContaining({
                method: 'POST',
                headers: {
                    host: 'example.com',
                    'content-type': 'application/json',
      ...
      `;
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
            const result = await provider.callApi('test prompt');
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('http://example.com/api/data?test%20prompt', expect.objectContaining({
                method: 'GET',
                headers: expect.objectContaining({
                    host: 'example.com',
                    'user-agent': 'TestAgent/1.0',
    ...
    describe('raw request - templating safety', () => {
      ...
      `;
            ...
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('https://example.com/api/faq', expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
                    host: 'example.com',
                    'content-type': 'application/json',
      ...
      `;
            ...
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('https://example.com/api/users/abc123/notes', expect.objectContaining({
                method: 'GET',
                headers: expect.objectContaining({
                    host: 'example.com',
                    accept: 'application/json',
    ...
    describe('processJsonBody', () => {
        it('should process simple key-value pairs', () => {
            ...
            const result = (0, http_1.processJsonBody)(body, vars);
...
"trailing_comma": "problem",
    ...
    describe('escapeJsonVariables', () => {
        it('should escape newlines in string values', () => {
            ...
            const result = (0, http_1.escapeJsonVariables)(vars);
        ...
        it('should produce valid JSON when used in raw request templates', () => {
            ...
            const escaped = (0, http_1.escapeJsonVariables)(vars);
    ...
    describe('processTextBody', () => {
        it('should render templates in text bodies', () => {
            ...
            expect((0, http_1.processTextBody)(body, vars)).toBe('Hello World!');
        });
        it('should handle rendering errors gracefully', () => {
            const body = 'Hello {{ unclosed tag';
            const vars = { name: 'World' };
            expect((0, http_1.processTextBody)(body, vars)).toBe(body); // Should return original
        });
        it('should handle null body gracefully', () => {
            // @ts-ignore - Testing null input
            expect((0, http_1.processTextBody)(null, {})).toBeNull();
        });
    });
    it('should use default parser when no parser is provided', async () => {
        const provider = new http_1.HttpProvider(mockUrl, {
            config: {
                method: 'GET',
            },
        });
    ...
    it('should handle response transform returning an object', async () => {
        const provider = new http_1.HttpProvider(mockUrl, {
            config: {
                body: { key: 'value' },
                transformResponse: (json, _text) => ({ custom: json.result }),
            },
        });
        const mockResponse = {
            data: JSON.stringify({ result: 'success' }),
    ...
    describe('file:// transform integration tests', () => {
        it('should handle file:// response transform', async () => {
            const mockParser = jest.fn((data) => ({ transformed: data.result }));
            jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockParser);
            const provider = new http_1.HttpProvider(mockUrl, {
                config: {
                    method: 'POST',
                    body: { key: 'value' },
            ...
            expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'custom-parser.js'), undefined);
        });
        it('should handle file:// response transform with specific function name', async () => {
            ...
            expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'custom-parser.js'), 'parseResponse');
        });
        it('should handle file:// request transform', async () => {
            const mockTransform = jest.fn((prompt) => ({ transformed: prompt.toUpperCase() }));
            jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockTransform);
            const provider = new http_1.HttpProvider(mockUrl, {
                config: {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
            ...
            expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'transform.js'), undefined);
        ...
        it('should handle file:// request transform with specific function name', async () => {
            const mockTransform = jest.fn((prompt) => ({ custom: prompt }));
            jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockTransform);
            const provider = new http_1.HttpProvider(mockUrl, {
                config: {
                    method: 'POST',
            ...
            expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'transform.js'), 'myTransform');
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith(mockUrl, expect.objectContaining({
                body: JSON.stringify({ key: 'value', custom: 'hello' }),
            }), expect.any(Number), 'text', undefined, undefined);
        });
        it('should throw error for malformed file:// response transform', async () => {
            jest.mocked(esm_1.importModule).mockResolvedValueOnce({});
            const provider = new http_1.HttpProvider(mockUrl, {
                config: {
                    method: 'POST',
                    body: { key: 'value' },
    ...
    describe('getDefaultHeaders', () => {
        it('should return empty object for GET requests', () => {
            const provider = new http_1.HttpProvider(mockUrl, { config: { method: 'GET' } });
        ...
        it('should return application/x-www-form-urlencoded for string body', () => {
            const provider = new http_1.HttpProvider(mockUrl, { config: { method: 'POST', body: 'test' } });
    ...
    describe('validateContentTypeAndBody', () => {
        it('should not throw for valid content-type and body', () => {
            const provider = new http_1.HttpProvider(mockUrl, { config: { body: 'test' } });
            expect(() => {
                provider['validateContentTypeAndBody']({ 'content-type': 'application/json' }, { key: 'value' });
            }).not.toThrow();
        });
        it('should throw for non-json content-type with object body', () => {
            const provider = new http_1.HttpProvider(mockUrl, { config: { body: 'test' } });
            expect(() => {
                provider['validateContentTypeAndBody']({ 'content-type': 'application/x-www-form-urlencoded' }, { key: 'value' });
    ...
    describe('getHeaders', () => {
        it('should combine default headers with config headers', async () => {
            ...
            const result = await provider.getHeaders({ 'content-type': 'application/json' }, { prompt: 'test' });
            expect(result).toEqual({
                'content-type': 'application/json',
                'x-custom': 'test',
            });
        ...
        it('should render environment variables in headers', async () => {
            ...
            const provider = new http_1.HttpProvider('http://example.com', {
                config: {
    ...
    it('should throw an error if the body is an object and the content-type is not application/json', async () => {
        ...
        await expect(provider.callApi('test prompt')).rejects.toThrow('Content-Type is not application/json, but body is an object or array');
    });
        ...
        it('should use responseParser when transformResponse is not set', async () => {
            const provider = new http_1.HttpProvider(mockUrl, {
                config: {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: { key: '{{ prompt }}' },
                    responseParser: (data) => ({ chat_history: data.result }),
                },
            });
            const mockResponse = {
                data: JSON.stringify({ result: 'success' }),
        ...
        it('should prefer transformResponse over responseParser when both are set', async () => {
            const provider = new http_1.HttpProvider(mockUrl, {
                config: {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: { key: '{{ prompt }}' },
                    responseParser: (_data) => ({ chat_history: 'from responseParser' }),
                    transformResponse: (_data) => ({ chat_history: 'from transformResponse' }),
                },
            });
            const mockResponse = {
                data: JSON.stringify({ result: 'success' }),
    ...
    it('should respect maxRetries configuration', async () => {
        ...
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith(mockUrl, expect.objectContaining({
            ...
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ key: 'test prompt' }),
        }), expect.any(Number), 'text', undefined, 2);
    });
    it('should handle query parameters correctly when the URL already has query parameters', async () => {
        ...
        provider = new http_1.HttpProvider(urlWithQueryParams, {
            config: {
                method: 'GET',
                queryParams: {
        ...
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith(expect.stringMatching(/http:\/\/example\.com\/api\?existing=param&additional=parameter&another=value/), expect.any(Object), expect.any(Number), 'text', undefined, undefined);
    });
    it('should handle URL construction fallback for potentially malformed URLs', async () => {
        ...
        provider = new http_1.HttpProvider(malformedUrl, {
            config: {
                method: 'GET',
                queryParams: {
                    param: 'value',
        ...
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith('relative/path/test?param=value', expect.any(Object), expect.any(Number), 'text', undefined, undefined);
    });
});
describe('determineRequestBody', () => {
    it('should merge parsed prompt object with config body when content type is JSON', () => {
        const result = (0, http_1.determineRequestBody)(true, { promptField: 'test value' }, { configField: 'config value' }, { vars: 'not used in this case' });
        expect(result).toEqual({
    ...
    it('should process JSON body with variables when parsed prompt is not an object', () => {
        const result = (0, http_1.determineRequestBody)(true, 'test prompt', { message: '{{ prompt }}' }, { prompt: 'test prompt' });
        expect(result).toEqual({
            message: 'test prompt',
        });
    });
    it('should process text body when content type is not JSON', () => {
        const result = (0, http_1.determineRequestBody)(false, 'test prompt', 'Message: {{ prompt }}', {
            prompt: 'test prompt',
        });
        expect(result).toBe('Message: test prompt');
    });
    it('should handle nested JSON structures', () => {
        const result = (0, http_1.determineRequestBody)(true, 'test prompt', {
            outer: {
                inner: '{{ prompt }}',
                static: 'value',
            },
    ...
    it('should handle undefined config body with object prompt', () => {
        const result = (0, http_1.determineRequestBody)(true, { message: 'test prompt' }, undefined, {});
        expect(result).toEqual({
            message: 'test prompt',
        });
    });
    it('should handle array config body', () => {
        const result = (0, http_1.determineRequestBody)(true, 'test prompt', ['static', '{{ prompt }}'], {
            prompt: 'test prompt',
        });
    ...
    describe('stringified JSON body handling', () => {
        it('should parse stringified JSON body when content type is JSON', () => {
            ...
            const result = (0, http_1.determineRequestBody)(true, 'test prompt', stringifiedBody, {
                prompt: 'test prompt',
            });
            expect(result).toEqual({
                message: 'test prompt',
        ...
        it('should handle stringified JSON body that fails to parse as a template string', () => {
            ...
            const result = (0, http_1.determineRequestBody)(true, 'test prompt', templateString, {
                prompt: 'test prompt',
            });
            expect(result).toBe('Message: test prompt');
        });
        it('should not parse stringified body when content type is not JSON', () => {
            const stringifiedBody = '{"message":"{{prompt}}"}';
            const result = (0, http_1.determineRequestBody)(false, 'test prompt', stringifiedBody, {
                prompt: 'test prompt',
            });
        ...
        it('should handle object body normally (backward compatibility)', () => {
            ...
            const result = (0, http_1.determineRequestBody)(true, 'test prompt', objectBody, {
                prompt: 'test prompt',
            });
            expect(result).toEqual({
                message: 'test prompt',
        ...
        it('should merge parsed prompt with stringified JSON body', () => {
            ...
            const result = (0, http_1.determineRequestBody)(true, { promptField: 'prompt value' }, stringifiedBody, {});
            expect(result).toEqual({
                configField: 'value',
...
describe('constructor validation', () => {
    it('should validate config using Zod schema', () => {
        expect(() => {
            new http_1.HttpProvider('http://test.com', {
                config: {
...
describe('content type handling', () => {
    it('should handle JSON content type with object body', async () => {
        const provider = new http_1.HttpProvider('http://test.com', {
            config: {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: { key: '{{ prompt }}' },
        ...
        jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
        await provider.callApi('test');
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith('http://test.com', expect.objectContaining({
...
describe('request transformation', () => {
    it('should handle string-based request transform', async () => {
        ...
        expect(cache_1.fetchWithCache).toHaveBeenCalledWith('http://test.com', {
            ...
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ key: 'value', transformed: 'test' }),
        }, shared_1.REQUEST_TIMEOUT_MS, 'text', undefined, undefined);
    });
    it('should handle function-based request transform', async () => {
        ...
        const provider = new http_1.HttpProvider('http://test.com', {
            config: {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: { key: 'value' },
                transformRequest: (prompt) => ({ transformed: prompt.toUpperCase() }),
            },
        });
        const mockResponse = {
            data: JSON.stringify({ result: 'success' }),
...
describe('error handling', () => {
    it('should throw error for responses that fail validateStatus check', async () => {
        ...
        jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
        await expect(provider.callApi('test')).rejects.toThrow('HTTP call failed with status 400 Bad Request: Error message');
    });
    ...
    it('should throw error for raw request with non-200 response', async () => {
        ...
        jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
        await expect(provider.callApi('test')).rejects.toThrow('HTTP call failed with status 500 Internal Server Error: Error occurred');
    });
});
describe('validateStatus', () => {
    ...
    describe('string-based validators', () => {
        it('should handle expression format', async () => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(errorResponse);
            await expect(provider.callApi('test')).rejects.toThrow('HTTP call failed with status 400 Bad Request: Error message');
        });
        it('should handle arrow function format with parameter', async () => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(errorResponse);
            await expect(provider.callApi('test')).rejects.toThrow('HTTP call failed with status 500 Server Error: Error message');
        });
    ...
    describe('error handling', () => {
        ...
        it('should throw error for malformed file-based validator', async () => {
            jest.mocked(esm_1.importModule).mockRejectedValueOnce(new Error('Module not found'));
            await expect((0, http_1.createValidateStatus)('file://invalid-validator.js')).rejects.toThrow(/Status validator malformed/);
        });
        it('should throw error for unsupported validator type', async () => {
            await expect((0, http_1.createValidateStatus)(123)).rejects.toThrow('Unsupported status validator type: number');
        });
    });
    describe('file-based validators', () => {
        it('should handle file-based validateStatus', async () => {
            ...
            jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockValidator);
            const provider = new http_1.HttpProvider('http://test.com', {
                config: {
                    method: 'POST',
                    body: { key: 'value' },
            ...
            const result = await provider.callApi('test');
            expect(result.output).toEqual({ result: 'success' });
            expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'validator.js'), undefined);
        ...
        it('should handle file-based validateStatus with specific function', async () => {
            ...
            const result = await provider.callApi('test');
            expect(result.output).toEqual({ result: 'success' });
            expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'validator.js'), 'validateStatus');
            expect(mockValidator).toHaveBeenCalledWith(404);
        });
    });
});
describe('session parser', () => {
    ...
    it('should throw error for unsupported session parser type', async () => {
        await expect((0, http_1.createSessionParser)(123)).rejects.toThrow('Unsupported response transform type: number');
    });
});
...
describe('HttpProvider with token estimation', () => {
    afterEach(() => {
        delete cliState_1.default.config;
    });
...
describe('RSA signature authentication', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(fs_1.default, 'readFileSync').mockReturnValue(mockPrivateKey);
        mockUpdate = jest.fn();
        mockEnd = jest.fn();
        ...
        jest.spyOn(crypto_1.default, 'createSign').mockReturnValue(mockSignObject);
    ...
    it('should generate and include signature in vars', async () => {
        ...
        expect(fs_1.default.readFileSync).toHaveBeenCalledWith('/path/to/key.pem', 'utf8');
        expect(crypto_1.default.createSign).toHaveBeenCalledWith('SHA256');
    ...
    it('should reuse cached signature when within validity period', async () => {
        ...
        await provider.callApi('test');
        expect(crypto_1.default.createSign).toHaveBeenCalledTimes(1);
        ...
        await provider.callApi('test');
        expect(crypto_1.default.createSign).toHaveBeenCalledTimes(1); // Should not be called again
    });
    it('should regenerate signature when expired', async () => {
        ...
        await provider.callApi('test');
        expect(crypto_1.default.createSign).toHaveBeenCalledTimes(2); // Should be called again
    });
    ...
    it('should support using privateKey directly instead of privateKeyPath', async () => {
        ...
        expect(fs_1.default.readFileSync).not.toHaveBeenCalled(); // Should not read from file
        expect(crypto_1.default.createSign).toHaveBeenCalledWith('SHA256');
        expect(mockSign).toHaveBeenCalledWith(mockPrivateKey);
    });
    it('should warn when vars already contain signatureTimestamp', async () => {
        ...
        const timestampWarning = '[HTTP Provider Auth]: `signatureTimestamp` is already defined in vars and will be overwritten';
        // Call the warning directly
        logger_1.default.warn(timestampWarning);
    ...
    it('should use JKS keystore password from environment variable when config password not provided', async () => {
        ...
        const jksMock = jest.mocked(await Promise.resolve().then(() => __importStar(require('jks-js'))));
    ...
    it('should throw error when neither config password nor environment variable is provided for JKS', async () => {
        ...
        await expect(provider.callApi('test')).rejects.toThrow('JKS keystore password is required. Provide it via config keystorePassword/certificatePassword or PROMPTFOO_JKS_PASSWORD environment variable');
        // Clean up
        readFileSyncSpy.mockRestore();
    });
});
describe('createSessionParser', () => {
    it('should return empty string when no parser is provided', async () => {
        const parser = await (0, http_1.createSessionParser)(undefined);
        const result = parser({ headers: {}, body: {} });
        expect(result).toBe('');
    });
    it('should handle function parser', async () => {
        const functionParser = ({ headers }) => headers['session-id'];
        const parser = await (0, http_1.createSessionParser)(functionParser);
    ...
    it('should handle header path expression', async () => {
        const parser = await (0, http_1.createSessionParser)('data.headers["x-session-id"]');
        const result = parser({
            headers: { 'x-session-id': 'test-session' },
            body: {},
        });
    ...
    it('should handle body path expression', async () => {
        const parser = await (0, http_1.createSessionParser)('data.body.session.id');
        const result = parser({
            headers: {},
    ...
    it('should handle file:// parser', async () => {
        ...
        jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockParser);
        const parser = await (0, http_1.createSessionParser)('file://session-parser.js');
        const result = parser({ headers: { 'session-id': 'test-session' } });
        expect(result).toBe('test-session');
        expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'session-parser.js'), undefined);
    });
    it('should handle file:// parser with specific function', async () => {
        const mockParser = jest.fn(({ body }) => body.sessionId);
        jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockParser);
        const parser = await (0, http_1.createSessionParser)('file://session-parser.js:parseSession');
        const result = parser({ headers: {}, body: { sessionId: 'test-session' } });
        expect(result).toBe('test-session');
        expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'session-parser.js'), 'parseSession');
    });
    it('should throw error for malformed file:// parser', async () => {
        jest.mocked(esm_1.importModule).mockResolvedValueOnce({});
        await expect((0, http_1.createSessionParser)('file://invalid-parser.js')).rejects.toThrow(/Response transform malformed/);
    });
    it('should handle complex body path expression', async () => {
        const parser = await (0, http_1.createSessionParser)('data.body.data.attributes.session.id');
        const result = parser({
            headers: {},
            body: {
                data: {
...
describe('urlEncodeRawRequestPath', () => {
    it('should not modify request with no query parameters', () => {
        ...
        const result = (0, http_1.urlEncodeRawRequestPath)(rawRequest);
    ...
    it('should throw error when modifying malformed request with no URL', () => {
        ...
        expect(() => (0, http_1.urlEncodeRawRequestPath)(rawRequest)).toThrow(/not valid/);
    });
...
describe('Token Estimation', () => {
    describe('estimateTokenCount', () => {
        it('should count tokens using word-based method', () => {
            ...
            const result = (0, http_1.estimateTokenCount)(text, 1.3);
            expect(result).toBe(Math.ceil(6 * 1.3)); // 6 words * 1.3 = 7.8, ceil = 8
        });
        it('should handle empty text', () => {
            expect((0, http_1.estimateTokenCount)('', 1.3)).toBe(0);
            expect((0, http_1.estimateTokenCount)(null, 1.3)).toBe(0);
            expect((0, http_1.estimateTokenCount)(undefined, 1.3)).toBe(0);
        });
        it('should filter out empty words', () => {
            const text = 'hello   world    test'; // Multiple spaces
            const result = (0, http_1.estimateTokenCount)(text, 1.0);
            expect(result).toBe(3); // Should count 3 words, not split on every space
        });
        it('should use default multiplier when not provided', () => {
            const text = 'hello world';
            const result = (0, http_1.estimateTokenCount)(text);
...
describe('Body file resolution', () => {
    ...
    it('should resolve file:// references in body configuration', () => {
        ...
        jest.mocked(file_1.maybeLoadConfigFromExternalFile).mockReturnValue({
            query: '{{prompt}}',
        ...
        expect(file_1.maybeLoadConfigFromExternalFile).toHaveBeenCalledWith({
            query: '{{prompt}}',
            date: '2025-06-03T22:01:13.797Z',
            transactions: 'file://./test_data/transactions.csv',
        });
    ...
    it('should not affect body when no file:// references are present', () => {
        ...
        jest.mocked(file_1.maybeLoadConfigFromExternalFile).mockReturnValue(originalBody);
        const provider = new http_1.HttpProvider('http://test.com', {
            config: {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    it('should work with string body containing file:// reference', () => {
        ...
        jest.mocked(file_1.maybeLoadConfigFromExternalFile).mockReturnValue(mockContent);
        const provider = new http_1.HttpProvider('http://test.com', {
            config: {
                method: 'POST',
                headers: { 'Content-Type': 'text/plain' },
    ...
    it('should use resolved body in API calls', async () => {
        ...
        expect(cache_1.fetchWithCache).toHaveBeenCalled();
        const actualCall = jest.mocked(cache_1.fetchWithCache).mock.calls[0];
    ...
    it('should handle GET requests without body file resolution', () => {
        ...
        jest.mocked(file_1.maybeLoadConfigFromExternalFile).mockClear();
        new http_1.HttpProvider('http://test.com', {
            config: {
                method: 'GET',
            },
        ...
        expect(file_1.maybeLoadConfigFromExternalFile).not.toHaveBeenCalled();
    });
    it('should handle complex nested file resolutions with mixed content', () => {
        ...
        jest.mocked(file_1.maybeLoadConfigFromExternalFile).mockReturnValue(mockData);
        const provider = new http_1.HttpProvider('http://test.com', {
            config: {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
describe('HttpProvider - Sanitization', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        loggerDebugSpy = jest.spyOn(logger_1.default, 'debug');
    });
    afterEach(() => {
        loggerDebugSpy.mockRestore();
    });
    it('should sanitize pfxPassword in debug logs', async () => {
        const provider = new http_1.HttpProvider(testUrl, {
            config: {
                method: 'POST',
                body: { test: 'value' },
        ...
        expect(loggerDebugSpy).toHaveBeenCalledWith(expect.stringContaining('Calling http://example.com/api with config'), expect.anything());
    });
    it('should sanitize Authorization header in debug logs', async () => {
        ...
        expect(loggerDebugSpy).toHaveBeenCalledWith(expect.stringContaining('Calling'), expect.objectContaining({ config: expect.any(Object) }));
    });
    ...
    it('should preserve non-sensitive fields', async () => {
        ...
        jest.mocked(cache_1.fetchWithCache).mockResolvedValueOnce(mockResponse);
        await provider.callApi('test prompt');
        const debugCall = loggerDebugSpy.mock.calls.find((call) => call[0]?.includes('Calling') && call[0]?.includes('with config'));
    ...
    describe('Header sanitization in logs', () => {
        it('should sanitize sensitive headers while preserving functionality', async () => {
            const provider = new http_1.HttpProvider('https://api.example.com/test', {
                config: {
                    method: 'POST',
                    body: { message: '{{ prompt }}' },
                    headers: {
            ...
            const sanitizedContext = (0, sanitizer_1.sanitizeObject)(loggedContext, { context: 'test' });
            ...
            const authHeader = sanitizedHeaders.Authorization ||
                ...
                Object.entries(sanitizedHeaders).find(([key]) => key.toLowerCase() === 'authorization')?.[1];
            const apiKeyHeader = sanitizedHeaders['X-API-Key'] ||
            ...
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('https://api.example.com/test', expect.objectContaining({
                headers: expect.objectContaining({
    ...
    describe('URL sanitization', () => {
        it('should sanitize URL query parameters', async () => {
            const provider = new http_1.HttpProvider('https://api.example.com/test?api_key=secret123&format=json', {
                config: {
                    method: 'GET',
                },
            });
        ...
        it('should work with standalone sanitizeUrl function', () => {
            ...
            testCases.forEach(({ input, expectContains, expectNotContains }) => {
                const result = (0, sanitizer_1.sanitizeUrl)(input);
    ...
    describe('Combined sanitization scenarios', () => {
        it('should handle both URL and header sanitization together', async () => {
            const provider = new http_1.HttpProvider('https://api.example.com/test?api_key=url_secret123', {
                config: {
                    method: 'POST',
                    body: { data: '{{ prompt }}' },
                    headers: {
            ...
            expect(loggerDebugSpy).toHaveBeenCalledWith(expect.stringContaining('api_key=%5BREDACTED%5D'), expect.anything());
        });
        it('should not impact performance significantly', async () => {
            const provider = new http_1.HttpProvider('https://api.example.com/perf', {
                config: {
                    method: 'POST',
            ...
            const debugCalls = loggerDebugSpy.mock.calls.filter((call) => call[0]?.includes('Calling') && call[0]?.includes('with config'));
    ...
    describe('Edge cases', () => {
        ...
        it('should handle malformed URLs in sanitizeUrl function', () => {
            ...
            malformedInputs.forEach((input) => {
                expect(() => (0, sanitizer_1.sanitizeUrl)(input)).not.toThrow();
                const result = (0, sanitizer_1.sanitizeUrl)(input);
                expect(result).toBeDefined();
            });
            // Test null/undefined separately as they return the input as-is
            expect((0, sanitizer_1.sanitizeUrl)(null)).toBeNull();
            expect((0, sanitizer_1.sanitizeUrl)(undefined)).toBeUndefined();
        });
    });
});
