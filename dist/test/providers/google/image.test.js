...
const image_1 = require("../../../src/providers/google/image");
jest.mock('../../../src/cache', () => ({
...
describe('GoogleImageProvider', () => {
    ...
    const mockGetGoogleClient = utilMocks.getGoogleClient;
    const mockLoadCredentials = utilMocks.loadCredentials;
    const mockResolveProjectId = utilMocks.resolveProjectId;
    beforeEach(() => {
        jest.clearAllMocks();
    ...
    it('should construct with model name', () => {
        const provider = new image_1.GoogleImageProvider('imagen-3.0-generate-001');
    ...
    it('should use Google AI Studio when project ID is missing but API key is available', async () => {
        ...
        expect(mockFetchWithCache).toHaveBeenCalledWith('https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-001:predict', expect.objectContaining({
    ...
    it('should support different model name formats', () => {
        const provider1 = new image_1.GoogleImageProvider('imagen-3.0-generate-001');
        ...
        const provider2 = new image_1.GoogleImageProvider('gemini/imagen-3.0-generate-001');
        ...
        const provider3 = new image_1.GoogleImageProvider('3.0-generate-001');
    ...
    it('should handle model path prefixing correctly', async () => {
        ...
        for (const { input, expected } of testCases) {
            const provider = new image_1.GoogleImageProvider(input, {
                config: {
    ...
    it('should return correct cost for different models', async () => {
        ...
        for (const { model, expectedCost } of testCases) {
            const provider = new image_1.GoogleImageProvider(model);
            mockFetchWithCache.mockResolvedValueOnce({
                status: 200,
                data: {
    ...
    describe('Google AI Studio', () => {
        ...
        it('should make correct API request to Google AI Studio', async () => {
            const provider = new image_1.GoogleImageProvider('imagen-4.0-generate-preview-06-06', {
                config: {
                    n: 2,
                    aspectRatio: '16:9',
            ...
            const result = await provider.callApi('Test prompt');
            expect(mockFetchWithCache).toHaveBeenCalledWith('https://generativelanguage.googleapis.com/v1beta/models/imagen-4.0-generate-preview-06-06:predict', expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
                    'Content-Type': 'application/json',
                    'x-goog-api-key': 'test-api-key',
        ...
        it('should support different API key environment variables', async () => {
            ...
            const result = await provider.callApi('Test prompt');
            expect(mockFetchWithCache).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                headers: expect.objectContaining({
