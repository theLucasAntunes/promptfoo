...
const vertexUtil = __importStar(require("../../../src/providers/google/util"));
...
describe('VertexChatProvider.callGeminiApi', () => {
    let provider;
    beforeEach(() => {
        provider = new vertex_1.VertexChatProvider('gemini-pro', {
            config: {
        ...
        jest.mocked(cache_1.getCache).mockReturnValue({
            get: jest.fn(),
            set: jest.fn(),
            wrap: jest.fn(),
            del: jest.fn(),
    ...
    it('should return cached response if available', async () => {
        ...
        jest.mocked((0, cache_1.getCache)().get).mockResolvedValue(JSON.stringify(mockCachedResponse));
        const response = await provider.callGeminiApi('test prompt');
        expect(response).toEqual({
            ...mockCachedResponse,
            tokenUsage: {
    ...
    it('should use model name in cache key', async () => {
        ...
        jest.spyOn(vertexUtil, 'loadCredentials').mockImplementation((creds) => creds);
        jest.spyOn(vertexUtil, 'resolveProjectId').mockResolvedValue('test-project-id');
        provider = new vertex_1.VertexChatProvider('gemini-2.0-flash-001');
        await provider.callGeminiApi('test prompt');
        expect(mockCacheSet).toHaveBeenCalledWith(expect.stringContaining('vertex:gemini-2.0-flash-001:'), expect.any(String));
    });
    ...
    describe('External Function Callbacks', () => {
        ...
        it('should handle errors in external function loading gracefully', async () => {
            ...
            expect(result.output).toBe('{"functionCall":{"name":"error_function","args":"{\\"test\\":\\"data\\"}"}}');
        });
    ...
    describe('thinking token tracking', () => {
        it('should track thinking tokens when present in response', async () => {
            const provider = new vertex_1.VertexChatProvider('gemini-2.5-flash', {
                config: {
                    generationConfig: {
                        thinkingConfig: {
                            thinkingBudget: 1024,
...
describe('VertexChatProvider.callLlamaApi', () => {
    ...
    it('should enforce us-central1 region for Llama models', async () => {
        ...
        provider = new vertex_1.VertexChatProvider('llama-3.3-70b-instruct-maas', {
            config: { region: 'europe-west1' },
        });
        const response = await provider.callLlamaApi('test prompt');
        // Should return error about region
        expect(response).toEqual({
            error: "Llama models are only available in the us-central1 region. Current region: europe-west1. Please set region: 'us-central1' in your configuration.",
        });
    });
    ...
    it('should successfully call Llama API with valid configuration', async () => {
        ...
        // Verify API request
        expect(mockRequest).toHaveBeenCalledWith(expect.objectContaining({
            url: expect.stringContaining('us-central1-aiplatform.googleapis.com/v1beta1/projects/test-project-id/locations/us-central1/endpoints/openapi/chat/completions'),
            method: 'POST',
            data: expect.objectContaining({
    ...
    it('should load system instructions from file', async () => {
        ...
        provider = new vertex_1.VertexChatProvider('gemini-1.5-flash', {
            config: {
...
describe('VertexChatProvider.callClaudeApi parameter naming', () => {
    ...
    it('should accept anthropicVersion parameter', async () => {
        provider = new vertex_1.VertexChatProvider('claude-3-5-sonnet-v2@20241022', {
            config: {
    ...
    describe('responseSchema handling', () => {
        ...
        beforeEach(() => {
            ...
            jest.mocked(fs.existsSync).mockImplementation((filePath) => {
                ...
                return (pathStr.includes('simple.json') ||
        ...
        it('should handle responseSchema with JSON string', async () => {
            provider = new vertex_1.VertexChatProvider('gemini-2.5-flash', {
                config: {
        ...
        it('should handle responseSchema with variable substitution in schema content', async () => {
            ...
            jest.mocked(fs.existsSync).mockImplementation((filePath) => {
                const pathStr = filePath.toString();
                return (pathStr.includes('variable-content.json') ||
        ...
        it('should throw error when both responseSchema and generationConfig.response_schema are provided', async () => {
            ...
            await expect(provider.callGeminiApi('test')).rejects.toThrow('`responseSchema` provided but `generationConfig.response_schema` already set.');
        });
