...
const image_2 = require("../../../src/providers/xai/image");
...
describe('XAI Image Provider', () => {
    ...
    beforeEach(() => {
        jest.resetAllMocks();
        jest.clearAllMocks();
        jest.mocked(image_1.callOpenAiImageApi).mockResolvedValue(mockSuccessResponse);
    });
    describe('Provider creation and configuration', () => {
        it('throws an error if no model name is provided', () => {
            expect(() => (0, image_2.createXAIImageProvider)('xai:image:')).toThrow('Model name is required');
        });
        it('creates an xAI image provider with specified model', () => {
            const provider = (0, image_2.createXAIImageProvider)('xai:image:grok-2-image');
            expect(provider).toBeInstanceOf(image_2.XAIImageProvider);
            expect(provider.id()).toBe('xai:image:grok-2-image');
        });
        it('should create provider with correct defaults', () => {
            const provider = new image_2.XAIImageProvider('grok-2-image');
        ...
        it('uses correct model mapping', () => {
            const provider = new image_2.XAIImageProvider('grok-image', {
    ...
    describe('Basic functionality', () => {
        it('should generate an image successfully', async () => {
            ...
            expect(image_1.callOpenAiImageApi).toHaveBeenCalledWith('https://api.x.ai/v1/images/generations', {
                model: 'grok-2-image',
        ...
        it('should use cached response', async () => {
            ...
            jest.mocked(image_1.callOpenAiImageApi).mockResolvedValue(mockCachedResponse);
        ...
        it('should use XAI API endpoint', async () => {
            ...
            expect(image_1.callOpenAiImageApi).toHaveBeenCalledWith('https://api.x.ai/v1/images/generations', expect.any(Object), expect.any(Object), expect.any(Number));
        });
        it('should throw an error if API key is not set', async () => {
            ...
            try {
                ...
                await expect(provider.callApi('Generate a cat')).rejects.toThrow('xAI API key is not set. Set the XAI_API_KEY environment variable or add `apiKey` to the provider config.');
            }
            finally {
    ...
    describe('Model name mapping', () => {
        it('should map grok-image to grok-2-image', async () => {
            ...
            await provider.callApi('test prompt');
            expect(image_1.callOpenAiImageApi).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                model: 'grok-2-image',
            }), expect.any(Object), expect.any(Number));
        });
        it('should use grok-2-image as default for unknown models', async () => {
            const provider = new image_2.XAIImageProvider('unknown-model', {
                config: { apiKey: mockApiKey },
            });
            await provider.callApi('test prompt');
            expect(image_1.callOpenAiImageApi).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
    ...
    describe('Error handling', () => {
        it('should handle API errors', async () => {
            ...
            jest.mocked(image_1.callOpenAiImageApi).mockResolvedValue(errorResponse);
            const result = await provider.callApi('test prompt');
        ...
        it('should handle HTTP errors', async () => {
            ...
            jest.mocked(image_1.callOpenAiImageApi).mockResolvedValue({
                data: 'Error message',
                cached: false,
                status: 500,
        ...
        it('should handle fetch errors', async () => {
            ...
            jest.mocked(image_1.callOpenAiImageApi).mockRejectedValue(new Error('Network error'));
            const result = await provider.callApi('test prompt');
            expect(result).toHaveProperty('error');
    ...
    describe('Response format handling', () => {
        it('should handle base64 response format', async () => {
            ...
            jest.mocked(image_1.callOpenAiImageApi).mockResolvedValue(mockBase64Response);
            const result = await provider.callApi('test prompt');
            expect(result).toEqual({
    ...
    describe('Configuration options', () => {
        ...
        it('should include custom headers when provided', async () => {
            ...
            await provider.callApi('test prompt');
            expect(image_1.callOpenAiImageApi).toHaveBeenCalledWith(expect.any(String), expect.any(Object), expect.objectContaining({
                'X-Custom-Header': 'custom-value',
            }), expect.any(Number));
        });
    ...
    describe('createXAIImageProvider factory function', () => {
        ...
        it('should pass through options correctly', () => {
            ...
            const provider = (0, image_2.createXAIImageProvider)('xai:image:grok-2-image', options);
    ...
    describe('Cost calculation for images', () => {
        ...
        it('should calculate cost for multiple images', async () => {
            ...
            jest.mocked(image_1.callOpenAiImageApi).mockResolvedValue(multiImageResponse);
            const result = await provider.callApi('test prompt');
