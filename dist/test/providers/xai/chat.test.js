...
const chat_2 = require("../../../src/providers/xai/chat");
jest.mock('../../../src/providers/openai/chat');
jest.mock('../../../src/logger');
// Mock the parent class methods
const mockGetOpenAiBody = jest.fn();
const mockCallApi = jest.fn();
function createMockToJSON(modelName, config = {}) {
...
describe('xAI Chat Provider', () => {
    beforeEach(() => {
        ...
        mockGetOpenAiBody.mockImplementation(function (prompt, context, callApiOptions) {
            ...
            if (this.modelName && chat_2.GROK_4_MODELS.includes(this.modelName)) {
        ...
        chat_1.OpenAiChatCompletionProvider.mockImplementation((modelName, options) => {
            return {
                id: () => `xai:${modelName}`,
                ...
                isReasoningModel: () => chat_2.GROK_REASONING_MODELS.includes(modelName),
                supportsReasoningEffort: () => chat_2.GROK_REASONING_EFFORT_MODELS.includes(modelName),
    ...
    describe('Provider creation and configuration', () => {
        it('throws an error if no model name is provided', () => {
            expect(() => (0, chat_2.createXAIProvider)('xai:')).toThrow('Model name is required');
        });
        it('creates an xAI provider with specified model', () => {
            const provider = (0, chat_2.createXAIProvider)('xai:grok-2');
            expect(chat_1.OpenAiChatCompletionProvider).toHaveBeenCalledWith('grok-2', expect.any(Object));
        ...
        it('sets the correct API base URL and API key environment variable', () => {
            (0, chat_2.createXAIProvider)('xai:grok-2');
            expect(chat_1.OpenAiChatCompletionProvider).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
        ...
        it('merges provided options with xAI-specific config', () => {
            ...
            (0, chat_2.createXAIProvider)('xai:grok-2', options);
            expect(chat_1.OpenAiChatCompletionProvider).toHaveBeenCalledWith('grok-2', expect.objectContaining({
                config: expect.objectContaining({
                    apiBaseUrl: 'https://api.x.ai/v1',
                    apiKeyEnvar: 'XAI_API_KEY',
                    temperature: 0.7,
    ...
    describe('Search parameters handling', () => {
        it('renders search_parameters with context variables', () => {
            const mockGetOpenAiBody = jest.fn().mockImplementation((prompt, context) => ({
                body: {
                    model: 'grok-3-beta',
                    messages: [{ role: 'user', content: prompt }],
                    search_parameters: {
            ...
            chat_1.OpenAiChatCompletionProvider.mockImplementation(() => ({
                getOpenAiBody: mockGetOpenAiBody,
            }));
            const provider = (0, chat_2.createXAIProvider)('xai:grok-3-beta', {
                config: {
        ...
        it('does not include search_parameters when undefined and preserves original configuration', () => {
            ...
            const providerWithParams = (0, chat_2.createXAIProvider)('xai:grok-3-beta', {
                config: {
                    config: {
    ...
    describe('Model type detection and capabilities', () => {
        it('identifies reasoning models correctly', () => {
            expect(chat_2.GROK_3_MINI_MODELS).toContain('grok-3-mini-beta');
            expect(chat_2.GROK_3_MINI_MODELS).toContain('grok-3-mini-fast-beta');
            expect(chat_2.GROK_3_MINI_MODELS).not.toContain('grok-2-1212');
        });
        it('identifies Grok Code Fast as reasoning models', () => {
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-code-fast-1');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-code-fast');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-code-fast-1-0825');
        });
    });
    describe('Reasoning models configuration', () => {
        it('supports reasoning effort parameters for mini models', () => {
            // Test high reasoning effort
            (0, chat_2.createXAIProvider)('xai:grok-3-mini-beta', {
                config: {
                    config: {
            ...
            expect(chat_1.OpenAiChatCompletionProvider).toHaveBeenCalledWith('grok-3-mini-beta', expect.objectContaining({
                config: expect.objectContaining({
                    apiBaseUrl: 'https://api.x.ai/v1',
                    apiKeyEnvar: 'XAI_API_KEY',
                }),
            ...
            expect(chat_1.OpenAiChatCompletionProvider).toHaveBeenLastCalledWith('grok-3-mini-beta', expect.objectContaining({
                config: expect.objectContaining({
                    apiKeyEnvar: 'XAI_API_KEY',
                }),
            }));
        ...
        it('handles multiple configuration options for mini models', () => {
            ...
            (0, chat_2.createXAIProvider)('xai:grok-3-mini-beta', options);
            expect(chat_1.OpenAiChatCompletionProvider).toHaveBeenCalledWith('grok-3-mini-beta', expect.objectContaining({
                config: expect.objectContaining({
    ...
    describe('Grok-4 specific functionality', () => {
        it('recognizes Grok-4 models as reasoning models', () => {
            const provider = (0, chat_2.createXAIProvider)('xai:grok-4-0709');
            expect(provider.isReasoningModel()).toBe(true);
            const providerAlias = (0, chat_2.createXAIProvider)('xai:grok-4');
        ...
        it('filters unsupported parameters for Grok-4 aliases', () => {
            const provider = (0, chat_2.createXAIProvider)('xai:grok-4');
            const mockContext = {
                prompt: {
                    config: {
    ...
    describe('Model constants', () => {
        it('includes Grok-4 in reasoning models list', () => {
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-4-0709');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-4');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-4-latest');
        });
        it('does not include Grok-4 in reasoning effort models list', () => {
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).not.toContain('grok-4-0709');
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).not.toContain('grok-4');
        });
        it('includes Grok-3 mini models in reasoning effort models list', () => {
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).toContain('grok-3-mini-beta');
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).toContain('grok-3-mini-fast-beta');
        });
        it('includes Grok-4 in XAI_CHAT_MODELS with correct pricing', () => {
            const grok4Model = chat_2.XAI_CHAT_MODELS.find((model) => model.id === 'grok-4-0709');
        ...
        it('includes all Grok-3 mini aliases in reasoning constants', () => {
            ...
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).toContain('grok-3-mini-beta');
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).toContain('grok-3-mini');
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).toContain('grok-3-mini-latest');
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).toContain('grok-3-mini-fast-beta');
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).toContain('grok-3-mini-fast');
            expect(chat_2.GROK_REASONING_EFFORT_MODELS).toContain('grok-3-mini-fast-latest');
            // Test GROK_REASONING_MODELS
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-3-mini-beta');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-3-mini');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-3-mini-latest');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-3-mini-fast-beta');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-3-mini-fast');
            expect(chat_2.GROK_REASONING_MODELS).toContain('grok-3-mini-fast-latest');
        });
        it('recognizes Grok-3 mini aliases as reasoning models', () => {
            ...
            aliases.forEach((alias) => {
                const provider = (0, chat_2.createXAIProvider)(`xai:${alias}`);
                expect(provider.isReasoningModel()).toBe(true);
    ...
    describe('Cost calculation', () => {
        it('calculates costs correctly for all model types', () => {
            // Standard models
            const betaCost = (0, chat_2.calculateXAICost)('grok-3-beta', {}, 1000, 500);
            expect(betaCost).toBeCloseTo(0.0105);
            expect(betaCost).toBe(1000 * (3.0 / 1e6) + 500 * (15.0 / 1e6));
            const fastCost = (0, chat_2.calculateXAICost)('grok-3-fast-beta', {}, 1000, 500);
            expect(fastCost).toBeCloseTo(0.0175);
            // Mini models
            const miniCost = (0, chat_2.calculateXAICost)('grok-3-mini-beta', {}, 1000, 500);
            expect(miniCost).toBeCloseTo(0.00055);
            expect(miniCost).toBe(1000 * (0.3 / 1e6) + 500 * (0.5 / 1e6));
            const miniFastCost = (0, chat_2.calculateXAICost)('grok-3-mini-fast-beta', {}, 1000, 500);
            expect(miniFastCost).toBeCloseTo(0.0026);
            // Legacy models
            const grok2Cost = (0, chat_2.calculateXAICost)('grok-2-latest', {}, 1000, 500);
            expect(grok2Cost).toBeCloseTo(0.007);
            const grok2VisionCost = (0, chat_2.calculateXAICost)('grok-2-vision-latest', {}, 1000, 500);
            expect(grok2VisionCost).toBeCloseTo(0.007);
            // Grok Code Fast models
            const codefast1Cost = (0, chat_2.calculateXAICost)('grok-code-fast-1', {}, 1000, 500);
            expect(codefast1Cost).toBeCloseTo(0.00095);
            expect(codefast1Cost).toBe(1000 * (0.2 / 1e6) + 500 * (1.5 / 1e6));
            const codefastAliasCost = (0, chat_2.calculateXAICost)('grok-code-fast', {}, 1000, 500);
            expect(codefastAliasCost).toBeCloseTo(0.00095);
            const codefast0825Cost = (0, chat_2.calculateXAICost)('grok-code-fast-1-0825', {}, 1000, 500);
        ...
        it('handles model aliases correctly', () => {
            const costWithAlias = (0, chat_2.calculateXAICost)('grok-3-latest', {}, 1000, 500);
            const costWithId = (0, chat_2.calculateXAICost)('grok-3-beta', {}, 1000, 500);
            const aliasTest = (0, chat_2.calculateXAICost)('grok-3', {}, 1000, 500);
            expect(costWithAlias).toBe(costWithId);
            expect(aliasTest).toBe(costWithId);
            expect(costWithAlias).toBeCloseTo(0.0105);
        });
        it('returns undefined for invalid inputs', () => {
            expect((0, chat_2.calculateXAICost)('non-existent-model', {}, 1000, 500)).toBeUndefined();
            expect((0, chat_2.calculateXAICost)('unknown-model', {}, 1000, 500)).toBeUndefined();
            expect((0, chat_2.calculateXAICost)('grok-3-beta', {}, undefined, 500)).toBeUndefined();
            expect((0, chat_2.calculateXAICost)('grok-3-beta', {}, 1000, undefined)).toBeUndefined();
            expect((0, chat_2.calculateXAICost)('grok-3-beta', {}, undefined, undefined)).toBeUndefined();
        });
        it('uses custom cost values when provided', () => {
            const customCost = 10.0 / 1e6;
            const cost = (0, chat_2.calculateXAICost)('grok-3-beta', { cost: customCost }, 1000, 500);
            ...
            const costWithReasoning = (0, chat_2.calculateXAICost)('grok-3-mini-beta', { cost: customCostWithReasoning }, 100, 50, 30);
        ...
        it('handles reasoning tokens correctly', () => {
            ...
            const miniCostWithReasoning = (0, chat_2.calculateXAICost)('grok-3-mini-beta', {}, 100, 50, 30);
            ...
            const standardCost = (0, chat_2.calculateXAICost)('grok-3-beta', {}, 100, 50);
            const standardWithReasoningCost = (0, chat_2.calculateXAICost)('grok-3-beta', {}, 100, 50, 30);
            expect(standardCost).toBe(standardWithReasoningCost);
            // Different reasoning token counts don't change cost for mini models
            const cost1 = (0, chat_2.calculateXAICost)('grok-3-mini-beta', {}, 100, 50, 10);
            const cost2 = (0, chat_2.calculateXAICost)('grok-3-mini-beta', {}, 100, 50, 30);
            expect(cost1).toBe(cost2);
            // Reasoning tokens in cost calculation
            const costWithReasoningTokens = (0, chat_2.calculateXAICost)('grok-3-mini-beta', {}, 1000, 500, 200);
    ...
    describe('Model constants and configuration', () => {
        it('defines correct Grok-3 mini models', () => {
            expect(chat_2.GROK_3_MINI_MODELS).toEqual([
                'grok-3-mini-beta',
                'grok-3-mini',
                'grok-3-mini-latest',
        ...
        it('defines model costs correctly', () => {
            const grok3Beta = chat_2.XAI_CHAT_MODELS.find((m) => m.id === 'grok-3-beta');
        ...
        it('includes all required model properties', () => {
            chat_2.XAI_CHAT_MODELS.forEach((model) => {
        ...
        it('has correct aliases for models', () => {
            const modelWithAliases = chat_2.XAI_CHAT_MODELS.find((m) => m.id === 'grok-3-beta');
    ...
    describe('callApi error handling', () => {
        beforeEach(() => {
            ...
            chat_1.OpenAiChatCompletionProvider.mockImplementation((modelName, options) => {
                const provider = {
                    ...
                    isReasoningModel: () => chat_2.GROK_REASONING_MODELS.includes(modelName),
                    supportsReasoningEffort: () => chat_2.GROK_REASONING_EFFORT_MODELS.includes(modelName),
                    callApi: async (prompt, context, callApiOptions) => {
                        ...
                        try {
                            ...
                            if (errorMessage.includes('Error parsing response') &&
        ...
        it('should handle JSON parsing errors from 502 HTML responses', async () => {
            ...
            mockCallApi.mockRejectedValueOnce(new Error('Error parsing response from https://api.x.ai/v1/chat/completions: Unexpected token \'<\', "<html>\\n<h"... is not valid JSON. Received text: <html>...'));
            const provider = (0, chat_2.createXAIProvider)('xai:grok-4', {
                config: {
                    apiKey: 'test-key',
                    config: {},
            ...
            expect(result.error).toContain('Ensure your XAI_API_KEY environment variable is set correctly');
