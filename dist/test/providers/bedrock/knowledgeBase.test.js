...
const knowledgeBase_1 = require("../../../src/providers/bedrock/knowledgeBase");
...
const { BedrockAgentRuntimeClient, RetrieveAndGenerateCommand } = jest.requireMock('@aws-sdk/client-bedrock-agent-runtime');
...
describe('AwsBedrockKnowledgeBaseProvider', () => {
    ...
    it('should throw an error if knowledgeBaseId is not provided', () => {
        expect(() => {
            new knowledgeBase_1.AwsBedrockKnowledgeBaseProvider('us.anthropic.claude-3-7-sonnet-20241022-v2:0', {
                config: {},
            });
        }).toThrow('Knowledge Base ID is required');
    });
    it('should create provider with required options', () => {
        const provider = new knowledgeBase_1.AwsBedrockKnowledgeBaseProvider('us.anthropic.claude-3-7-sonnet-20241022-v2:0', {
            config: {
    ...
    it('should use custom modelArn if provided', async () => {
        ...
        mockSend.mockResolvedValueOnce(mockResponse);
        const provider = new knowledgeBase_1.AwsBedrockKnowledgeBaseProvider('amazon.nova-lite-v1:0', {
            config: {
                knowledgeBaseId: 'kb-123',
                region: 'us-east-1',
    ...
    it('should retrieve citations from cache when available', async () => {
        ...
        const result = await provider.callApi('What is the capital of France?');
        expect(mockGet).toHaveBeenCalledWith(expect.stringMatching(/^bedrock-kb:.*:What is the capital of France\?$/));
        expect(result).toEqual({
            output: 'Cached response from knowledge base',
            metadata: {
                citations: [
    ...
    it('should use custom modelArn in cache key when provided', async () => {
        ...
        await provider.callApi('What is the capital of France?');
        expect(mockGet).toHaveBeenCalledWith(expect.stringMatching(/^bedrock-kb:.*:What is the capital of France\?$/));
        expect(mockSet).toHaveBeenCalledWith(expect.stringMatching(/^bedrock-kb:.*:What is the capital of France\?$/), expect.any(String));
        mockIsCacheEnabled.mockReturnValue(false);
    });
