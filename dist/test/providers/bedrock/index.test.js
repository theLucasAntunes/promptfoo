...
class TestBedrockProvider extends bedrock_1.AwsBedrockGenericProvider {
    constructor(config = {}) {
...
describe('AwsBedrockGenericProvider', () => {
    ...
    it('should create Bedrock instance without proxy settings', async () => {
        const provider = new (class extends bedrock_1.AwsBedrockGenericProvider {
            constructor() {
    ...
    it('should not include credentials if not provided', async () => {
        ...
        expect(BedrockRuntime).not.toHaveBeenCalledWith(expect.objectContaining({ credentials: expect.anything() }));
    });
    ...
    describe('Custom endpoint support', () => {
        it('should use custom endpoint when endpoint is specified', async () => {
            ...
            await provider.getBedrockInstance();
            expect(BedrockRuntime).toHaveBeenCalledWith(expect.objectContaining({
                region: 'us-east-1',
                retryMode: 'adaptive',
                maxAttempts: 10,
                endpoint: 'https://custom-bedrock-endpoint.example.com',
    ...
    describe('Inference Profile ARN support', () => {
        it('should handle inference profile ARN with claude model type', () => {
            const arnModelName = 'arn:aws:bedrock:us-east-1:123456789012:inference-profile/claude-inference';
            const config = { inferenceModelType: 'claude' };
            // This test checks that getHandlerForModel correctly identifies the handler
            // We need to import and test the actual function
            const provider = new bedrock_1.AwsBedrockCompletionProvider(arnModelName, { config });
            // The handler should be CLAUDE_MESSAGES based on the inferenceModelType
            expect(provider.modelName).toBe(arnModelName);
            expect(provider.config.inferenceModelType).toBe('claude');
        });
        it('should handle inference profile ARN with nova model type', () => {
            const arnModelName = 'arn:aws:bedrock:us-east-1:123456789012:inference-profile/nova-inference';
            const config = { inferenceModelType: 'nova' };
            const provider = new bedrock_1.AwsBedrockCompletionProvider(arnModelName, { config });
            expect(provider.modelName).toBe(arnModelName);
            expect(provider.config.inferenceModelType).toBe('nova');
        });
        it('should handle inference profile ARN with llama model type', () => {
            const arnModelName = 'arn:aws:bedrock:us-east-1:123456789012:inference-profile/llama-inference';
            const config = { inferenceModelType: 'llama' };
            const provider = new bedrock_1.AwsBedrockCompletionProvider(arnModelName, { config });
            expect(provider.modelName).toBe(arnModelName);
            expect(provider.config.inferenceModelType).toBe('llama');
        });
        it('should handle inference profile ARN with deepseek model type', () => {
            const arnModelName = 'arn:aws:bedrock:us-east-1:123456789012:inference-profile/deepseek-inference';
            const config = { inferenceModelType: 'deepseek' };
            const provider = new bedrock_1.AwsBedrockCompletionProvider(arnModelName, { config });
            expect(provider.modelName).toBe(arnModelName);
            expect(provider.config.inferenceModelType).toBe('deepseek');
        });
        it('should handle inference profile ARN with openai model type', () => {
            const arnModelName = 'arn:aws:bedrock:us-east-1:123456789012:inference-profile/openai-inference';
            const config = { inferenceModelType: 'openai' };
            const provider = new bedrock_1.AwsBedrockCompletionProvider(arnModelName, { config });
            expect(provider.modelName).toBe(arnModelName);
            expect(provider.config.inferenceModelType).toBe('openai');
        });
        it('should throw error for inference profile ARN without inferenceModelType', () => {
            const arnModelName = 'arn:aws:bedrock:us-east-1:123456789012:inference-profile/some-inference';
            const provider = new bedrock_1.AwsBedrockCompletionProvider(arnModelName, { config: {} });
    ...
    describe('BEDROCK_MODEL CLAUDE_MESSAGES', () => {
        const modelHandler = bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES;
        ...
        it('should handle JSON message array with image content', () => {
            ...
            const params = bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES.params(config, prompt);
    ...
    describe('BEDROCK_MODEL AI21', () => {
        const modelHandler = bedrock_1.BEDROCK_MODEL.AI21;
...
describe('addConfigParam', () => {
    it('should add config value if provided', () => {
        const params = {};
        (0, bedrock_1.addConfigParam)(params, 'key', 'configValue');
    ...
    it('should add env value if config value is not provided', () => {
        ...
        (0, bedrock_1.addConfigParam)(params, 'key', undefined, process.env.TEST_ENV_KEY);
        expect(params.key).toBe('envValue');
        delete process.env.TEST_ENV_KEY;
    });
    it('should add default value if neither config nor env value is provided', () => {
        const params = {};
        (0, bedrock_1.addConfigParam)(params, 'key', undefined, undefined, 'defaultValue');
        expect(params.key).toBe('defaultValue');
    });
    it('should prioritize config value over env and default values', () => {
        const params = {};
        process.env.TEST_ENV_KEY = 'envValue';
        (0, bedrock_1.addConfigParam)(params, 'key', 'configValue', process.env.TEST_ENV_KEY, 'defaultValue');
        expect(params.key).toBe('configValue');
        delete process.env.TEST_ENV_KEY;
    });
    it('should prioritize env value over default value if config value is not provided', () => {
        const params = {};
        process.env.TEST_ENV_KEY = 'envValue';
        (0, bedrock_1.addConfigParam)(params, 'key', undefined, process.env.TEST_ENV_KEY, 'defaultValue');
        expect(params.key).toBe('envValue');
        delete process.env.TEST_ENV_KEY;
    });
    it('should parse env value if default value is a number', () => {
        ...
        (0, bedrock_1.addConfigParam)(params, 'key', undefined, process.env.TEST_ENV_KEY, 0);
        expect(params.key).toBe(42);
        delete process.env.TEST_ENV_KEY;
    });
    it('should handle undefined config, env, and default values gracefully', () => {
        const params = {};
        (0, bedrock_1.addConfigParam)(params, 'key', undefined, undefined, undefined);
    ...
    it('should handle env value not set', () => {
        const params = {};
        (0, bedrock_1.addConfigParam)(params, 'key', undefined, process.env.UNSET_ENV_KEY, 'defaultValue');
        expect(params.key).toBe('defaultValue');
    });
...
describe('parseValue', () => {
    it('should return the original value if defaultValue is not a number', () => {
        expect((0, bedrock_1.parseValue)('stringValue', 'defaultValue')).toBe('stringValue');
    });
    it('should return parsed float value if defaultValue is a number', () => {
        expect((0, bedrock_1.parseValue)('42.5', 0)).toBe(42.5);
    });
    it('should return NaN for non-numeric strings if defaultValue is a number', () => {
        expect((0, bedrock_1.parseValue)('notANumber', 0)).toBe(0);
    });
    it('should return 0 for an empty string if defaultValue is a number', () => {
        expect((0, bedrock_1.parseValue)('', 0)).toBe(0);
    });
    it('should return null for a null value if defaultValue is not a number', () => {
        expect((0, bedrock_1.parseValue)(null, 'defaultValue')).toBeNull();
    });
    it('should return undefined for an undefined value if defaultValue is not a number', () => {
        expect((0, bedrock_1.parseValue)(undefined, 'defaultValue')).toBeUndefined();
    });
});
describe('llama', () => {
    describe('getLlamaModelHandler', () => {
        describe('LLAMA2', () => {
            const handler = (0, bedrock_1.getLlamaModelHandler)(bedrock_1.LlamaVersion.V2);
            ...
            it('should handle a system message followed by a user message', () => {
                ...
                expect(handler.params(config, prompt)).toEqual({
                    prompt: (0, dedent_1.default) `<s>[INST] <<SYS>>
        ...
        What is the capital of France? [/INST]`,
            ...
            it('should handle multiple turns of conversation', () => {
                ...
                expect(handler.params(config, prompt)).toEqual({
                    prompt: "<s>[INST] Hello [/INST] Hi there! How can I assist you today? </s><s>[INST] What's the weather like? [/INST]",
                    temperature: 0,
                    top_p: 1,
                    max_gen_len: 1024,
                });
        ...
        describe('LLAMA3', () => {
            const handler = (0, bedrock_1.getLlamaModelHandler)(bedrock_1.LlamaVersion.V3);
            it('should generate correct prompt for a single user message', () => {
                const config = { temperature: 0.5, top_p: 0.9, max_gen_len: 512 };
                const prompt = 'Describe the purpose of a "hello world" program in one sentence.';
                expect(handler.params(config, prompt)).toEqual({
                    prompt: (0, dedent_1.default) `<|begin_of_text|><|start_header_id|>user<|end_header_id|>
        ...
        Describe the purpose of a "hello world" program in one sentence.<|eot_id|><|start_header_id|>assistant<|end_header_id|>`,
            ...
            it('should handle a system message followed by a user message', () => {
                ...
                expect(handler.params(config, prompt)).toEqual({
                    prompt: (0, dedent_1.default) `<|begin_of_text|><|start_header_id|>system<|end_header_id|>
        ...
        describe('LLAMA3_1', () => {
            const handler = (0, bedrock_1.getLlamaModelHandler)(bedrock_1.LlamaVersion.V3_1);
            it('should generate correct prompt for a single user message', () => {
                const config = { temperature: 0.5, top_p: 0.9, max_gen_len: 512 };
                const prompt = 'Describe the purpose of a "hello world" program in one sentence.';
                expect(handler.params(config, prompt)).toEqual({
        ...
        describe('LLAMA3_2', () => {
            const handler = (0, bedrock_1.getLlamaModelHandler)(bedrock_1.LlamaVersion.V3_2);
            it('should generate correct prompt for a single user message', () => {
                const config = { temperature: 0.5, top_p: 0.9, max_gen_len: 512 };
                const prompt = 'Describe the purpose of a "hello world" program in one sentence.';
                expect(handler.params(config, prompt)).toEqual({
        ...
        describe('LLAMA3_3', () => {
            const handler = (0, bedrock_1.getLlamaModelHandler)(bedrock_1.LlamaVersion.V3_3);
            it('should generate correct prompt for a single user message', () => {
                const config = { temperature: 0.5, top_p: 0.9, max_gen_len: 512 };
                const prompt = 'Describe the purpose of a "hello world" program in one sentence.';
                expect(handler.params(config, prompt)).toEqual({
        ...
        describe('LLAMA4', () => {
            const handler = (0, bedrock_1.getLlamaModelHandler)(bedrock_1.LlamaVersion.V4);
            it('should generate correct prompt for a single user message', () => {
                const config = { temperature: 0.5, top_p: 0.9, max_gen_len: 512 };
                const prompt = 'Describe the purpose of a "hello world" program in one sentence.';
                expect(handler.params(config, prompt)).toEqual({
                    prompt: (0, dedent_1.default) `<|begin_of_text|><|header_start|>user<|header_end|>
...
Describe the purpose of a "hello world" program in one sentence.<|eot|><|header_start|>assistant<|header_end|>`,
            ...
            it('should handle a system message followed by a user message', () => {
                ...
                expect(handler.params(config, prompt)).toEqual({
                    prompt: (0, dedent_1.default) `<|begin_of_text|><|header_start|>system<|header_end|>
...
What's the weather like?<|eot|><|header_start|>assistant<|header_end|>`,
        ...
        it('should throw an error for unsupported LLAMA version', () => {
            expect(() => (0, bedrock_1.getLlamaModelHandler)(1)).toThrow('Unsupported LLAMA version: 1');
        });
        it('should handle output correctly', () => {
            const handler = (0, bedrock_1.getLlamaModelHandler)(bedrock_1.LlamaVersion.V2);
    ...
    describe('formatPromptLlama2Chat', () => {
        it('should format a single user message correctly', () => {
            ...
            expect((0, bedrock_1.formatPromptLlama2Chat)(messages)).toBe(expectedPrompt);
        });
        it('should handle a system message followed by a user message', () => {
            const messages = [
                { role: 'system', content: 'You are a helpful assistant.' },
            ...
            const expectedPrompt = (0, dedent_1.default) `
      <s>[INST] <<SYS>>
      You are a helpful assistant.
      <</SYS>>
    ...
    describe('formatPromptLlama3Instruct', () => {
        ...
        Hello, how are you?<|eot_id|><|start_header_id|>assistant<|end_header_id|>`;
            expect((0, bedrock_1.formatPromptLlama3Instruct)(messages)).toBe(expected);
        });
    ...
    describe('formatPromptLlama4', () => {
        it('should format a single user message correctly', () => {
            const messages = [{ role: 'user', content: 'Hello, how are you?' }];
            const expected = (0, dedent_1.default) `<|begin_of_text|><|header_start|>user<|header_end|>

Hello, how are you?<|eot|><|header_start|>assistant<|header_end|>`;
            expect((0, bedrock_1.formatPromptLlama4)(messages)).toBe(expected);
        });
        it('should format multiple messages correctly', () => {
            const messages = [
                { role: 'user', content: 'Hello' },
...
What's the weather like?<|eot|><|header_start|>assistant<|header_end|>`;
        ...
        it('should handle system messages correctly', () => {
            ...
            const expected = (0, dedent_1.default) `<|begin_of_text|><|header_start|>system<|header_end|>
...
describe('BEDROCK_MODEL AMAZON_NOVA', () => {
    const modelHandler = bedrock_1.BEDROCK_MODEL.AMAZON_NOVA;
...
describe('BEDROCK_MODEL MISTRAL', () => {
    const modelHandler = bedrock_1.BEDROCK_MODEL.MISTRAL;
    describe('params', () => {
...
describe('BEDROCK_MODEL OPENAI', () => {
    const modelHandler = bedrock_1.BEDROCK_MODEL.OPENAI;
    describe('params', () => {
    ...
    describe('output', () => {
        it('should extract output from OpenAI chat completion format', () => {
            ...
            expect(modelHandler.output({}, mockResponse)).toBe('This is a test response from OpenAI model.');
        });
        it('should throw an error for API errors', () => {
            const mockErrorResponse = { error: 'API Error occurred' };
            expect(() => modelHandler.output({}, mockErrorResponse)).toThrow('OpenAI API error: API Error occurred');
        });
        it('should return undefined for unrecognized formats', () => {
            const mockResponse = { something: 'else' };
            const result = modelHandler.output({}, mockResponse);
...
describe('BEDROCK_MODEL MISTRAL_LARGE_2407', () => {
    const modelHandler = bedrock_1.BEDROCK_MODEL.MISTRAL_LARGE_2407;
    describe('params', () => {
    ...
    describe('output', () => {
        it('should extract output from chat completion format', () => {
            ...
            expect(modelHandler.output({}, mockResponse)).toBe('This is a test response in chat completion format.');
        });
        it('should return undefined for unrecognized formats', () => {
            const mockResponse = { something: 'else' };
            const result = modelHandler.output({}, mockResponse);
...
describe('BEDROCK_MODEL DEEPSEEK', () => {
    const modelHandler = bedrock_1.BEDROCK_MODEL.DEEPSEEK;
    describe('params', () => {
    ...
    describe('output', () => {
        ...
        it('should handle error in response', () => {
            ...
            expect(() => modelHandler.output({}, mockResponse)).toThrow('DeepSeek API error: API error occurred');
        });
...
describe('BEDROCK_MODEL token counting functionality', () => {
    ...
    describe('Claude model handlers', () => {
        it('should handle new-style token fields in Claude Messages', () => {
            ...
            const result = bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES.tokenUsage(mockResponse, 'Test prompt');
            expect(result).toEqual({
                prompt: 15,
                completion: 25,
                total: 40,
        ...
        it('should handle old-style token fields in Claude Completion', () => {
            ...
            const result = bedrock_1.BEDROCK_MODEL.CLAUDE_COMPLETION.tokenUsage(mockResponse, 'Test prompt');
            expect(result).toEqual({
                prompt: 20,
                completion: 30,
                total: 50,
    ...
    describe('AMAZON_NOVA model handler', () => {
        it('should handle numeric token counts', () => {
            ...
            const result = bedrock_1.BEDROCK_MODEL.AMAZON_NOVA.tokenUsage(mockResponse, 'Test prompt');
            expect(result).toEqual({
                prompt: 100,
                completion: 200,
                total: 300,
    ...
    describe('COHERE model handlers', () => {
        it('should handle numeric token counts in COHERE_COMMAND', () => {
            ...
            const result = bedrock_1.BEDROCK_MODEL.COHERE_COMMAND.tokenUsage(mockResponse, 'Test prompt');
            expect(result).toEqual({
                prompt: 50,
                completion: 100,
                total: 150,
...
describe('AWS_BEDROCK_MODELS mapping', () => {
    it('should have the correct model mappings', () => {
        expect(bedrock_1.AWS_BEDROCK_MODELS['anthropic.claude-3-5-sonnet-20241022-v2:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-5-sonnet-20241022-v2:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['anthropic.claude-3-7-sonnet-20250219-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-7-sonnet-20250219-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.anthropic.claude-3-7-sonnet-20250219-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['meta.llama3-1-405b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_1);
        expect(bedrock_1.AWS_BEDROCK_MODELS['mistral.mistral-large-2407-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.MISTRAL_LARGE_2407);
        expect(bedrock_1.AWS_BEDROCK_MODELS['meta.llama4-scout-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
        expect(bedrock_1.AWS_BEDROCK_MODELS['meta.llama4-maverick-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama4-scout-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama4-maverick-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
    });
    it('should support newer model IDs via region prefixes', () => {
        [
            ...
            const handler = bedrock_1.AWS_BEDROCK_MODELS[baseModelId] ||
                (baseModelId.startsWith('meta.llama3-2') ? bedrock_1.BEDROCK_MODEL.LLAMA3_2 : null) ||
                (baseModelId.startsWith('meta.llama4') ? bedrock_1.BEDROCK_MODEL.LLAMA4 : null) ||
                (baseModelId.startsWith('anthropic.claude') ? bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES : null);
            expect(handler).toBeTruthy();
        });
    });
    it('should map DeepSeek models correctly', () => {
        expect(bedrock_1.AWS_BEDROCK_MODELS['deepseek.r1-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.DEEPSEEK);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.deepseek.r1-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.DEEPSEEK);
    });
    it('should map OpenAI models correctly', () => {
        expect(bedrock_1.AWS_BEDROCK_MODELS['openai.gpt-oss-120b-1:0']).toBe(bedrock_1.BEDROCK_MODEL.OPENAI);
        expect(bedrock_1.AWS_BEDROCK_MODELS['openai.gpt-oss-20b-1:0']).toBe(bedrock_1.BEDROCK_MODEL.OPENAI);
    });
    it('should map APAC regional models correctly', () => {
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.amazon.nova-lite-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.amazon.nova-micro-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.amazon.nova-pro-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.amazon.nova-premier-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.anthropic.claude-3-5-sonnet-20240620-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.anthropic.claude-3-haiku-20240307-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.anthropic.claude-opus-4-1-20250805-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.anthropic.claude-sonnet-4-20250514-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.meta.llama4-scout-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
        expect(bedrock_1.AWS_BEDROCK_MODELS['apac.meta.llama4-maverick-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
    });
    it('should map EU regional models correctly', () => {
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.amazon.nova-lite-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.amazon.nova-micro-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.amazon.nova-pro-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.amazon.nova-premier-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.anthropic.claude-3-5-sonnet-20240620-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.anthropic.claude-3-7-sonnet-20250219-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.anthropic.claude-3-haiku-20240307-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.anthropic.claude-opus-4-1-20250805-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.anthropic.claude-sonnet-4-20250514-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.meta.llama3-2-1b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_2);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.meta.llama3-2-3b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_2);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.meta.llama4-scout-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
        expect(bedrock_1.AWS_BEDROCK_MODELS['eu.meta.llama4-maverick-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
    });
    it('should map US regional models correctly', () => {
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.amazon.nova-lite-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.amazon.nova-micro-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.amazon.nova-pro-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.amazon.nova-premier-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.AMAZON_NOVA);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-5-haiku-20241022-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-5-sonnet-20240620-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-5-sonnet-20241022-v2:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-7-sonnet-20250219-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-haiku-20240307-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-opus-20240229-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-opus-4-20250514-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-opus-4-1-20250805-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-sonnet-4-20250514-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.deepseek.r1-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.DEEPSEEK);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama3-1-405b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_1);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama3-1-70b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_1);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama3-1-8b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_1);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama3-2-11b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_2);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama3-2-1b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_2);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama3-2-3b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_2);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama3-2-90b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_2);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama3-3-70b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA3_3);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama4-scout-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.meta.llama4-maverick-17b-instruct-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.LLAMA4);
    });
    it('should map US Gov Cloud models correctly', () => {
        expect(bedrock_1.AWS_BEDROCK_MODELS['us-gov.anthropic.claude-3-5-sonnet-20240620-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
        expect(bedrock_1.AWS_BEDROCK_MODELS['us-gov.anthropic.claude-3-haiku-20240307-v1:0']).toBe(bedrock_1.BEDROCK_MODEL.CLAUDE_MESSAGES);
    });
});
describe('AwsBedrockCompletionProvider', () => {
    const { BedrockRuntime } = jest.requireMock('@aws-sdk/client-bedrock-runtime');
    const mockInvokeModel = jest.fn();
    let originalModelHandler;
    let mockCache;
    beforeEach(() => {
        jest.clearAllMocks();
        ...
        originalModelHandler = bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-7-sonnet-20250219-v1:0'];
        bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-7-sonnet-20250219-v1:0'] = {
    ...
    afterEach(() => {
        bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-7-sonnet-20250219-v1:0'] = originalModelHandler;
    });
    it('should pass base config to model.params when context is not provided', async () => {
        const provider = new (class extends bedrock_1.AwsBedrockCompletionProvider {
            constructor() {
        ...
        expect(bedrock_1.AWS_BEDROCK_MODELS['us.anthropic.claude-3-7-sonnet-20250219-v1:0'].params).toHaveBeenCalledWith(expect.objectContaining({
            region: 'us-east-1',
            temperature: 0.5,
        }), 'test prompt', expect.any(Array), 'us.anthropic.claude-3-7-sonnet-20250219-v1:0');
    });
...
describe('BEDROCK_MODEL.QWEN', () => {
    const qwenHandler = bedrock_1.BEDROCK_MODEL.QWEN;
    describe('params', () => {
...
describe('Qwen model mapping', () => {
    it('should include all Qwen models in AWS_BEDROCK_MODELS', () => {
        ...
        qwenModels.forEach((modelId) => {
            expect(bedrock_1.AWS_BEDROCK_MODELS[modelId]).toBeDefined();
            expect(bedrock_1.AWS_BEDROCK_MODELS[modelId]).toBe(bedrock_1.BEDROCK_MODEL.QWEN);
        });
    });
    it('should recognize qwen models by prefix', () => {
        const provider = new bedrock_1.AwsBedrockCompletionProvider('qwen.qwen3-coder-480b-a35b-v1:0');
...
describe('coerceStrToNum', () => {
    it('should convert string numbers to numeric values', () => {
        expect((0, bedrock_1.coerceStrToNum)('42')).toBe(42);
        expect((0, bedrock_1.coerceStrToNum)('3.14')).toBe(3.14);
        expect((0, bedrock_1.coerceStrToNum)('-10')).toBe(-10);
        expect((0, bedrock_1.coerceStrToNum)('0')).toBe(0);
    });
    ...
    it('should handle undefined values', () => {
        expect((0, bedrock_1.coerceStrToNum)(undefined)).toBeUndefined();
    });
    it('should convert invalid string numbers to NaN', () => {
        expect(Number.isNaN((0, bedrock_1.coerceStrToNum)('not-a-number'))).toBe(true);
    });
});
