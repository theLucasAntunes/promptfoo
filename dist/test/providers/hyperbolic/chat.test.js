...
describe('HyperbolicProvider', () => {
    ...
    beforeEach(() => {
        provider = new chat_1.HyperbolicProvider(modelName, options);
    });
    afterEach(() => {
...
describe('calculateHyperbolicCost', () => {
    ...
    it('should use custom cost from config if provided', () => {
        ...
        const cost = (0, chat_1.calculateHyperbolicCost)('deepseek-ai/DeepSeek-R1', config, 1000, 500);
        expect(cost).toBe(0.001 * 1000 + 0.001 * 500);
    });
});
describe('createHyperbolicProvider', () => {
    it('should create provider with correct model name', () => {
        const provider = (0, chat_1.createHyperbolicProvider)('hyperbolic:deepseek-ai/DeepSeek-R1');
        expect(provider.id()).toBe('hyperbolic:deepseek-ai/DeepSeek-R1');
    });
    it('should throw error if model name is missing', () => {
        expect(() => (0, chat_1.createHyperbolicProvider)('hyperbolic:')).toThrow('Model name is required');
    });
});
describe('HYPERBOLIC_CHAT_MODELS', () => {
    it('should contain valid model definitions', () => {
        for (const model of chat_1.HYPERBOLIC_CHAT_MODELS) {
...
describe('HYPERBOLIC_REASONING_MODELS', () => {
    it('should be a subset of HYPERBOLIC_CHAT_MODELS', () => {
        for (const modelId of chat_1.HYPERBOLIC_REASONING_MODELS) {
            const found = chat_1.HYPERBOLIC_CHAT_MODELS.some((m) => m.id === modelId || m.aliases.includes(modelId));
