...
const index_2 = require("../src/prompts/index");
const providers = __importStar(require("../src/providers/index"));
const index_3 = require("../src/util/index");
const fileUtils = __importStar(require("../src/util/file"));
jest.mock('../src/cache');
...
jest.mock('../src/evaluator', () => {
    ...
    return {
        ...
        evaluate: jest.fn().mockImplementation(async (testSuite) => {
            ...
            const results = (testSuite.tests || []).map((test, idx) => {
...
describe('evaluate function', () => {
    let loadApiProvidersSpy;
    let loadApiProviderSpy;
    beforeEach(() => {
        jest.clearAllMocks();
    ...
    it('should handle function prompts correctly', async () => {
        ...
        expect(index_2.readProviderPromptMap).toHaveBeenCalledWith(testSuite, [
            {
    ...
    it('should write output to file when outputPath is set', async () => {
        ...
        await (0, index_1.evaluate)(testSuite);
        expect(index_3.writeOutput).toHaveBeenCalledWith('test.json', expect.any(eval_1.default), null);
    });
    it('should write multiple outputs when outputPath is an array', async () => {
        ...
        await (0, index_1.evaluate)(testSuite);
        expect(index_3.writeMultipleOutputs).toHaveBeenCalledWith(['test1.json', 'test2.json'], expect.any(eval_1.default), null);
    });
...
describe('evaluate with external defaultTest', () => {
    ...
    it('should load defaultTest from external file when using file:// syntax', async () => {
        ...
        const summary = await (0, index_1.evaluate)(testSuite, {});
        expect(fileUtils.maybeLoadFromExternalFile).toHaveBeenCalledWith('file://path/to/defaultTest.yaml');
    ...
    it('should handle inline defaultTest objects', async () => {
        const inlineDefaultTest = {
            assert: [{ type: 'contains', value: 'inline' }],
            vars: { inline: true },
        };
        const mockApiProvider = {
            id: () => 'mock-provider',
    ...
    it('should handle missing external defaultTest file gracefully', async () => {
        ...
        maybeLoadFromExternalFileSpy.mockRejectedValueOnce(new Error('File does not exist: /Users/mdangelo/projects/pf-codium/nonexistent.yaml'));
        await expect((0, index_1.evaluate)(testSuite, {})).rejects.toThrow('File does not exist');
    });
    ...
    describe('ApiProvider instances in configurations', () => {
        ...
        beforeEach(() => {
            ...
            resolveProviderSpy = jest
                ...
                if (typeof provider === 'object' && 'id' in provider && typeof provider.id === 'string') {
                    ...
                    return { id: () => provider.id, callApi: jest.fn() };
                }
        ...
        it('should not resolve ApiProvider instances in defaultTest.options.provider', async () => {
            ...
            expect(resolveProviderSpy).not.toHaveBeenCalledWith(mockCustomProvider, expect.any(Object), expect.any(Object));
