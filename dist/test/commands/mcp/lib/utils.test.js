...
const utils_1 = require("../../../../src/commands/mcp/lib/utils");
describe('MCP Utility Functions', () => {
    describe('createToolResponse', () => {
        it('should create successful tool response with data', () => {
            const response = (0, utils_1.createToolResponse)('test_tool', true, { result: 'success' });
        ...
        it('should create error tool response with error message', () => {
            const response = (0, utils_1.createToolResponse)('test_tool', false, undefined, 'Something went wrong');
        ...
        it('should create response without data or error', () => {
            const response = (0, utils_1.createToolResponse)('test_tool', true);
            const parsedContent = JSON.parse(response.content[0].text);
            expect(parsedContent.tool).toBe('test_tool');
            expect(parsedContent.success).toBe(true);
            expect(parsedContent.data).toBeUndefined();
    ...
    describe('withTimeout', () => {
        it('should resolve when promise resolves before timeout', async () => {
            ...
            const result = await (0, utils_1.withTimeout)(promise, 1000, 'Timed out');
            expect(result).toBe('success');
        });
        it('should reject when promise times out', async () => {
            const promise = new Promise((resolve) => setTimeout(() => resolve('late'), 100));
            await expect((0, utils_1.withTimeout)(promise, 50, 'Operation timed out')).rejects.toThrow('Operation timed out');
        });
        it('should reject with original error if promise rejects', async () => {
            const promise = Promise.reject(new Error('Original error'));
            await expect((0, utils_1.withTimeout)(promise, 1000, 'Timed out')).rejects.toThrow('Original error');
        });
    });
    describe('safeStringify', () => {
        ...
        it('should use custom replacer when provided', () => {
            ...
            const result = (0, utils_1.safeStringify)(obj, 2, (key, value) => key === 'secret' ? '[REDACTED]' : value);
            expect(result).toContain('[REDACTED]');
            expect(result).toContain('data');
        });
        it('should format with spacing when provided', () => {
            const obj = { a: 1, b: 2 };
            const result = (0, utils_1.safeStringify)(obj, 2);
    ...
    describe('hasKey', () => {
        it('should return true when key exists', () => {
            const obj = { name: 'test', value: 123 };
            const result = (0, utils_1.hasKey)(obj, 'name');
            expect(result).toBe(true);
        });
        it('should return false when key does not exist', () => {
            const obj = { name: 'test' };
            const result = (0, utils_1.hasKey)(obj, 'missing');
            expect(result).toBe(false);
        });
        it('should work with inherited properties', () => {
            const obj = Object.create({ inherited: 'value' });
            obj.own = 'value';
            expect((0, utils_1.hasKey)(obj, 'own')).toBe(true);
            expect((0, utils_1.hasKey)(obj, 'inherited')).toBe(true);
        });
    });
    describe('getProperty', () => {
        it('should return property value when it exists', () => {
            const obj = { name: 'test', value: 123 };
            const result = (0, utils_1.getProperty)(obj, 'name', 'default');
            expect(result).toBe('test');
        });
        it('should return default value when property is undefined', () => {
            const obj = { name: 'test' };
            const result = (0, utils_1.getProperty)(obj, 'missing', 'default');
            expect(result).toBe('default');
        });
        it('should return property value when it is null', () => {
            const obj = { value: null };
            const result = (0, utils_1.getProperty)(obj, 'value', 'default');
            expect(result).toBeNull();
        });
        it('should return property value when it is falsy but not undefined', () => {
            const obj = { value: 0, empty: '', bool: false };
            expect((0, utils_1.getProperty)(obj, 'value', 'default')).toBe(0);
            expect((0, utils_1.getProperty)(obj, 'empty', 'default')).toBe('');
            expect((0, utils_1.getProperty)(obj, 'bool', 'default')).toBe(false);
        });
    });
    describe('assertNotNull', () => {
        it('should return value when it is not null or undefined', () => {
            expect((0, utils_1.assertNotNull)('test')).toBe('test');
            expect((0, utils_1.assertNotNull)(0)).toBe(0);
            expect((0, utils_1.assertNotNull)(false)).toBe(false);
            expect((0, utils_1.assertNotNull)('')).toBe('');
        });
        it('should throw error when value is null', () => {
            expect(() => (0, utils_1.assertNotNull)(null)).toThrow('Value cannot be null or undefined');
        });
        it('should throw error when value is undefined', () => {
            expect(() => (0, utils_1.assertNotNull)(undefined)).toThrow('Value cannot be null or undefined');
        });
        it('should throw custom error message', () => {
            expect(() => (0, utils_1.assertNotNull)(null, 'Custom error message')).toThrow('Custom error message');
        });
    });
    describe('filterNonNull', () => {
        it('should filter out null and undefined values', () => {
            const array = ['a', null, 'b', undefined, 'c', null];
            const result = (0, utils_1.filterNonNull)(array);
    ...
    describe('debounce', () => {
        ...
        it('should debounce function calls', () => {
            ...
            const debouncedFn = (0, utils_1.debounce)(mockFn, 100);
            debouncedFn('arg1');
            debouncedFn('arg2');
            debouncedFn('arg3');
    ...
    describe('retry', () => {
        it('should succeed on first attempt', async () => {
            ...
            const result = await (0, utils_1.retry)(operation);
            expect(result).toBe('success');
        ...
        it('should retry on failure and eventually succeed', async () => {
            ...
            const result = await (0, utils_1.retry)(operation, { maxAttempts: 3, baseDelay: 10 });
            expect(result).toBe('success');
            expect(operation).toHaveBeenCalledTimes(3);
        });
        it('should throw last error after max attempts', async () => {
            const operation = jest.fn().mockRejectedValue(new Error('persistent failure'));
            await expect((0, utils_1.retry)(operation, { maxAttempts: 2, baseDelay: 10 })).rejects.toThrow('persistent failure');
        ...
        it('should use exponential backoff', async () => {
            ...
            await (0, utils_1.retry)(operation, { maxAttempts: 3, baseDelay: 100, backoffFactor: 2 });
        ...
        it('should respect max delay', async () => {
            ...
            await (0, utils_1.retry)(operation, {
                maxAttempts: 2,
                baseDelay: 1000,
                backoffFactor: 10,
                maxDelay: 500,
    ...
    describe('formatDuration', () => {
        ...
        it('should format seconds', () => {
            ...
            expect((0, utils_1.formatDuration)(59000)).toBe('59s');
        });
        it('should format minutes and seconds', () => {
            expect((0, utils_1.formatDuration)(60000)).toBe('1m');
            expect((0, utils_1.formatDuration)(90000)).toBe('1m 30s');
            expect((0, utils_1.formatDuration)(3599000)).toBe('59m 59s');
        });
        it('should format hours and minutes', () => {
            expect((0, utils_1.formatDuration)(3600000)).toBe('1h');
            expect((0, utils_1.formatDuration)(3660000)).toBe('1h 1m');
            expect((0, utils_1.formatDuration)(7200000)).toBe('2h');
        });
        it('should handle zero duration', () => {
            expect((0, utils_1.formatDuration)(0)).toBe('0ms');
        });
    });
    describe('truncateText', () => {
        it('should return original text if shorter than max length', () => {
            const text = 'Short text';
            const result = (0, utils_1.truncateText)(text, 20);
            expect(result).toBe('Short text');
        });
        it('should return original text if equal to max length', () => {
            const text = 'Exact length';
            const result = (0, utils_1.truncateText)(text, 12);
        ...
        it('should handle max length less than ellipsis length', () => {
            const text = 'Hello';
            const result = (0, utils_1.truncateText)(text, 2);
        ...
        it('should handle empty string', () => {
            const result = (0, utils_1.truncateText)('', 10);
            expect(result).toBe('');
        });
    });
});
