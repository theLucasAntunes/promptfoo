...
describe('AssertionSchema', () => {
    it('should validate a basic assertion', () => {
        ...
        const result = index_1.AssertionSchema.safeParse(basicAssertion);
    ...
    it('should validate an assertion with all optional fields', () => {
        ...
        const result = index_1.AssertionSchema.safeParse(fullAssertion);
        expect(result.success).toBe(true);
    });
    it('should validate all base assertion types', () => {
        const baseTypes = index_1.BaseAssertionTypesSchema.options;
        baseTypes.forEach((type) => {
            const assertion = {
                type,
                value: 'test value',
            };
            const result = index_1.AssertionSchema.safeParse(assertion);
            expect(result.success).toBe(true);
        });
    });
    it('should validate "not-" prefixed assertion types', () => {
        ...
        const result = index_1.AssertionSchema.safeParse(notPrefixedAssertion);
        expect(result.success).toBe(true);
    });
    it('should validate assertions with function values', () => {
        ...
        const result = index_1.AssertionSchema.safeParse(functionAssertion);
        expect(result.success).toBe(true);
    });
    it('should validate assertions with array values', () => {
        ...
        const result = index_1.AssertionSchema.safeParse(arrayAssertion);
        expect(result.success).toBe(true);
    });
});
describe('VarsSchema', () => {
    it('should validate and transform various types of values', () => {
        ...
        testCases.forEach(({ input, expected }) => {
            expect(index_1.VarsSchema.safeParse(input)).toEqual({ success: true, data: expected });
        });
    });
    it('should throw an error for invalid types', () => {
        ...
        invalidCases.forEach((invalidInput) => {
            expect(() => index_1.VarsSchema.parse(invalidInput)).toThrow(zod_1.z.ZodError);
        });
    });
});
describe('isGradingResult', () => {
    it('should return true for valid grading result object', () => {
        ...
        expect((0, index_1.isGradingResult)(validResult)).toBe(true);
    });
    it('should return true for grading result with optional fields', () => {
        ...
        expect((0, index_1.isGradingResult)(resultWithOptional)).toBe(true);
    });
    it('should correctly identify valid GradingResult objects', () => {
        ...
        validResults.forEach((result) => {
            expect((0, index_1.isGradingResult)(result)).toBe(true);
        });
    });
    it('should return false for null', () => {
        expect((0, index_1.isGradingResult)(null)).toBe(false);
    });
    it('should return false for non-object', () => {
        expect((0, index_1.isGradingResult)('not an object')).toBe(false);
        expect((0, index_1.isGradingResult)(123)).toBe(false);
        expect((0, index_1.isGradingResult)(undefined)).toBe(false);
    });
    it('should return false if missing required fields', () => {
        expect((0, index_1.isGradingResult)({ score: 1, reason: 'test' })).toBe(false);
        expect((0, index_1.isGradingResult)({ pass: true, reason: 'test' })).toBe(false);
        expect((0, index_1.isGradingResult)({ pass: true, score: 1 })).toBe(false);
    });
    it('should correctly identify invalid GradingResult objects', () => {
        ...
        invalidResults.forEach((result) => {
            expect((0, index_1.isGradingResult)(result)).toBe(false);
        });
    });
    it('should return false if fields have wrong types', () => {
        expect((0, index_1.isGradingResult)({
            pass: 'true',
            score: '0.8',
            reason: 123,
        })).toBe(false);
...
describe('TestCaseSchema assertScoringFunction', () => {
    it('should validate test case with valid file-based scoring function', () => {
        ...
        expect(() => index_1.TestCaseSchema.parse(testCase)).not.toThrow('Invalid test case schema');
    });
...
describe('CommandLineOptionsSchema', () => {
    it('should validate options with filterErrorsOnly string', () => {
        ...
        expect(() => index_1.CommandLineOptionsSchema.parse(options)).not.toThrow('Invalid command line options');
    });
    ...
    it('should reject options with non-string filterErrorsOnly', () => {
        ...
        expect(() => index_1.CommandLineOptionsSchema.parse(options)).toThrow('Expected string, received boolean');
    });
...
describe('TestSuiteConfigSchema', () => {
    ...
    describe('env property', () => {
        it('should validate config with string env values', () => {
            ...
            expect(() => index_1.TestSuiteConfigSchema.parse(config)).not.toThrow();
        });
        ...
        it('should validate env field with primitive values that get transformed to strings', () => {
            ...
            const result = index_1.TestSuiteConfigSchema.safeParse(config);
            expect(result.success).toBe(true);
    ...
    describe('extensions field', () => {
        ...
        it('should transform and remove null, undefined, and empty array extensions', () => {
            ...
            configs.forEach((config) => {
                const result = index_1.UnifiedConfigSchema.safeParse(config);
                expect(result.success).toBe(true);
    ...
    describe('defaultTest validation', () => {
        it('should accept string defaultTest starting with file://', () => {
            ...
            const result = index_1.TestSuiteConfigSchema.safeParse(validConfig);
            expect(result.success).toBe(true);
        ...
        it('should reject string defaultTest not starting with file://', () => {
            ...
            const result = index_1.TestSuiteConfigSchema.safeParse(invalidConfig);
        ...
        it('should accept defaultTest with description field (it will be stripped)', () => {
            ...
            const result = index_1.TestSuiteConfigSchema.safeParse(configWithDescription);
            expect(result.success).toBe(true);
            // The description field should be stripped from the parsed result
            expect(result.data.defaultTest).toEqual({
                assert: [{ type: 'equals', value: 'test' }],
            });
            expect(result.data.defaultTest.description).toBeUndefined();
        });
    });
    for (const file of configFiles) {
        it(`should validate ${path_1.default.relative(rootDir, file)}`, async () => {
            ...
            const extendedSchema = index_1.TestSuiteConfigSchema.extend({
                targets: zod_1.z.union([index_1.TestSuiteConfigSchema.shape.providers, zod_1.z.undefined()]),
                providers: zod_1.z.union([index_1.TestSuiteConfigSchema.shape.providers, zod_1.z.undefined()]),
                ...(typeof config.redteam !== 'undefined' && {
                    prompts: zod_1.z.optional(index_1.TestSuiteConfigSchema.shape.prompts),
                }),
            }).refine((data) => {
...
describe('UnifiedConfigSchema extensions handling', () => {
    it('should remove null extensions', () => {
        ...
        const parsed = index_1.UnifiedConfigSchema.parse(config);
    ...
    it('should throw an error when both targets and providers are present', () => {
        ...
        expect(() => index_1.UnifiedConfigSchema.parse(config)).toThrow("Exactly one of 'targets' or 'providers' must be provided, but not both");
    });
});
describe('TestSuiteSchema', () => {
    ...
    describe('extensions field', () => {
        it('should allow null extensions', () => {
            ...
            expect(() => index_1.TestSuiteSchema.parse(suite)).not.toThrow();
        });
        ...
        it('should accept valid Python extension paths', () => {
            ...
            validExtensions.forEach((extension) => {
                const result = index_1.TestSuiteSchema.safeParse({ ...baseTestSuite, extensions: [extension] });
                expect(result.success).toBe(true);
            });
        });
        ...
        it('should allow an empty array of extensions', () => {
            const result = index_1.TestSuiteSchema.safeParse({ ...baseTestSuite, extensions: [] });
            expect(result.success).toBe(true);
        });
    });
    describe('defaultTest validation', () => {
        it('should accept string defaultTest starting with file://', () => {
            ...
            const result = index_1.TestSuiteSchema.safeParse(validConfig);
            expect(result.success).toBe(true);
            expect(result.data.defaultTest).toBe('file://path/to/defaultTest.yaml');
        });
        it('should reject string defaultTest not starting with file://', () => {
            ...
            const result = index_1.TestSuiteSchema.safeParse(invalidConfig);
            expect(result.success).toBe(false);
            expect(result.error.errors[0].message).toContain('defaultTest string must start with file://');
        });
        it('should accept object defaultTest', () => {
            ...
            const result = index_1.TestSuiteSchema.safeParse(validConfig);
            expect(result.success).toBe(true);
            expect(result.data.defaultTest).toEqual(expect.objectContaining({
                assert: [{ type: 'equals', value: 'test' }],
                vars: { foo: 'bar' },
            }));
        });
