...
describe('Google Sheets Integration', () => {
    ...
    describe('checkGoogleSheetAccess', () => {
        it('should return public:true for accessible sheets', async () => {
            jest.mocked(index_1.fetchWithProxy).mockResolvedValue((0, utils_1.createMockResponse)({ status: 200 }));
            const result = await (0, googleSheets_1.checkGoogleSheetAccess)(TEST_SHEET_URL);
            expect(result).toEqual({ public: true, status: 200 });
            expect(index_1.fetchWithProxy).toHaveBeenCalledWith(TEST_SHEET_URL);
        });
        it('should return public:false for inaccessible sheets', async () => {
            jest.mocked(index_1.fetchWithProxy).mockResolvedValue((0, utils_1.createMockResponse)({ status: 403 }));
            const result = await (0, googleSheets_1.checkGoogleSheetAccess)(TEST_SHEET_URL);
            expect(result).toEqual({ public: false, status: 403 });
            expect(index_1.fetchWithProxy).toHaveBeenCalledWith(TEST_SHEET_URL);
        });
        it('should handle network errors gracefully', async () => {
            jest.mocked(index_1.fetchWithProxy).mockRejectedValue(new Error('Network error'));
            const result = await (0, googleSheets_1.checkGoogleSheetAccess)(TEST_SHEET_URL);
            expect(result).toEqual({ public: false });
            expect(logger_1.default.error).toHaveBeenCalledWith(`Error checking sheet access: ${new Error('Network error')}`);
        });
    });
    describe('fetchCsvFromGoogleSheetUnauthenticated', () => {
        it('should fetch and parse CSV data correctly', async () => {
            ...
            expect(index_1.fetchWithProxy).toHaveBeenCalledWith(expectedUrl);
        });
    ...
    describe('fetchCsvFromGoogleSheetAuthenticated', () => {
        ...
        it('should throw error when no sheets found', async () => {
            ...
            await expect((0, googleSheets_1.fetchCsvFromGoogleSheetAuthenticated)(TEST_SHEET_URL)).rejects.toThrow('No sheets found in spreadsheet');
        });
        it('should throw error when sheet with gid not found', async () => {
            await expect((0, googleSheets_1.fetchCsvFromGoogleSheetAuthenticated)('https://docs.google.com/spreadsheets/d/1234567890/edit?gid=99999')).rejects.toThrow('Sheet not found for gid: 99999');
        });
    });
    describe('Range behavior and backwards compatibility', () => {
        ...
        it('should calculate correct column letters for write operations', async () => {
            ...
            for (const { cols, expected } of testCases) {
                ...
                await (0, googleSheets_1.writeCsvToGoogleSheet)(mockRows, TEST_SHEET_URL);
