...
const functionToolCall_1 = require("../../src/assertions/functionToolCall");
const openai_1 = require("../../src/assertions/openai");
const chat_1 = require("../../src/providers/openai/chat");
const util_1 = require("../../src/providers/openai/util");
const index_2 = require("../../src/util/index");
jest.mock('fs');
jest.mock('path', () => ({
...
const mockMaybeLoadToolsFromExternalFile = jest.mocked(index_2.maybeLoadToolsFromExternalFile);
const toolsAssertion = {
    type: 'is-valid-openai-tools-call',
};
const functionAssertion = {
    type: 'is-valid-openai-function-call',
};
const mockProvider = new chat_1.OpenAiChatCompletionProvider('test-provider', {
    config: {
        tools: [
            {
                type: 'function',
...
describe('OpenAI assertions', () => {
    ...
    describe('is-valid-openai-function-call assertion', () => {
        it('should pass for a valid function call with correct arguments', async () => {
            ...
            const provider = new chat_1.OpenAiChatCompletionProvider('foo', {
                config: {
                    functions: [
                        {
                            name: 'add',
        ...
        it('should fail for an invalid function call with incorrect arguments', async () => {
            ...
            const result = await (0, index_1.runAssertion)({
                prompt: 'Some prompt',
                provider: new chat_1.OpenAiChatCompletionProvider('foo', {
                    config: {
                        functions: [
                            {
                                name: 'add',
    ...
    describe('handleIsValidFunctionCall', () => {
        it('should pass when function call matches schema', () => {
            ...
            const result = (0, functionToolCall_1.handleIsValidFunctionCall)({
...
`;
            ...
            expect(() => {
                (0, util_1.validateFunctionCall)(functionOutput, fileProvider.config.functions, {});
            }).not.toThrow();
            expect(mockedFs.existsSync).toHaveBeenCalledWith('./test/fixtures/weather_functions.yaml');
            expect(mockedFs.readFileSync).toHaveBeenCalledWith('./test/fixtures/weather_functions.yaml', 'utf8');
        });
        it('should render variables in function definitions', () => {
            ...
            const varProvider = new chat_1.OpenAiChatCompletionProvider('test-provider', {
                config: {
                    functions: [
                        {
                            name: 'getCurrentTemperature',
        ...
        it('should fail when functions are not defined', () => {
            ...
            const emptyProvider = new chat_1.OpenAiChatCompletionProvider('test-provider', {
                config: {},
            });
            const result = (0, functionToolCall_1.handleIsValidFunctionCall)({
                assertion: functionAssertion,
    ...
    describe('handleIsValidOpenAiToolsCall', () => {
        it('should pass when tool calls match schema', () => {
            ...
            const result = (0, openai_1.handleIsValidOpenAiToolsCall)({
        ...
        it('should load tools from external file', () => {
            ...
            expect(mockMaybeLoadToolsFromExternalFile).toHaveBeenCalledWith('file://./test/fixtures/weather_tools.json', {});
        });
        ...
        it('should fail when tools are not defined', () => {
            ...
            expect(() => (0, openai_1.handleIsValidOpenAiToolsCall)({
                assertion: toolsAssertion,
                output: toolsOutput,
                provider: emptyProvider,
                test: { vars: {} },
        ...
        it('should fail when tool output is not an array', () => {
            ...
            const emptyToolsProvider = new chat_1.OpenAiChatCompletionProvider('test-provider', {
                config: {
                    tools: [],
                },
            });
        ...
        it('should handle external file references for tools', () => {
            ...
            const fileProvider = new chat_1.OpenAiChatCompletionProvider('test-provider', {
                config: {
                    tools: 'file://./test/fixtures/weather_tools.json',
                },
            });
    ...
    describe('handleIsValidOpenAiToolsCall with MCP support', () => {
        it('should pass when MCP tool call succeeds', () => {
            const mcpOutput = 'MCP Tool Result (ask_question): React is a JavaScript library for building user interfaces.';
            const result = (0, openai_1.handleIsValidOpenAiToolsCall)({
                assertion: toolsAssertion,
                output: mcpOutput,
                provider: mockProvider,
        ...
        it('should handle MCP tool result with complex tool name', () => {
            const mcpOutput = 'MCP Tool Result (read_wiki_structure): Successfully retrieved repository structure.';
            const result = (0, openai_1.handleIsValidOpenAiToolsCall)({
                assertion: toolsAssertion,
                output: mcpOutput,
                provider: mockProvider,
        ...
        it('should handle MCP tool error with complex error message', () => {
            const mcpOutput = 'MCP Tool Error (stripe_payment): Authentication failed: Invalid API key provided';
            const result = (0, openai_1.handleIsValidOpenAiToolsCall)({
                assertion: toolsAssertion,
                output: mcpOutput,
                provider: mockProvider,
            ...
            expect(result).toEqual({
                pass: false,
                score: 0,
                reason: 'MCP tool call failed for stripe_payment: Authentication failed: Invalid API key provided',
                assertion: toolsAssertion,
            });
        });
