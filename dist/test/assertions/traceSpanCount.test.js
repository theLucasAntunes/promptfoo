...
const traceSpanCount_1 = require("../../src/assertions/traceSpanCount");
const mockProvider = {
    id: () => 'mock',
...
describe('handleTraceSpanCount', () => {
    it('should pass when span count is within max limit', () => {
        ...
        const result = (0, traceSpanCount_1.handleTraceSpanCount)(params);
        expect(result).toEqual({
            pass: true,
            score: 1,
            reason: 'Found 2 spans matching pattern "*llm*" (expected at most 3). Matched spans: llm.completion, llm.chat',
    ...
    it('should fail when span count exceeds max limit', () => {
        ...
        expect(result).toEqual({
            pass: false,
            score: 0,
            reason: 'Found 2 spans matching pattern "*llm*", expected at most 1. Matched spans: llm.completion, llm.chat',
            assertion: params.assertion,
        });
    });
    it('should pass when span count meets min requirement', () => {
        ...
        expect(result).toEqual({
            pass: true,
            score: 1,
            reason: 'Found 1 spans matching pattern "*retrieval*" (expected at least 1). Matched spans: retrieval.search',
            assertion: params.assertion,
        });
    });
    it('should fail when span count is below min requirement', () => {
        ...
        expect(result).toEqual({
            pass: false,
            score: 0,
            reason: 'Found 1 spans matching pattern "*retrieval*", expected at least 2. Matched spans: retrieval.search',
            assertion: params.assertion,
        });
    });
    it('should pass when span count is within min and max range', () => {
        ...
        expect(result).toEqual({
            pass: true,
            score: 1,
            reason: 'Found 5 spans matching pattern "*" (expected 3-10). Matched spans: llm.completion, llm.chat, database.query, retrieval.search, api.external_call',
            assertion: params.assertion,
        });
    });
    it('should handle patterns with ? wildcard', () => {
        ...
        expect(result).toEqual({
            pass: true,
            score: 1,
            reason: 'Found 1 spans matching pattern "llm.c?at" (expected at most 1). Matched spans: llm.chat',
            assertion: params.assertion,
        });
    });
    it('should handle case-insensitive matching', () => {
        ...
        expect(result).toEqual({
            pass: true,
            score: 1,
            reason: 'Found 2 spans matching pattern "*LLM*" (expected at most 5). Matched spans: llm.completion, llm.chat',
            assertion: params.assertion,
        });
    });
    it('should fail when no trace data is available', () => {
        ...
        expect(() => (0, traceSpanCount_1.handleTraceSpanCount)(params)).toThrow('No trace data available for trace-span-count assertion');
    });
    ...
    it('should throw error for invalid value format', () => {
        ...
        expect(() => (0, traceSpanCount_1.handleTraceSpanCount)(params)).toThrow('trace-span-count assertion must have a value object with pattern property');
    });
...
//# sourceMappingURL=traceSpanCount.test.js.map