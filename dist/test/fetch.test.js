...
jest.mock('../src/envars', () => {
    return {
        ...
        getEnvInt: jest.fn().mockImplementation((key, defaultValue = 0) => {
...
describe('fetchWithProxy', () => {
    ...
    it('should add version header to all requests', async () => {
        ...
        await (0, index_1.fetchWithProxy)(url);
    ...
    it('should handle URLs with basic auth credentials', async () => {
        ...
        await (0, index_1.fetchWithProxy)(url, options);
    ...
    it('should handle invalid URLs gracefully', async () => {
        ...
        await (0, index_1.fetchWithProxy)(invalidUrl);
    ...
    it('should use custom CA certificate when PROMPTFOO_CA_CERT_PATH is set', async () => {
        ...
        await (0, index_1.fetchWithProxy)('https://example.com');
    ...
    it('should use proxy URL from environment variables in order of precedence', async () => {
        ...
        for (const testCase of httpTestCases) {
            ...
            await (0, index_1.fetchWithProxy)('http://example.com');
            expect(undici_1.ProxyAgent).toHaveBeenCalledWith({
    ...
    it('should use proxy for domains not in NO_PROXY', async () => {
        ...
        await (0, index_1.fetchWithProxy)('https://api.example.com/v1');
...
describe('fetchWithTimeout', () => {
    ...
    it('should resolve when fetch completes before timeout', async () => {
        ...
        const fetchPromise = (0, index_1.fetchWithTimeout)('https://example.com', {}, 5000);
...
describe('isRateLimited', () => {
    it('should detect standard rate limit headers', () => {
        ...
        expect((0, index_1.isRateLimited)(response)).toBe(true);
    });
    ...
    it('should detect OpenAI specific rate limits', () => {
        ...
        expect((0, index_1.isRateLimited)(tokenResponse)).toBe(true);
    });
    it('should return false when not rate limited', () => {
        ...
        expect((0, index_1.isRateLimited)(response)).toBe(false);
    });
});
describe('handleRateLimit', () => {
    ...
    it('should handle OpenAI reset headers', async () => {
        ...
        const promise = (0, index_1.handleRateLimit)(response);
        jest.advanceTimersByTime(5000);
        await promise;
...
describe('fetchWithRetries', () => {
    ...
    it('should make exactly one attempt when retries is 0', async () => {
        ...
        await (0, index_1.fetchWithRetries)('https://example.com', {}, 1000, 0);
    ...
    it('should handle negative retry values by treating them as 0', async () => {
        const successResponse = (0, utils_1.createMockResponse)();
        jest.mocked(global.fetch).mockResolvedValueOnce(successResponse);
        await (0, index_1.fetchWithRetries)('https://example.com', {}, 1000, -1);
        expect(global.fetch).toHaveBeenCalledTimes(1);
        expect(time_1.sleep).not.toHaveBeenCalled();
    });
    it('should make retries+1 total attempts', async () => {
        jest.mocked(global.fetch).mockRejectedValue(new Error('Network error'));
        await expect((0, index_1.fetchWithRetries)('https://example.com', {}, 1000, 2)).rejects.toThrow('Request failed after 2 retries: Error: Network error');
        expect(global.fetch).toHaveBeenCalledTimes(3);
        expect(time_1.sleep).toHaveBeenCalledTimes(2);
    });
    it('should not sleep after the final attempt', async () => {
        jest.mocked(global.fetch).mockRejectedValue(new Error('Network error'));
        await expect((0, index_1.fetchWithRetries)('https://example.com', {}, 1000, 1)).rejects.toThrow('Request failed after 1 retries: Error: Network error');
    ...
    it('should handle 5XX errors when PROMPTFOO_RETRY_5XX is true', async () => {
        ...
        global.fetch = mockFetch;
        await (0, index_1.fetchWithRetries)('https://example.com', {}, 1000, 2);
    ...
    it('should handle detailed error information', async () => {
        ...
        await expect((0, index_1.fetchWithRetries)('https://example.com', {}, 1000, 1)).rejects.toThrow('Request failed after 1 retries: Error: Network error (Cause: Connection refused) (Code: ECONNREFUSED)');
        expect(global.fetch).toHaveBeenCalledTimes(2);
        expect(time_1.sleep).toHaveBeenCalledTimes(1);
    });
    it('should handle non-Error objects in rejection', async () => {
        jest.mocked(global.fetch).mockRejectedValue('String error');
        await expect((0, index_1.fetchWithRetries)('https://example.com', {}, 1000, 1)).rejects.toThrow('Request failed after 1 retries: String error');
        expect(global.fetch).toHaveBeenCalledTimes(2);
        expect(time_1.sleep).toHaveBeenCalledTimes(1);
    });
...
describe('fetchWithProxy with NO_PROXY', () => {
    ...
    it('should respect NO_PROXY for localhost URLs', async () => {
        ...
        await (0, index_1.fetchWithProxy)('http://localhost:3000/api');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
    });
    it('should respect NO_PROXY for 127.0.0.1', async () => {
        ...
        await (0, index_1.fetchWithProxy)('http://127.0.0.1:3000/api');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
    });
    it('should respect NO_PROXY with multiple entries', async () => {
        ...
        process.env.HTTPS_PROXY = mockProxyUrl;
        process.env.NO_PROXY = noProxyList;
        await (0, index_1.fetchWithProxy)('https://example.org/api');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        process.env.HTTPS_PROXY = mockProxyUrl;
        process.env.NO_PROXY = noProxyList;
        await (0, index_1.fetchWithProxy)('https://internal.example.com/api');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        process.env.HTTPS_PROXY = mockProxyUrl;
        process.env.NO_PROXY = noProxyList;
        await (0, index_1.fetchWithProxy)('https://example.com/api');
        expect(undici_1.ProxyAgent).toHaveBeenCalledWith(expect.objectContaining({
            uri: mockProxyUrl,
        }));
        expect(undici_1.setGlobalDispatcher).toHaveBeenCalledWith(expect.any(Object));
    ...
    it('should handle wildcard patterns in NO_PROXY', async () => {
        ...
        await (0, index_1.fetchWithProxy)('https://api.example.org/v1');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        process.env.HTTPS_PROXY = mockProxyUrl;
        await (0, index_1.fetchWithProxy)('https://subdomain.api.example.org/v1');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        process.env.HTTPS_PROXY = mockProxyUrl;
        await (0, index_1.fetchWithProxy)('https://example.com/v1');
        expect(undici_1.ProxyAgent).toHaveBeenCalledWith(expect.objectContaining({
            uri: mockProxyUrl,
        }));
    });
    it('should handle domain suffix patterns in NO_PROXY', async () => {
        ...
        jest.clearAllMocks();
        process.env.HTTPS_PROXY = mockProxyUrl;
        await (0, index_1.fetchWithProxy)('https://abc.example.org/v1');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        process.env.HTTPS_PROXY = mockProxyUrl;
        await (0, index_1.fetchWithProxy)('https://abc.example.com/v1');
        expect(undici_1.ProxyAgent).toHaveBeenCalledWith(expect.objectContaining({
            uri: mockProxyUrl,
        }));
    });
    it('should handle URLs without schemes', async () => {
        ...
        await (0, index_1.fetchWithProxy)('localhost:3000');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        await (0, index_1.fetchWithProxy)('example.org');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
    });
    it('should properly parse URLs with credentials when checking against NO_PROXY', async () => {
        ...
        await (0, index_1.fetchWithProxy)('https://username:password@api.example.org/v1');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
    ...
    it('should handle bad URL inputs gracefully when checking NO_PROXY', async () => {
        ...
        process.env.HTTPS_PROXY = mockProxyUrl;
        process.env.NO_PROXY = 'localhost';
        await (0, index_1.fetchWithProxy)(':::not-a-valid-url:::');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
    });
    it('should use lowercase for NO_PROXY checks', async () => {
        ...
        await (0, index_1.fetchWithProxy)('http://localhost:3000');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        await (0, index_1.fetchWithProxy)('https://api.example.org/v1');
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
    ...
    it('should handle URL objects and Request objects', async () => {
        ...
        await (0, index_1.fetchWithProxy)(urlObj.toString());
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        process.env.HTTPS_PROXY = mockProxyUrl;
        process.env.NO_PROXY = 'localhost,example.org';
        const request = new Request('https://example.org/api');
        await (0, index_1.fetchWithProxy)(request);
        expect(undici_1.ProxyAgent).not.toHaveBeenCalled();
        jest.clearAllMocks();
        process.env.HTTP_PROXY = mockProxyUrl;
        process.env.NO_PROXY = 'localhost,example.org';
        const otherRequest = new Request('http://example.com/api');
        await (0, index_1.fetchWithProxy)(otherRequest);
        expect(undici_1.ProxyAgent).toHaveBeenCalledWith(expect.objectContaining({
            uri: mockProxyUrl,
        }));
    });
