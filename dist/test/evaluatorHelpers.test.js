...
describe('runExtensionHook', () => {
    beforeEach(() => {
        ...
        jest.mocked(transform_1.transform).mockResolvedValue(undefined);
    });
    describe('beforeAll', () => {
        ...
        describe('no extensions provided', () => {
            it('should not call transform', async () => {
                await (0, evaluatorHelpers_1.runExtensionHook)([], hookName, context);
                expect(transform_1.transform).not.toHaveBeenCalled();
                await (0, evaluatorHelpers_1.runExtensionHook)(undefined, hookName, context);
                expect(transform_1.transform).not.toHaveBeenCalled();
                await (0, evaluatorHelpers_1.runExtensionHook)(null, hookName, context);
                expect(transform_1.transform).not.toHaveBeenCalled();
            });
            it('should return the original context', async () => {
                const out = await (0, evaluatorHelpers_1.runExtensionHook)([], hookName, context);
                expect(out).toEqual(context);
                const out2 = await (0, evaluatorHelpers_1.runExtensionHook)(undefined, hookName, context);
                expect(out2).toEqual(context);
                const out3 = await (0, evaluatorHelpers_1.runExtensionHook)(null, hookName, context);
        ...
        describe('extensions provided', () => {
            ...
            it('should return the original context when extension(s) do not return a value', async () => {
                const out = await (0, evaluatorHelpers_1.runExtensionHook)(['ext1', 'ext2', 'ext3'], hookName, context);
                expect(out).toEqual(context);
            });
            it('returned context should conform to the expected schema', async () => {
                ...
                jest.mocked(transform_1.transform).mockResolvedValue({
                    suite: {
            ...
            it('should handle invalid context but not throw validation error', async () => {
                ...
                const result = await (0, evaluatorHelpers_1.runExtensionHook)(['ext1', 'ext2', 'ext3'], hookName, context);
...
describe('collectFileMetadata', () => {
    ...
    it('should identify image files correctly', () => {
        ...
        const metadata = (0, evaluatorHelpers_1.collectFileMetadata)(vars);
