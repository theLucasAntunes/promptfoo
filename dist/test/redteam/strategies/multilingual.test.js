...
describe('Multilingual Strategy', () => {
    ...
    describe('translateBatch', () => {
        it('prefers multilingual provider when available', async () => {
            ...
            jest
                .mocked(shared_1.redteamProviderManager.getMultilingualProvider)
                .mockResolvedValueOnce(multilingualProvider);
            const result = await (0, multilingual_1.translateBatch)('Hello', ['es']);
            expect(result).toEqual({ es: 'Hola' });
            expect(jest.mocked(shared_1.redteamProviderManager.getProvider)).not.toHaveBeenCalled();
        ...
        it('falls back to regular provider when multilingual provider is unavailable', async () => {
            ...
            jest.mocked(shared_1.redteamProviderManager.getProvider).mockResolvedValueOnce(provider);
            const result = await (0, multilingual_1.translateBatch)('Hello', ['fr']);
            expect(result).toEqual({ fr: 'Bonjour' });
            expect(jest.mocked(shared_1.redteamProviderManager.getProvider)).toHaveBeenCalled();
        ...
        it('should handle JSON response format', async () => {
            ...
            const result = await (0, multilingual_1.translateBatch)('Hello', ['es', 'fr']);
        ...
        it('should return empty object if nothing can be parsed', async () => {
            ...
            jest.mocked(shared_1.redteamProviderManager.getProvider).mockResolvedValue(mockProvider);
            const result = await (0, multilingual_1.translateBatch)('Hello', ['zh', 'ja']);
            expect(result).toEqual({});
        });
        ...
        it('should handle batch with empty languages', async () => {
            ...
            jest.mocked(shared_1.redteamProviderManager.getProvider).mockResolvedValue(mockProvider);
            const result = await (0, multilingual_1.translateBatch)('Hello', []);
            expect(result).toEqual({});
        });
    ...
    describe('addMultilingual', () => {
        ...
        it('should use remote generation when available', async () => {
            ...
            const result = await (0, multilingual_1.addMultilingual)([{ vars: { text: 'Test' } }], 'text', {});
            expect(result).toEqual(remoteResult);
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith('http://test.url', expect.objectContaining({
                method: 'POST',
        ...
        it('should process batches with correct size', async () => {
            ...
            await (0, multilingual_1.addMultilingual)(testCases, 'text', { languages: ['es'], maxConcurrency: 2 });
        ...
        it('should respect concurrency limits', async () => {
            ...
            await (0, multilingual_1.addMultilingual)(testCases, 'text', { languages: ['es'], maxConcurrency });
        ...
        it('should handle multiple languages in batches', async () => {
            ...
            const result = await (0, multilingual_1.addMultilingual)([testCase], 'text', {
                languages: ['es', 'fr', 'de'],
        ...
        it('should skip if languages is not an array', async () => {
            const result = await (0, multilingual_1.addMultilingual)([{ vars: { text: 'Hello' } }], 'text', {
                languages: null,
            });
            expect(result).toEqual([]);
        });
        ...
        it('should handle empty testCases array', async () => {
            const result = await (0, multilingual_1.addMultilingual)([], 'text', { languages: ['es'] });
            expect(result).toEqual([]);
        });
        it('should handle empty languages array', async () => {
            ...
            const result = await (0, multilingual_1.addMultilingual)([testCase], 'text', { languages: [] });
            expect(result).toEqual([]);
        });
        ...
        describe('getConcurrencyLimit', () => {
            it('should return default concurrency when no config provided', () => {
                expect((0, multilingual_1.getConcurrencyLimit)()).toBe(4);
            });
            it('should return configured concurrency limit', () => {
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: 8 })).toBe(8);
            });
            it('should handle invalid concurrency values', () => {
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: undefined })).toBe(4);
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: 0 })).toBe(4);
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: null })).toBe(4);
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: -1 })).toBe(4);
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: 'invalid' })).toBe(4);
            });
            it('should handle valid concurrency values', () => {
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: 1 })).toBe(1);
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: 10 })).toBe(10);
                expect((0, multilingual_1.getConcurrencyLimit)({ maxConcurrency: '5' })).toBe(5);
            });
        });
        it('should handle partial local translation results', async () => {
            ...
            jest.mocked(shared_1.redteamProviderManager.getProvider).mockResolvedValue(mockProvider);
            const result = await (0, multilingual_1.translateBatch)('Hello', ['es', 'fr'], 2);
            expect(result).toEqual({ es: 'Hola', fr: 'Bonjour' });
        ...
        it('should escape language codes in regex fallback', async () => {
            ...
            jest.mocked(shared_1.redteamProviderManager.getProvider).mockResolvedValue(mockProvider);
            const result = await (0, multilingual_1.translateBatch)('Hello', ['zh-CN', 'en-US']);
        ...
        it('should handle local partial fallback and deduplication', async () => {
            ...
            jest.mocked(shared_1.redteamProviderManager.getProvider).mockResolvedValue(mockProvider);
            const testCases = [{ vars: { prompt: 'Hello' }, metadata: {} }];
            const result = await (0, multilingual_1.addMultilingual)(testCases, 'prompt', {
                languages: ['es', 'fr'],
        ...
        it('should handle improved missing detection logic', async () => {
            ...
            jest.mocked(shared_1.redteamProviderManager.getProvider).mockResolvedValue(mockProvider);
            const testCases = [{ vars: { prompt: 'Hello world' }, metadata: {} }];
            const result = await (0, multilingual_1.addMultilingual)(testCases, 'prompt', {
                languages: ['es', 'fr'],
