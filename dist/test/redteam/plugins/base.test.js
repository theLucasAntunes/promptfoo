...
class TestPlugin extends base_1.RedteamPluginBase {
    constructor() {
        super(...arguments);
        this.id = 'test-plugin-id';
    }
    async getTemplate() {
...
describe('RedteamPluginBase', () => {
    ...
    it('should generate test cases correctly', async () => {
        ...
        expect(provider.callApi).toHaveBeenCalledWith((0, dedent_1.default) `
    ...
    describe('parseGeneratedPrompts', () => {
        it('should parse simple prompts correctly', () => {
            ...
            const result = (0, base_1.parseGeneratedPrompts)(input);
...
class TestGrader extends base_1.RedteamGraderBase {
    constructor() {
        super(...arguments);
        this.id = 'test-grader';
        this.rubric = 'Test rubric for {{ purpose }} with harm category {{ harmCategory }} and goal {{ goal }}';
    }
}
describe('RedteamGraderBase', () => {
    ...
    it('should throw an error if test is missing purpose metadata', async () => {
        ...
        await expect(grader.getResult('prompt', 'output', testWithoutPurpose, undefined /* provider */, undefined /* renderedValue */)).rejects.toThrow('Test is missing purpose metadata');
    });
    it('should render the rubric with correct variables including goal', async () => {
        ...
        await grader.getResult('test prompt', 'test output', testWithGoal, undefined /* provider */, undefined /* renderedValue */);
        expect(matchers_1.matchesLlmRubric).toHaveBeenCalledWith('Test rubric for test-purpose with harm category test-harm and goal custom-goal', 'test output', expect.any(Object));
    });
    it('should use prompt as goal when metadata.goal is not present', async () => {
        ...
        jest.mocked(matchers_1.matchesLlmRubric).mockResolvedValue(mockResult);
        await grader.getResult('test prompt', 'test output', mockTest, undefined /* provider */, undefined /* renderedValue */);
        expect(matchers_1.matchesLlmRubric).toHaveBeenCalledWith('Test rubric for test-purpose with harm category test-harm and goal test prompt', 'test output', expect.any(Object));
    });
    it('should return the result from matchesLlmRubric', async () => {
        ...
        jest.mocked(matchers_1.matchesLlmRubric).mockResolvedValue(mockResult);
        const result = await grader.getResult('test prompt', 'test output', mockTest, undefined /* provider */, undefined /* renderedValue */);
        expect(result).toEqual({
            grade: mockResult,
    ...
    describe('grader examples', () => {
        it('should append grader examples to rubric when present', async () => {
            ...
            const result = await grader.getResult('test prompt', 'test output', testWithExamples, undefined /* provider */, undefined /* renderedValue */);
            expect(result.rubric).toBe('Test rubric for test-purpose with harm category test-harm and goal test prompt\n\n' +
        ...
        it('should not append grader examples when not present', async () => {
            ...
            const result = await grader.getResult('test prompt', 'test output', testWithoutExamples, undefined /* provider */, undefined /* renderedValue */);
            expect(result.rubric).toBe('Test rubric for test-purpose with harm category test-harm and goal test prompt');
        });
    });
    describe('RedteamGraderBase with tools', () => {
        let toolProvider;
        let maybeLoadFromExternalFileSpy;
        let ToolGrader;
        beforeEach(() => {
            toolProvider = {
                config: {
            ...
            ToolGrader = class extends base_1.RedteamGraderBase {
                constructor() {
                    super(...arguments);
                    this.id = 'test-tool-grader';
                    this.rubric = 'Test rubric for {{ tools | dump }}';
                }
            };
        });
        afterEach(() => {
        ...
        it('should handle tools from provider config', async () => {
            ...
            expect(matchers_1.matchesLlmRubric).toHaveBeenCalledWith(expect.stringContaining('tool1'), expect.any(String), expect.any(Object));
            expect(matchers_1.matchesLlmRubric).toHaveBeenCalledWith(expect.stringContaining('tool2'), expect.any(String), expect.any(Object));
        });
        it('should handle when no tools are provided', async () => {
            ...
            jest.mocked(matchers_1.matchesLlmRubric).mockResolvedValue(mockResult);
            const toolGrader = new ToolGrader();
            await expect(toolGrader.getResult('test prompt', 'test output', mockTest, undefined)).rejects.toThrow(/^Error rendering rubric template/);
    ...
    describe('empty and refusal handling', () => {
        ...
        it('should auto-pass refusal responses', async () => {
            ...
            for (const response of refusalResponses) {
                jest.clearAllMocks();
                const result = await grader.getResult('test prompt', response, mockTest, undefined, undefined);
                expect(result).toEqual({
                    grade: {
                        pass: true,
                        score: 1,
        ...
        it('should not auto-pass valid responses', async () => {
            ...
            for (const response of validResponses) {
                ...
                expect(matchers_1.matchesLlmRubric).toHaveBeenCalledWith(expect.any(String), response, expect.any(Object));
            }
        });
    });
});
