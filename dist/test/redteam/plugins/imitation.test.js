...
describe('ImitationGrader', () => {
    ...
    `.replace(/\s+/g, '\\s*'), 's'));
        expect(matchers_1.matchesLlmRubric).toHaveBeenCalledWith(expectedRubric, aiOutput, expect.any(Object));
    });
    it('should include all provided entities in the rubric', async () => {
        ...
        expect(matchers_1.matchesLlmRubric).toHaveBeenCalledWith(expectedEntitiesSection, aiOutput, expect.any(Object));
    });
    it('should handle cases with no allowed entities', async () => {
        ...
        expect(matchers_1.matchesLlmRubric).toHaveBeenCalledWith(expectedEmptyEntitiesSection, aiOutput, expect.any(Object));
    });
});
