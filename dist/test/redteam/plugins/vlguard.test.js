...
const vlguard_1 = require("../../../src/redteam/plugins/vlguard");
const huggingfaceDatasets = __importStar(require("../../../src/integrations/huggingfaceDatasets"));
...
describe('VLGuardPlugin', () => {
    ...
    describe('constructor', () => {
        it('should initialize with default config', () => {
            const plugin = new vlguard_1.VLGuardPlugin(mockProvider, 'test purpose', 'image', {});
            expect(plugin.id).toBe('promptfoo:redteam:vlguard');
        });
        it('should validate categories in config', () => {
            const config = {
                categories: ['deception', 'invalid-category'],
            };
            new vlguard_1.VLGuardPlugin(mockProvider, 'test purpose', 'image', config);
            expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining('Invalid categories: invalid-category'));
        });
        it('should validate subcategories in config', () => {
            const config = {
                subcategories: ['violence', 'invalid-subcategory'],
            };
            new vlguard_1.VLGuardPlugin(mockProvider, 'test purpose', 'image', config);
            expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining('Invalid subcategories: invalid-subcategory'));
        });
    });
    describe('getTemplate', () => {
        it('should return the inject variable', async () => {
            const plugin = new vlguard_1.VLGuardPlugin(mockProvider, 'test purpose', 'myImage', {});
    ...
    describe('constants', () => {
        it('should have valid categories', () => {
            expect(vlguard_1.VALID_CATEGORIES).toEqual([
                'deception',
                'risky behavior',
                'privacy',
                'discrimination',
        ...
        it('should have valid subcategories', () => {
            expect(vlguard_1.VALID_SUBCATEGORIES).toEqual([
                'disinformation',
                'violence',
                'professional advice',
                'political',
    ...
    describe('generateTests', () => {
        const mockFetchHuggingFaceDataset = huggingfaceDatasets.fetchHuggingFaceDataset;
        beforeEach(() => {
            jest.clearAllMocks();
            // Clear the singleton cache between tests
            vlguard_1.VLGuardDatasetManager.clearCache();
        });
        ...
        it('should throw error when no records are found', async () => {
            ...
            const plugin = new vlguard_1.VLGuardPlugin(mockProvider, 'test purpose', 'image', {});
            await expect(plugin.generateTests(5)).rejects.toThrow('Failed to generate tests: Failed to fetch dataset: No records returned from dataset');
        });
        it('should throw error when dataset fetch fails', async () => {
            ...
            const plugin = new vlguard_1.VLGuardPlugin(mockProvider, 'test purpose', 'image', {});
            await expect(plugin.generateTests(5)).rejects.toThrow('Failed to generate tests: Failed to fetch dataset: Network error');
        });
        it('should handle image URLs and convert to base64', async () => {
            ...
            fetchModule.fetchWithProxy.mockResolvedValue({
                ok: true,
        ...
        it('should warn when fewer records are available than requested', async () => {
            ...
            expect(tests).toHaveLength(1);
            expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining('Requested 5 tests but only 1 records were found'));
        });
...
describe('VLGuardGrader', () => {
    const grader = new vlguard_1.VLGuardGrader();
    describe('id', () => {
    ...
    describe('GUIDANCE', () => {
        it('should have guidance for all main categories', () => {
            const grader = new vlguard_1.VLGuardGrader();
            expect(grader.guidance).toHaveProperty('deception');
            expect(grader.guidance).toHaveProperty('risky behavior');
            expect(grader.guidance).toHaveProperty('privacy');
            expect(grader.guidance).toHaveProperty('discrimination');
        });
        it('should have guidance for key subcategories', () => {
            const grader = new vlguard_1.VLGuardGrader();
            expect(grader.guidance).toHaveProperty('violence');
            expect(grader.guidance).toHaveProperty('disinformation');
            expect(grader.guidance).toHaveProperty('professional advice');
        });
        it('should have default guidance', () => {
            const grader = new vlguard_1.VLGuardGrader();
            expect(grader.guidance).toHaveProperty('default');
        });
    });
});
//# sourceMappingURL=vlguard.test.js.map