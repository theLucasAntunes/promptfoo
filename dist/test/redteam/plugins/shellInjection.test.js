...
describe('ShellInjectionPlugin', () => {
    ...
    it('should generate template with custom examples', async () => {
        ...
        const plugin = new shellInjection_1.ShellInjectionPlugin(mockProvider, 'test purpose', 'test_var', customConfig);
        const template = await plugin['getTemplate']();
    ...
    it('should handle empty examples config', async () => {
        const emptyConfig = { examples: [] };
        const plugin = new shellInjection_1.ShellInjectionPlugin(mockProvider, 'test purpose', 'test_var', emptyConfig);
        const template = await plugin['getTemplate']();
        expect(template).toContain('Examples targeting security boundary violations:');
        expect(template).toContain(shellInjection_1.DEFAULT_EXAMPLES);
    });
    it('should handle undefined examples config', async () => {
        const plugin = new shellInjection_1.ShellInjectionPlugin(mockProvider, 'test purpose', 'test_var', undefined);
        const template = await plugin['getTemplate']();
        expect(template).toContain('Examples targeting security boundary violations:');
        expect(template).toContain(shellInjection_1.DEFAULT_EXAMPLES);
    });
    it('should handle malformed examples config', async () => {
        ...
        const plugin = new shellInjection_1.ShellInjectionPlugin(mockProvider, 'test purpose', 'test_var', malformedConfig);
        const template = await plugin['getTemplate']();
        expect(template).toContain('Examples targeting security boundary violations:');
        expect(template).toContain(shellInjection_1.DEFAULT_EXAMPLES);
    });
    it('should expose DEFAULT_EXAMPLES as a string containing all new examples', () => {
        expect(shellInjection_1.DEFAULT_EXAMPLES).toContain('AI code review assistant for development teams');
        expect(shellInjection_1.DEFAULT_EXAMPLES).toContain('developer productivity chatbot');
        expect(shellInjection_1.DEFAULT_EXAMPLES).toContain('CI/CD pipeline assistant');
        expect(shellInjection_1.DEFAULT_EXAMPLES).toContain('compliance audit assistant');
        expect(shellInjection_1.DEFAULT_EXAMPLES).toContain('document management chatbot');
        expect(shellInjection_1.DEFAULT_EXAMPLES).toContain('internal RAG log assistant chatbot');
        expect(shellInjection_1.DEFAULT_EXAMPLES).toContain('QA automation assistant');
        expect(shellInjection_1.DEFAULT_EXAMPLES).toContain('knowledge base assistant for technical documentation');
    });
});
describe('ShellInjectionGrader', () => {
    ...
    it('should include key evaluation criteria in rubric', () => {
        ...
        expect(grader.rubric).toContain('Generic error messages that do not indicate command execution');
        expect(grader.rubric).toContain('Example code snippets not from result of executing the command');
    });
