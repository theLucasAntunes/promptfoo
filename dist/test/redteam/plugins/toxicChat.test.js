...
describe('ToxicChatPlugin', () => {
    let plugin;
    beforeEach(() => {
        plugin = new toxicChat_1.ToxicChatPlugin({ id: 'test-provider' }, 'Test purpose', 'prompt');
    });
    it('should set canGenerateRemote to false', () => {
        expect(toxicChat_1.ToxicChatPlugin.canGenerateRemote).toBe(false);
    });
    ...
    it('should generate tests correctly', async () => {
        ...
        jest.mocked(huggingfaceDatasets_1.fetchHuggingFaceDataset).mockResolvedValue(mockData);
...
describe('ToxicChatGrader', () => {
    let grader;
    beforeEach(() => {
        grader = new toxicChat_1.ToxicChatGrader();
    });
...
describe('fetchDataset', () => {
    ...
    it('should fetch and filter toxic prompts', async () => {
        ...
        jest.mocked(huggingfaceDatasets_1.fetchHuggingFaceDataset).mockResolvedValue(mock);
        const result = await (0, toxicChat_1.fetchDataset)(3);
    ...
    it('should handle missing user_input field', async () => {
        ...
        jest.mocked(huggingfaceDatasets_1.fetchHuggingFaceDataset).mockResolvedValue(mock);
        const result = await (0, toxicChat_1.fetchDataset)(2);
    ...
    it('should filter out safe prompts', async () => {
        ...
        jest.mocked(huggingfaceDatasets_1.fetchHuggingFaceDataset).mockResolvedValue(mock);
        const result = await (0, toxicChat_1.fetchDataset)(5);
        expect(result).toHaveLength(1);
        expect(result[0]?.vars?.prompt).toBe('toxic prompt');
    });
    ...
    it('should handle fetch errors gracefully', async () => {
        jest.mocked(huggingfaceDatasets_1.fetchHuggingFaceDataset).mockRejectedValue(new Error('Network error'));
        const result = await (0, toxicChat_1.fetchDataset)(3);
        expect(result).toEqual([]);
    });
    it('should call fetchHuggingFaceDataset with correct parameters', async () => {
        const mock = [{ vars: { user_input: 'test', toxicity: 1, jailbreaking: 0 } }];
        jest.mocked(huggingfaceDatasets_1.fetchHuggingFaceDataset).mockResolvedValue(mock);
        await (0, toxicChat_1.fetchDataset)(5);
        expect(huggingfaceDatasets_1.fetchHuggingFaceDataset).toHaveBeenCalledWith('huggingface://datasets/lmsys/toxic-chat?config=toxicchat0124&split=train', 25);
    });
});
//# sourceMappingURL=toxicChat.test.js.map