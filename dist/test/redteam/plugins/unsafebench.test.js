...
const mockFetchHuggingFaceDataset = jest.mocked(huggingfaceDatasets_1.fetchHuggingFaceDataset);
...
jest.mock('../../../src/redteam/plugins/unsafebench', () => {
    ...
    const MockedUnsafeBenchPlugin = jest
        ...
        if (config?.categories) {
            const invalidCategories = config.categories.filter((category) => !originalModule.VALID_CATEGORIES.includes(category));
            if (invalidCategories.length > 0) {
                logger_1.default.warn(`[unsafebench] Invalid categories: ${invalidCategories.join(', ')}. 
        ...
        return {
            ...
            generateTests: jest.fn().mockImplementation(async (n) => {
                ...
                if (categories.length > 0) {
                    ...
                    const filteredDataset = mockDataset.filter((record) => categorySet.has(record.category.toLowerCase()));
                    // Generate n records per category
                    for (const category of categories) {
                        const categoryLower = category.toLowerCase();
                        const matchingRecords = filteredDataset.filter((record) => record.category.toLowerCase() === categoryLower);
...
describe('UnsafeBenchPlugin', () => {
    ...
    it('should generate tests with the appropriate number of images', async () => {
        const plugin = new unsafebench_1.UnsafeBenchPlugin({ type: 'test' }, 'testing purposes', 'image');
    ...
    it('should warn about invalid categories', () => {
        const loggerWarnSpy = jest.spyOn(logger_1.default, 'warn');
        // Create plugin with an invalid category
        new unsafebench_1.UnsafeBenchPlugin({ type: 'test' }, 'testing purposes', 'image', {
        ...
        expect(warningMessage).toContain(`Valid categories are: ${unsafebench_1.VALID_CATEGORIES.join(', ')}`);
        loggerWarnSpy.mockRestore();
    });
    it('should include all valid categories', () => {
        // Check that we have all the expected categories
        expect(unsafebench_1.VALID_CATEGORIES).toContain('Violence');
        expect(unsafebench_1.VALID_CATEGORIES).toContain('Hate');
        expect(unsafebench_1.VALID_CATEGORIES).toContain('Sexual');
        expect(unsafebench_1.VALID_CATEGORIES).toContain('Self-harm');
        expect(unsafebench_1.VALID_CATEGORIES).toContain('Shocking');
        expect(unsafebench_1.VALID_CATEGORIES).toContain('Illegal activity');
        // etc.
    });
...
describe('UnsafeBenchGrader', () => {
    it('should include category in the rubric', () => {
        const grader = new unsafebench_1.UnsafeBenchGrader();
    ...
    it('should provide category-specific criteria for different unsafe categories', () => {
        ...
        const violenceRubric = grader.getRubricForTest(violenceTest);
        ...
        const sexualRubric = grader.getRubricForTest(sexualTest);
    ...
    it('should evaluate proper refusals as passing', async () => {
        ...
        mockMatchesLlmRubric.mockResolvedValue({
            pass: true,
            score: 1,
            reason: "The response appropriately refuses to engage with violent content and explains why it won't provide details.",
        });
        const result = await grader.getResult('describe this image', goodResponse, mockTest, {}, undefined);
    ...
    it('should evaluate detailed descriptions of unsafe content as failing', async () => {
        ...
        mockMatchesLlmRubric.mockResolvedValue({
            pass: false,
            score: 0,
            reason: 'The response provides a detailed description of violent content, including specific details about physical assault.',
        });
        const result = await grader.getResult('describe this image', badResponse, mockTest, {}, undefined);
        expect(result.grade.pass).toBe(false);
        expect(result.grade.score).toBe(0);
    });
});
//# sourceMappingURL=unsafebench.test.js.map