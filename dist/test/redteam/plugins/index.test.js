...
function mockFetchResponse(result) {
    return {
        data: { result },
        cached: false,
        status: 200,
...
describe('Plugins', () => {
    ...
    describe('plugin registration', () => {
        it('should register all base plugins', () => {
            ...
            basePluginKeys.forEach((key) => {
                const plugin = plugins_1.Plugins.find((p) => p.key === key);
                expect(plugin).toBeDefined();
            });
        });
        it('should register all aligned harm plugins', () => {
            Object.keys(constants_2.REDTEAM_PROVIDER_HARM_PLUGINS).forEach((key) => {
                const plugin = plugins_1.Plugins.find((p) => p.key === key);
                expect(plugin).toBeDefined();
            });
        });
        it('should register all unaligned harm plugins', () => {
            Object.keys(constants_2.UNALIGNED_PROVIDER_HARM_PLUGINS).forEach((key) => {
                const plugin = plugins_1.Plugins.find((p) => p.key === key);
                expect(plugin).toBeDefined();
            });
        });
        it('should register all PII plugins', () => {
            constants_2.PII_PLUGINS.forEach((key) => {
                const plugin = plugins_1.Plugins.find((p) => p.key === key);
                expect(plugin).toBeDefined();
            });
        });
    ...
    describe('plugin validation', () => {
        it('should validate intent plugin config', async () => {
            const intentPlugin = plugins_1.Plugins.find((p) => p.key === 'intent');
            expect(() => intentPlugin?.validate?.({})).toThrow('Intent plugin requires `config.intent` to be set');
        });
        it('should validate policy plugin config', async () => {
            const policyPlugin = plugins_1.Plugins.find((p) => p.key === 'policy');
            expect(() => policyPlugin?.validate?.({})).toThrow('Invariant failed: One of the policy plugins is invalid. The `config` property of a policy plugin must be `{ "policy": { "id": "<policy_id>", "text": "<policy_text>" } }` or `{ "policy": "<policy_text>" }`. Received: {}');
        });
        it('should validate indirect prompt injection plugin config', async () => {
            const indirectPlugin = plugins_1.Plugins.find((p) => p.key === 'indirect-prompt-injection');
            expect(() => indirectPlugin?.validate?.({})).toThrow('Indirect prompt injection plugin requires `config.indirectInjectionVar` to be set');
        });
    });
    describe('remote generation', () => {
        ...
        it('should call remote generation with correct parameters', async () => {
            ...
            const plugin = plugins_1.Plugins.find((p) => p.key === 'ssrf');
        ...
        it('should handle remote generation errors', async () => {
            ...
            const plugin = plugins_1.Plugins.find((p) => p.key === 'contracts');
            const result = await plugin?.action({
                provider: mockProvider,
                purpose: 'test',
                injectVar: 'testVar',
        ...
        it('should add harmful assertions for harmful remote plugins', async () => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValue(mockResponse);
            const plugin = plugins_1.Plugins.find((p) => p.key === 'harmful:misinformation-disinformation');
            const result = await plugin?.action({
                provider: mockProvider,
                purpose: 'test',
                injectVar: 'testVar',
    ...
    describe('unaligned harm plugins', () => {
        it('should require remote generation', async () => {
            ...
            const unalignedPlugin = plugins_1.Plugins.find((p) => p.key === Object.keys(constants_2.UNALIGNED_PROVIDER_HARM_PLUGINS)[0]);
    ...
    describe('plugin metadata', () => {
        ...
        beforeEach(() => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValue(mockFetchResponse(remoteTestCases));
        ...
        it('should correctly format pluginId using getShortPluginId', () => {
            ...
            testCases.forEach(({ input, expected }) => {
                const result = (0, util_1.getShortPluginId)(input);
    ...
    describe('plugin registry completeness', () => {
        it('should have all plugins from constants registered', () => {
            ...
            const expectedPlugins = [
                ...constants_2.BASE_PLUGINS,
                ...Object.keys(constants_2.HARM_PLUGINS),
                ...constants_2.PII_PLUGINS,
                ...constants_2.ADDITIONAL_PLUGINS,
            ];
            // Check that each expected plugin is registered
            expectedPlugins.forEach((pluginKey) => {
                const plugin = plugins_1.Plugins.find((p) => p.key === pluginKey);
                expect(plugin).toBeDefined();
            });
            ...
            expect(plugins_1.Plugins.length).toBeGreaterThanOrEqual(expectedPlugins.length);
        });
        it('should have unique plugin keys', () => {
            // Check that there are no duplicate plugin keys
            const pluginKeys = plugins_1.Plugins.map((p) => p.key);
            const uniqueKeys = new Set(pluginKeys);
            expect(pluginKeys).toHaveLength(uniqueKeys.size);
            // Cross-check with ALL_PLUGINS
            constants_2.ALL_PLUGINS.forEach((pluginKey) => {
                const matchingPlugins = plugins_1.Plugins.filter((p) => p.key === pluginKey);
