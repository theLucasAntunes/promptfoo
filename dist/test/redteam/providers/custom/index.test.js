...
describe('MemorySystem', () => {
    let memorySystem;
    beforeEach(() => {
        memorySystem = new custom_1.MemorySystem();
    });
...
describe('CustomProvider', () => {
    ...
    beforeEach(() => {
        ...
        customProvider = new custom_1.CustomProvider({
            injectVar: 'objective',
    ...
    it('should initialize with default config values', () => {
        const provider = new custom_1.CustomProvider({
            injectVar: 'objective',
    ...
    it('should use default values when optional config not provided', () => {
        ...
        expect(provider.maxTurns).toBe(10); // DEFAULT_MAX_TURNS
        expect(provider.maxBacktracks).toBe(10); // DEFAULT_MAX_BACKTRACKS
        expect(provider.stateful).toBe(false); // Default false
    ...
    it('should succeed via internal evaluator when eval score is 100', async () => {
        ...
        const testProvider = new custom_1.CustomProvider({
            injectVar: 'objective',
    ...
    it('should handle stateful mode with sessionId', async () => {
        ...
        expect(context.vars.sessionId).toBe('test-session-123');
    });
    it('should pass additionalRubric to grader when goal is provided', async () => {
        ...
        jest.mocked(graders_1.getGraderById).mockReturnValue({
            getResult: jest.fn(async (prompt, output, test, provider, value, additionalRubric) => {
    ...
    it('should include modifiers in system prompt from test metadata', async () => {
        ...
        mockRedTeamProvider.callApi.mockImplementation(async (prompt) => {
        ...
        expect(capturedSystemPrompt).toContain('testGenerationInstructions: Generate prompts as realistic invoice queries');
