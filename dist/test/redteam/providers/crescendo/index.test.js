...
describe('MemorySystem', () => {
    let memorySystem;
    beforeEach(() => {
        memorySystem = new crescendo_1.MemorySystem();
    });
...
describe('CrescendoProvider', () => {
    let crescendoProvider;
    let mockRedTeamProvider;
    let mockScoringProvider;
    ...
    beforeEach(() => {
        ...
        crescendoProvider = new crescendo_1.CrescendoProvider({
            injectVar: 'objective',
            maxTurns: 10,
            maxBacktracks: 10,
        ...
        jest.spyOn(shared_1.redteamProviderManager, 'getProvider').mockImplementation(async (options) => {
        ...
        jest.mocked(server_1.checkServerFeatureSupport).mockResolvedValue(true);
        // Set up default getGraderById mock
        jest.mocked(graders_1.getGraderById).mockReset();
        jest.mocked(graders_1.getGraderById).mockReturnValue({
        ...
        jest.mocked(shared_1.tryUnblocking).mockReset();
        jest.mocked(shared_1.tryUnblocking).mockResolvedValue({ success: false });
    });
    afterEach(() => {
        jest.restoreAllMocks();
    });
    it('should initialize with default config values', () => {
        const provider = new crescendo_1.CrescendoProvider({
            injectVar: 'objective',
            maxTurns: 10,
            maxBacktracks: 10,
            redteamProvider: mockRedTeamProvider,
    ...
    describe('Unblocking functionality', () => {
        it('should detect blocking question and send unblocking response', async () => {
            ...
            jest.mocked(shared_1.tryUnblocking).mockResolvedValue({
                success: true,
            ...
            expect(shared_1.tryUnblocking).toHaveBeenCalledWith({
    ...
    it('should succeed via internal evaluator when eval score is 100', async () => {
        ...
        const testProvider = new crescendo_1.CrescendoProvider({
            injectVar: 'objective',
    ...
    it('should handle purpose from test metadata', async () => {
        ...
        expect(mockRedTeamProvider.callApi).toHaveBeenCalledWith(expect.stringContaining('test purpose'), expect.any(Object));
    });
    it('should pass purpose parameter to getAttackPrompt', async () => {
        ...
        await provider.callApi('test prompt', context);
        expect(mockRedTeamProvider.callApi).toHaveBeenCalledWith(expect.stringContaining('test purpose for attack'), expect.any(Object));
    });
...
describe('CrescendoProvider - Chat Template Support', () => {
  ...
  },
  {
    ...
    "content": "{{ user_input }}"
        ...
        jest.mocked(evaluatorHelpers.renderPrompt).mockResolvedValueOnce(JSON.stringify([
            { role: 'system', content: 'You are a helpful assistant' },
            { role: 'user', content: 'test attack' },
        ]));
        // Mock target provider to verify it receives structured JSON, not stringified conversation
        mockTargetProvider.callApi.mockImplementation((prompt) => {
    ...
    it('should always use rendered prompt when stateful=true regardless of template type', async () => {
        const statefulProvider = new crescendo_1.CrescendoProvider({
            injectVar: 'user_input',
            maxTurns: 1,
            redteamProvider: mockRedTeamProvider,
