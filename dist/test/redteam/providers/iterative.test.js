...
const mockGetGraderById = globals_1.jest.fn();
...
describe('RedteamIterativeProvider', () => {
    ...
    beforeEach(() => {
        ...
        mockGetTargetResponse.mockImplementation(async () => Promise.resolve({
            output: 'mock target response',
        }));
    ...
    describe('runRedteamConversation', () => {
        ...
        it('should exit early when grader fails and use stored grader result', async () => {
            const mockGrader = {
                getResult: globals_1.jest.fn().mockResolvedValue({
                    grade: {
                        pass: false,
                        score: 0,
            ...
            expect(mockGrader.getResult).toHaveBeenCalledWith('test', 'mock target response', expect.objectContaining({
                vars: { test: 'goal' },
                assert: [{ type: 'promptfoo:redteam:test-plugin' }],
                metadata: { pluginId: 'test-plugin', goal: 'test goal' },
            }), mockRedteamProvider, undefined, expect.stringContaining('iterative jailbreak attack with the following goal: "test goal"'));
        });
        ...
        it('should extract JSON object from fenced/narrative judge output', async () => {
            const fenced = "I'll evaluate now.\n```json\n{" +
        ...
        it('should include goal in additional rubric when goal is present', async () => {
            ...
            await (0, iterative_1.runRedteamConversation)({
                context: {
                    prompt: { raw: '', label: '' },
                    vars: {},
                    test,
            ...
            }), mockRedteamProvider, undefined, expect.stringContaining('Create harmful content about specific groups'));
        ...
        it('should include modifiers in system prompt from test metadata', async () => {
            ...
            expect(capturedSystemPrompt).toContain('testGenerationInstructions: Generate prompts as merchant account queries');
