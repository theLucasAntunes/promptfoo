...
const index_2 = require("../../src/redteam/index");
...
describe('synthesize', () => {
    ...
    beforeEach(() => {
        ...
        jest.resetAllMocks();
        // Set up logger mocks
        jest.mocked(logger_1.default.info).mockReturnValue(logger_1.default);
        jest.mocked(logger_1.default.warn).mockReturnValue(logger_1.default);
        jest.mocked(logger_1.default.error).mockReturnValue(logger_1.default);
        jest.mocked(logger_1.default.debug).mockReturnValue(logger_1.default);
        // Set up templates mock with consistent default behavior
        jest.mocked(templates_1.extractVariablesFromTemplates).mockReturnValue(['query']);
        ...
        jest.mocked(strategies_1.validateStrategies).mockImplementation(async () => { });
    ...
    describe('Input handling', () => {
        it('should use provided purpose and entities if given', async () => {
            const result = await (0, index_2.synthesize)({
        ...
        it('should handle empty prompts array', async () => {
            await expect((0, index_2.synthesize)({
                language: 'en',
                numTests: 1,
                plugins: [{ id: 'test-plugin', numTests: 1 }],
                prompts: [],
    ...
    describe('API provider', () => {
        it('should use the provided API provider if given', async () => {
            ...
            expect(index_1.loadApiProvider).not.toHaveBeenCalled();
        });
    });
    ...
    describe('Plugins and strategies', () => {
        ...
        it('should use default fan-out values when strategy config omits n', async () => {
            ...
            const strategyAction = jest
                .fn()
                .mockImplementation(async () => Array.from({ length: 5 }, (_, idx) => ({ vars: { query: `fanout-${idx}` } })));
            const strategyFindSpy = jest.spyOn(strategies_1.Strategies, 'find').mockImplementation((predicate) => {
            ...
            try {
                ...
                const reportMessage = jest
                    .mocked(logger_1.default.info)
                    .mock.calls.map(([arg]) => arg)
                    .find((arg) => typeof arg === 'string' && arg.includes('Test Generation Report'));
                expect(reportMessage).toBeDefined();
                const stripped = (0, utils_1.stripAnsi)(reportMessage);
                ...
                expect(columns[requestedIndex]).toBe((0, constants_1.getDefaultNFanout)('jailbreak:composite').toString());
                // Generated = what our mock strategy returned
                expect(columns[generatedIndex]).toBe((0, constants_1.getDefaultNFanout)('jailbreak:composite').toString());
                // Verify the "Using strategies:" summary shows correct fan-out
                const summaryMessage = jest
                    .mocked(logger_1.default.info)
                    .mock.calls.map(([arg]) => arg)
                    .find((arg) => typeof arg === 'string' && arg.includes('Using strategies:'));
                expect(summaryMessage).toBeDefined();
                const summaryStripped = (0, utils_1.stripAnsi)(summaryMessage);
                const summaryLine = summaryStripped
                    .split('\n')
                    .find((line) => line.includes('jailbreak:composite'));
                expect(summaryLine).toBeDefined();
                expect(summaryLine).toContain(`(${(0, constants_1.getDefaultNFanout)('jailbreak:composite')} additional tests)`);
                // Verify the "Test Generation Summary:" shows correct total
                const totalSummaryMessage = jest
                    .mocked(logger_1.default.info)
                    .mock.calls.map(([arg]) => arg)
                    .find((arg) => typeof arg === 'string' && arg.includes('Test Generation Summary:'));
                expect(totalSummaryMessage).toBeDefined();
                const totalStripped = (0, utils_1.stripAnsi)(totalSummaryMessage);
                // 1 base + default fan-out
                expect(totalStripped).toContain(`• Total tests: ${1 + (0, constants_1.getDefaultNFanout)('jailbreak:composite')}`);
            }
            finally {
        ...
        it('should expand strategy collections into individual strategies', async () => {
            ...
            expect(strategies_1.validateStrategies).toHaveBeenCalledWith(expect.any(Array));
        });
        it('should deduplicate strategies with the same ID', async () => {
            ...
            const validateStrategiesSpy = jest.mocked(strategies_1.validateStrategies);
        ...
        it('should handle missing strategy collections gracefully', async () => {
            ...
            expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining('unknown-collection'));
        });
        it('should find exact strategy IDs like jailbreak:composite', async () => {
            ...
            jest.spyOn(strategies_1.Strategies, 'find').mockImplementation((predicate) => {
                if (typeof predicate === 'function') {
                    const strategies = [
                        {
                            id: 'jailbreak',
            ...
            expect(strategyTestCases).toEqual(expect.arrayContaining([
        ...
        it('should warn when strategy is not found', async () => {
            ...
            jest.spyOn(strategies_1.Strategies, 'find').mockReturnValue(undefined);
            await (0, index_2.synthesize)({
                language: 'en',
                numTests: 1,
                plugins: [{ id: 'test-plugin', numTests: 1 }],
            ...
            expect(logger_1.default.warn).toHaveBeenCalledWith('Strategy nonexistent-strategy not registered, skipping');
        });
        ...
        it('should not store full config in metadata for intent plugin to prevent bloating', async () => {
            ...
            jest.spyOn(plugins_1.Plugins, 'find').mockImplementation((predicate) => {
                const mockPlugins = [
            ...
            const contractsTestCases = result.testCases.filter((tc) => tc.metadata?.pluginId === 'contracts');
    ...
    it('should handle basic strategy configuration', async () => {
        jest.mocked(index_1.loadApiProvider).mockResolvedValue({
            id: () => 'test',
        ...
        jest.spyOn(plugins_1.Plugins, 'find').mockReturnValue({
            key: 'test-plugin',
            action: mockTestPluginAction,
        });
        const resultEnabled = await (0, index_2.synthesize)({
            plugins: [mockPlugin],
        ...
        const resultDisabled = await (0, index_2.synthesize)({
            plugins: [mockPlugin],
...
describe('resolvePluginConfig', () => {
    ...
    it('should return an empty object if config is undefined', () => {
        const result = (0, index_2.resolvePluginConfig)(undefined);
        expect(result).toEqual({});
    });
    it('should return the original config if no file references are present', () => {
        const config = { key: 'value' };
        const result = (0, index_2.resolvePluginConfig)(config);
    ...
    it('should throw an error if the file does not exist', () => {
        ...
        expect(() => (0, index_2.resolvePluginConfig)(config)).toThrow('File not found: nonexistent.yaml');
    });
...
describe('calculateTotalTests', () => {
    ...
    it('should calculate basic test counts with no strategies', () => {
        const result = (0, index_2.calculateTotalTests)(mockPlugins, []);
        expect(result).toEqual({
            totalTests: 5,
            totalPluginTests: 5,
    ...
    it('should handle basic strategy when enabled', () => {
        ...
        const result = (0, index_2.calculateTotalTests)(mockPlugins, strategies);
        expect(result).toEqual({
            totalTests: 5,
            totalPluginTests: 5,
    ...
    it('should correctly calculate total tests for multiple plugins with jailbreak strategy', () => {
        ...
        const result = (0, index_2.calculateTotalTests)(plugins, strategies);
        expect(result).toEqual({
            totalTests: 100,
            totalPluginTests: 50,
            effectiveStrategyCount: 1,
...
describe('getMultilingualRequestedCount', () => {
    ...
    it('should calculate count with custom languages array', () => {
        ...
        const count = (0, index_2.getMultilingualRequestedCount)(testCases, strategy);
        expect(count).toBe(6);
    });
    ...
    it('should handle empty test cases', () => {
        ...
        const count = (0, index_2.getMultilingualRequestedCount)([], strategy);
        expect(count).toBe(0);
    });
});
describe('getTestCount', () => {
    it('should return totalPluginTests when basic strategy is enabled', () => {
        ...
        const result = (0, index_2.getTestCount)(strategy, 10, []);
