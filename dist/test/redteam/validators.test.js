...
describe('RedteamPluginObjectSchema', () => {
    it('should validate valid plugin object', () => {
        ...
        const result = redteam_1.RedteamPluginObjectSchema.safeParse(validPlugin);
    ...
    it('should reject invalid plugin id', () => {
        ...
        const result = redteam_1.RedteamPluginObjectSchema.safeParse(invalidPlugin);
    ...
    it('should allow custom plugin paths starting with file://', () => {
        ...
        const result = redteam_1.RedteamPluginObjectSchema.safeParse(customPlugin);
        expect(result.success).toBe(true);
    });
});
describe('redteamGenerateOptionsSchema', () => {
    ...
    it('should validate basic generate options', () => {
        ...
        const result = redteam_1.RedteamGenerateOptionsSchema.safeParse(options);
        expect(result.success).toBe(true);
    });
...
describe('redteamPluginSchema', () => {
    ...
    it('should reject an invalid plugin name', () => {
        expect(redteam_1.RedteamPluginSchema.safeParse('invalid-plugin-name').success).toBe(false);
    });
    ...
    it('should provide helpful error message for invalid plugin names', () => {
        const result = redteam_1.RedteamPluginSchema.safeParse('hate');
        expect(result.success).toBe(false);
        if (result.success) {
            return;
        }
    ...
    it('should provide helpful error message for invalid file:// paths', () => {
        const result = redteam_1.RedteamPluginSchema.safeParse('path/to/plugin.js');
        expect(result.success).toBe(false);
        if (result.success) {
            return;
        }
    ...
    it('should provide helpful error message for invalid plugin object', () => {
        const result = redteam_1.RedteamPluginSchema.safeParse({
            id: 'invalid-plugin',
            numTests: 5,
        });
        expect(result.success).toBe(false);
...
describe('redteamConfigSchema', () => {
    ...
    it('should use default values when fields are omitted', () => {
        ...
        expect(redteam_1.RedteamConfigSchema.safeParse(input)).toEqual({
            success: true,
            data: {
                plugins: expect.arrayContaining(Array(constants_1.DEFAULT_PLUGINS.size).fill(expect.any(Object))),
    ...
    it('should allow all valid plugin and strategy names', () => {
        ...
        const samplePlugins = constants_1.ALL_PLUGINS.slice(0, 20);
        const sampleStrategies = constants_1.ALL_STRATEGIES.filter((id) => id !== 'default' && id !== 'basic').slice(0, 10);
        const input = {
        ...
        const filteredPlugins = samplePlugins.filter((id) => !constants_1.COLLECTIONS.includes(id));
    ...
    it('should handle plugins with config attributes', () => {
        ...
        expect(redteam_1.RedteamConfigSchema.safeParse(input)?.data?.plugins).toHaveLength(Object.keys(constants_1.HARM_PLUGINS).length + 2);
    });
    it('should handle PII plugins with default numTests', () => {
        ...
        expect(piiPlugins).toEqual(expect.arrayContaining([
            { id: 'pii:session', numTests: 3 },
            ...constants_1.PII_PLUGINS.filter((id) => id !== 'pii:session').map((id) => ({ id, numTests: 7 })),
        ]));
        expect(piiPlugins).toHaveLength(constants_1.PII_PLUGINS.length);
    });
    ...
    describe('aliases', () => {
        it('should expand high-level aliased plugin names', () => {
            ...
            expect(result.data?.plugins).toEqual(expect.arrayContaining(expectedPlugins.map((id) => expect.objectContaining({ id, numTests: 3 }))));
        });
        ...
        it('should expand strategies for "owasp:llm" alias', () => {
            ...
            const result = redteam_1.RedteamConfigSchema.safeParse(input);
            expect(result.success).toBe(true);
            expect(result.data?.strategies).toEqual(expect.arrayContaining([
                { id: 'basic' },
                { id: 'jailbreak' },
...
describe('RedteamConfigSchema transform', () => {
    it('should deduplicate plugins with same id and config', () => {
        const result = redteam_1.RedteamConfigSchema.parse({
            plugins: [
    ...
    it('should sort plugins by id and then by config', () => {
        ...
        expect(result.plugins?.map((p) => ({ id: p.id, config: p.config }))).toEqual([
            { id: 'contracts', config: { key: 'a' } },
            { id: 'contracts', config: { key: 'b' } },
            { id: 'harmful:hate' },
        ]);
    ...
    it('should filter out collection plugins from final output', () => {
        ...
        expect(result.plugins?.some((p) => ['harmful', 'pii', 'default'].includes(p.id))).toBe(false);
    });
    it('should expand collection plugins correctly', () => {
        ...
        expect(result.plugins?.every((p) => p.id.startsWith('harmful:'))).toBe(true);
        expect(result.plugins?.every((p) => p.numTests === 5)).toBe(true);
    });
    it('should handle plugin aliases and their associated strategies', () => {
        ...
        expect(result.strategies?.some((s) => {
    ...
    describe('file:// plugins', () => {
        ...
        it('should reject non-file:// custom paths', () => {
            expect(() => redteam_1.RedteamConfigSchema.parse({
    ...
    it('should include all optional fields in transformed output', () => {
        ...
        const result = redteam_1.RedteamConfigSchema.parse(input);
    ...
    it('should preserve order of optional fields in transformed output', () => {
        ...
        const optionalFields = keys.filter((key) => ['delay', 'entities', 'injectVar', 'language', 'provider', 'purpose'].includes(key));
    ...
    it('should handle multiple collection plugins including foundation', () => {
        ...
        expect(result.plugins?.some((p) => Array.from(constants_1.FOUNDATION_PLUGINS).includes(p.id))).toBe(true);
        expect(result.plugins?.some((p) => p.id.startsWith('harmful:'))).toBe(true);
        expect(result.plugins?.some((p) => constants_1.PII_PLUGINS.includes(p.id))).toBe(true);
        expect(result.plugins?.every((p) => !['foundation', 'harmful', 'pii'].includes(p.id))).toBe(true);
    });
});
describe('RedteamStrategySchema', () => {
    it('should accept valid built-in strategy names', () => {
        expect(redteam_1.RedteamStrategySchema.safeParse('jailbreak').success).toBe(true);
        expect(redteam_1.RedteamStrategySchema.safeParse('prompt-injection').success).toBe(true);
    });
    it('should accept valid file:// paths', () => {
        expect(redteam_1.RedteamStrategySchema.safeParse('file://path/to/strategy.js').success).toBe(true);
        expect(redteam_1.RedteamStrategySchema.safeParse('file://path/to/strategy.ts').success).toBe(true);
    });
    it('should provide helpful error message for invalid strategy names', () => {
        const result = redteam_1.RedteamStrategySchema.safeParse('invalid-strategy');
        expect(result.success).toBe(false);
        if (result.success) {
            return;
        }
        ...
        constants_1.ALL_STRATEGIES.forEach((strategy) => {
            expect(errorMessage).toContain(strategy);
        });
    });
    it('should provide helpful error message for invalid file:// paths', () => {
        const result = redteam_1.RedteamStrategySchema.safeParse('file://path/to/strategy.invalid');
        expect(result.success).toBe(false);
        if (result.success) {
            return;
        }
    ...
    it('should provide helpful error message for non-file:// paths', () => {
        const result = redteam_1.RedteamStrategySchema.safeParse('path/to/strategy.js');
        expect(result.success).toBe(false);
        if (result.success) {
            return;
        }
    ...
    it('should provide helpful error message for invalid strategy object', () => {
        const result = redteam_1.RedteamStrategySchema.safeParse({
            id: 'invalid-strategy',
            config: {},
        });
        expect(result.success).toBe(false);
