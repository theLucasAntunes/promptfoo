...
describe('selectMaxScore', () => {
    const createMockResult = (score, testIdx) => ({
        promptIdx: 0,
        testIdx,
        testCase: {
            assert: [
            ...
            componentResults: [
                {
                    ...
                    assertion: { type: 'contains', value: 'apple' },
                },
                {
                    pass: score === 1,
                    score: score === 1 ? 1 : 0,
                    reason: score === 1 ? 'Contains orange' : 'Does not contain orange',
                    assertion: { type: 'contains', value: 'orange' },
                },
            ],
        },
        namedScores: {},
    ...
    it('should select the output with the highest score', async () => {
        ...
        const gradingResults = await (0, matchers_1.selectMaxScore)(outputs, results, mockAssertion);
    ...
    it('should apply threshold when specified', async () => {
        ...
        const assertionWithThreshold = {
            type: 'max-score',
            value: {
                threshold: 0.7,
            },
        };
        const gradingResults = await (0, matchers_1.selectMaxScore)(outputs, results, assertionWithThreshold);
        expect(gradingResults[0].pass).toBe(false);
    ...
    it('should throw error with fewer than 2 outputs', async () => {
        ...
        await expect((0, matchers_1.selectMaxScore)(outputs, results, mockAssertion)).rejects.toThrow('max-score assertion must have at least two outputs to compare between');
    });
    it('should throw error when no other assertions exist', async () => {
        ...
        await expect((0, matchers_1.selectMaxScore)(outputs, results, mockAssertion)).rejects.toThrow('max-score requires at least one other assertion (besides max-score or select-best) to aggregate scores from');
    });
    ...
    describe('Threshold Edge Cases - Truthy vs Undefined', () => {
        it('should handle threshold=0 differently from threshold=undefined', async () => {
            ...
            const assertionWithoutThreshold = {
                type: 'max-score',
                value: {},
            };
            const resultsWithoutThreshold = await (0, matchers_1.selectMaxScore)(outputs, results, assertionWithoutThreshold);
            ...
            const assertionWithZeroThreshold = {
                type: 'max-score',
                value: {
                    threshold: 0,
                },
            };
            const resultsWithZeroThreshold = await (0, matchers_1.selectMaxScore)(outputs, results, assertionWithZeroThreshold);
        ...
        it('should fail all outputs when max score is below threshold=0', async () => {
            ...
            const gradingResults = await (0, matchers_1.selectMaxScore)(outputs, results, assertionWithZeroThreshold);
