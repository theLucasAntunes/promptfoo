...
describe('matchesFactuality', () => {
    ...
    it('should pass when the factuality check passes with legacy format', async () => {
        ...
        jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockImplementation(mockCallApi);
        await expect((0, matchers_1.matchesFactuality)(input, expected, output, grading)).resolves.toEqual({
            pass: true,
    ...
    it('should pass when the factuality check passes with JSON format', async () => {
        ...
        const grading = {};
        const mockCallApi = jest.fn().mockResolvedValue({
            output: '{"category": "A", "reason": "The submitted answer is a subset of the expert answer and is fully consistent with it."}',
            tokenUsage: { total: 10, prompt: 5, completion: 5 },
        });
        jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockImplementation(mockCallApi);
        await expect((0, matchers_1.matchesFactuality)(input, expected, output, grading)).resolves.toEqual({
    ...
    it('should fail when the factuality check fails with JSON format', async () => {
        ...
        const grading = {};
        const mockCallApi = jest.fn().mockResolvedValue({
            output: '{"category": "D", "reason": "There is a disagreement between the submitted answer and the expert answer."}',
            tokenUsage: { total: 10, prompt: 5, completion: 5 },
        });
        jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockImplementation(mockCallApi);
        await expect((0, matchers_1.matchesFactuality)(input, expected, output, grading)).resolves.toEqual({
    ...
    it('should use category description as fallback when no reason is provided in JSON', async () => {
        ...
        await expect((0, matchers_1.matchesFactuality)(input, expected, output, grading)).resolves.toEqual({
            pass: true,
            reason: 'Category A: The submitted answer is a subset of the expert answer and is fully consistent with it.',
            score: 1,
            tokensUsed: expect.objectContaining({
                total: expect.any(Number),
                prompt: expect.any(Number),
...
(E) The answers differ, but these differences don't matter from the perspective of factuality.`,
        ...
        const result = await (0, matchers_1.matchesFactuality)(input, expected, output, grading);
        ...
        expect(mockCallApi).toHaveBeenCalledWith(expect.stringContaining('The submitted answer may either be a subset or superset of the expert answer'));
    });
    it('should throw an error when an error occurs', async () => {
        ...
        await expect((0, matchers_1.matchesFactuality)(input, expected, output, grading)).rejects.toThrow('An error occurred');
    });
    it('should use Nunjucks templating when PROMPTFOO_DISABLE_TEMPLATING is set', async () => {
        ...
        const grading = {
            provider: defaults_1.DefaultGradingProvider,
        };
        jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockResolvedValue({
            output: '{"category": "A", "reason": "The submitted answer is correct."}',
            tokenUsage: { total: 10, prompt: 5, completion: 5 },
        });
        await (0, matchers_1.matchesFactuality)(input, expected, output, grading);
        expect(defaults_1.DefaultGradingProvider.callApi).toHaveBeenCalledWith(expect.stringContaining('Input {{ var }}'));
        expect(defaults_1.DefaultGradingProvider.callApi).toHaveBeenCalledWith(expect.stringContaining('Expected {{ var }}'));
        expect(defaults_1.DefaultGradingProvider.callApi).toHaveBeenCalledWith(expect.stringContaining('Output {{ var }}'));
        process.env.PROMPTFOO_DISABLE_TEMPLATING = undefined;
    });
});
//# sourceMappingURL=factuality.test.js.map