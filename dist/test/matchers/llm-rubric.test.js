...
const remoteGrading = __importStar(require("../../src/remoteGrading"));
...
const Grader = new utils_2.TestGrader();
describe('matchesLlmRubric', () => {
    const mockFilePath = path_1.default.join('path', 'to', 'external', 'rubric.txt');
    const mockFileContent = 'This is an external rubric prompt';
    beforeEach(() => {
        jest.clearAllMocks();
        jest.resetAllMocks();
        jest.mocked(fs_1.default.existsSync).mockReturnValue(true);
        jest.mocked(fs_1.default.readFileSync).mockReturnValue(mockFileContent);
    ...
    it('should pass when the grading provider returns a passing result', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(expected, output, options)).resolves.toEqual({
            pass: true,
    ...
    it('should render rubric when provided as an object', async () => {
        ...
        await (0, matchers_1.matchesLlmRubric)(rubric, output, options);
        expect(options.provider.callApi).toHaveBeenCalledWith(expect.stringContaining(JSON.stringify(rubric)), expect.objectContaining({
    ...
    it('should throw error when throwOnError is true and provider returns an error', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubric, llmOutput, grading, {}, undefined, { throwOnError: true })).rejects.toThrow('Provider error');
    });
    it('should throw error when throwOnError is true and provider returns no result', async () => {
        ...
        // With throwOnError: true - should throw
        await expect((0, matchers_1.matchesLlmRubric)(rubric, llmOutput, grading, {}, undefined, { throwOnError: true })).rejects.toThrow('No output');
    });
    it('should use the overridden llm rubric grading config', async () => {
        ...
        const mockCallApi = jest.spyOn(chat_1.OpenAiChatCompletionProvider.prototype, 'callApi');
        ...
        expect(mockCallApi).toHaveBeenCalledWith('Grading prompt', expect.objectContaining({
            prompt: expect.objectContaining({
                label: 'llm-rubric',
                raw: 'Grading prompt',
            }),
    ...
    it('should use provided score threshold if llm does not return pass', async () => {
        ...
        const lowScoreProvider = {
            id: () => 'test-provider',
            callApi: jest.fn().mockResolvedValue({
                output: JSON.stringify(lowScoreResponse),
            }),
        };
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, llmOutput, { rubricPrompt, provider: lowScoreProvider }, {}, assertion)).resolves.toEqual(expect.objectContaining({ assertion, pass: false, ...lowScoreResponse }));
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, llmOutput, { rubricPrompt, provider: highScoreProvider }, {}, assertion)).resolves.toEqual(expect.objectContaining({ assertion, pass: true, ...highScoreResponse }));
    });
    it('should ignore the score threshold if llm returns pass', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, lowScoreOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({ assertion, ...lowScoreResult }));
    });
    it('should respect both threshold and explicit pass/fail when both are present', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, failingOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            score: 0.7,
            pass: false,
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, passingOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            score: 0.9,
            pass: false,
            reason: 'Score above threshold but explicit fail',
    ...
    it('should handle edge cases around threshold value', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, exactOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            score: 0.8,
            pass: true,
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, belowOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            score: 0.799,
            pass: false,
    ...
    it('should handle missing or invalid scores when threshold is present', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, missingScoreOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            score: 1.0,
            pass: true,
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, invalidScoreOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            score: 1.0,
            pass: true,
    ...
    it('should handle string scores', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, stringScoreOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            score: 0.9,
            pass: true,
            reason: 'String score',
    ...
    it('should handle string pass values', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, stringPassOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            pass: true,
            reason: 'String pass',
        }));
        const stringFailResult = { reason: 'String fail', pass: 'false' };
        const stringFailOptions = {
            rubricPrompt,
            provider: {
                id: () => 'test-provider',
                callApi: jest.fn().mockResolvedValue({
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubricPrompt, output, stringFailOptions, {}, assertion)).resolves.toEqual(expect.objectContaining({
            assertion,
            pass: false,
            reason: 'String fail',
        }));
    ...
    it('should load rubric prompt from external file when specified', async () => {
        ...
        const result = await (0, matchers_1.matchesLlmRubric)(rubric, llmOutput, grading);
        expect(fs_1.default.existsSync).toHaveBeenCalledWith(expect.stringContaining(path_1.default.join('path', 'to', 'external', 'rubric.txt')));
        expect(fs_1.default.readFileSync).toHaveBeenCalledWith(expect.stringContaining(path_1.default.join('path', 'to', 'external', 'rubric.txt')), 'utf8');
        expect(grading.provider.callApi).toHaveBeenCalledWith(expect.stringContaining(mockFileContent), expect.objectContaining({
            prompt: expect.objectContaining({
                label: 'llm-rubric',
    ...
    it('should load rubric prompt from js file when specified', async () => {
        const filePath = path_1.default.join('path', 'to', 'external', 'file.js');
        ...
        const result = await (0, matchers_1.matchesLlmRubric)(rubric, llmOutput, grading);
        await expect((0, utils_1.loadFromJavaScriptFile)(filePath, undefined, [])).resolves.toBe('Do this: {{ rubric }}');
        expect(grading.provider.callApi).toHaveBeenCalledWith(expect.stringContaining('Do this: Test rubric'), expect.objectContaining({
            prompt: expect.objectContaining({
                label: 'llm-rubric',
        ...
        expect(result).toEqual(expect.objectContaining({ pass: true, score: 1, reason: 'Test passed' }));
    });
    it('should throw an error when the external file is not found', async () => {
        ...
        await expect((0, matchers_1.matchesLlmRubric)(rubric, llmOutput, grading)).rejects.toThrow('File does not exist');
        expect(fs_1.default.existsSync).toHaveBeenCalledWith(expect.stringContaining(path_1.default.join('path', 'to', 'external', 'rubric.txt')));
    ...
    it('should not call remote when rubric prompt is overridden, even if redteam is enabled', async () => {
        ...
        await (0, matchers_1.matchesLlmRubric)(rubric, llmOutput, grading);
        const { doRemoteGrading } = remoteGrading;
        expect(doRemoteGrading).not.toHaveBeenCalled();
        expect(grading.provider.callApi).toHaveBeenCalledWith(expect.stringContaining('Custom prompt'), expect.objectContaining({
            prompt: expect.objectContaining({
                label: 'llm-rubric',
...
//# sourceMappingURL=llm-rubric.test.js.map