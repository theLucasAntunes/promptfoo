...
describe('matchesClosedQa', () => {
    ...
    it('should pass when the closed QA check passes', async () => {
        ...
        const grading = {};
        jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockResolvedValueOnce({
            output: 'foo \n \n bar\n Y Y \n',
            tokenUsage: { total: 10, prompt: 5, completion: 5 },
        });
        await expect((0, matchers_1.matchesClosedQa)(input, expected, output, grading)).resolves.toEqual({
            pass: true,
    ...
    it('should throw an error when an error occurs', async () => {
        ...
        await expect((0, matchers_1.matchesClosedQa)(input, expected, output, grading)).rejects.toThrow('An error occurred');
    });
    it('should handle input, criteria, and completion that need escaping', async () => {
        ...
        const grading = {};
        let isJson = false;
        jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockImplementation((prompt) => {
            try {
                JSON.parse(prompt);
                isJson = true;
            }
    ...
    it('should use Nunjucks templating when PROMPTFOO_DISABLE_TEMPLATING is set', async () => {
        ...
        await (0, matchers_1.matchesClosedQa)(input, expected, output, grading);
...
//# sourceMappingURL=closed-qa.test.js.map