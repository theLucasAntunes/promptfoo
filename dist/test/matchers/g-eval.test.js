...
describe('matchesGEval', () => {
    let originalCallApi;
    beforeEach(() => {
        originalCallApi = defaults_1.DefaultGradingProvider.callApi;
        jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockImplementation(async (prompt) => {
    ...
    afterEach(() => {
        defaults_1.DefaultGradingProvider.callApi = originalCallApi;
    });
    it('should properly evaluate with default prompts', async () => {
        ...
        const threshold = 0.7;
        const result = await (0, matchers_1.matchesGEval)(criteria, input, output, threshold);
        ...
        expect(defaults_1.DefaultGradingProvider.callApi).toHaveBeenCalledTimes(2);
    });
    it('should handle custom rubric prompts', async () => {
        ...
        defaults_1.DefaultGradingProvider.callApi = mockCallApi;
        const criteria = 'Evaluate coherence and clarity';
        const input = 'Test input';
        const output = 'Test output';
        const threshold = 0.7;
        ...
        const result = await (0, matchers_1.matchesGEval)(criteria, input, output, threshold, grading);
        expect(result.score).toBe(0.8);
        expect(mockCallApi).toHaveBeenCalledTimes(2);
        expect(mockCallApi).toHaveBeenNthCalledWith(1, expect.stringContaining('Custom steps template with'));
        expect(mockCallApi).toHaveBeenNthCalledWith(2, expect.stringContaining('Custom evaluation template with'));
        defaults_1.DefaultGradingProvider.callApi = originalCallApi;
    });
    it('should fail when score is below threshold', async () => {
        jest
            .spyOn(defaults_1.DefaultGradingProvider, 'callApi')
...
//# sourceMappingURL=g-eval.test.js.map