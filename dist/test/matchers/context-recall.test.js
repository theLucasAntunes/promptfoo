...
describe('matchesContextRecall', () => {
    ...
    it('should pass when the recall score is above the threshold', async () => {
        ...
        await expect((0, matchers_1.matchesContextRecall)(context, groundTruth, threshold)).resolves.toEqual({
            pass: true,
    ...
    it('should return detailed metadata with sentence attributions', async () => {
        ...
        const groundTruth = 'Paris is the capital of France. The Eiffel Tower is located there. London is the capital of UK.';
        const threshold = 0.6;
        const mockCallApi = jest.fn().mockImplementation(() => {
            return Promise.resolve({
                output: 'Paris is the capital of France. [Attributed]\nThe Eiffel Tower is located there. [Attributed]\nLondon is the capital of UK. [Not attributed]',
                tokenUsage: { total: 10, prompt: 5, completion: 5 },
            });
        });
        jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockImplementation(mockCallApi);
        const result = await (0, matchers_1.matchesContextRecall)(context, groundTruth, threshold);
    ...
    describe('Array Context Support', () => {
        it('should handle array of context chunks', async () => {
            ...
            jest.spyOn(defaults_1.DefaultGradingProvider, 'callApi').mockImplementation(mockCallApi);
            const result = await (0, matchers_1.matchesContextRecall)(contextChunks, groundTruth, threshold);
            expect(result.pass).toBe(true);
...
//# sourceMappingURL=context-recall.test.js.map