...
describe('wrapper', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        jest
            .mocked(pythonUtils_1.validatePythonPath)
            .mockImplementation((pythonPath, isExplicit) => {
            pythonUtils_1.state.cachedPythonPath = pythonPath;
    ...
    describe('runPythonCode', () => {
        it('should clean up the temporary files after execution', async () => {
            ...
            jest.spyOn(fs_1.default, 'writeFileSync').mockImplementation(mockWriteFileSync);
            jest.spyOn(fs_1.default, 'unlinkSync').mockImplementation(mockUnlinkSync);
            jest.mocked(pythonUtils_1.runPython).mockImplementation(mockRunPython);
            await (0, wrapper_1.runPythonCode)('print("cleanup test")', 'main', []);
            expect(mockWriteFileSync).toHaveBeenCalledTimes(1);
            expect(mockWriteFileSync).toHaveBeenCalledWith(expect.stringContaining('temp-python-code-'), 'print("cleanup test")');
            expect(mockRunPython).toHaveBeenCalledTimes(1);
            expect(mockRunPython).toHaveBeenCalledWith(expect.stringContaining('temp-python-code-'), 'main', []);
        ...
        it('should execute Python code from a string and read the output file', async () => {
            ...
            jest.spyOn(fs_1.default, 'writeFileSync').mockReturnValue();
            jest.spyOn(fs_1.default, 'unlinkSync').mockReturnValue();
            const mockRunPython = jest.mocked(pythonUtils_1.runPython);
            mockRunPython.mockResolvedValue(mockOutput.data);
            const code = 'print("Hello, world!")';
            const result = await (0, wrapper_1.runPythonCode)(code, 'main', []);
            expect(result).toBe('execution result');
            expect(mockRunPython).toHaveBeenCalledWith(expect.stringContaining('.py'), 'main', []);
            expect(fs_1.default.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('.py'), code);
        });
    });
    describe('validatePythonPath race conditions', () => {
        beforeAll(() => {
            ...
            jest.mocked(pythonUtils_1.validatePythonPath).mockImplementation(realModule.validatePythonPath);
        });
        beforeEach(() => {
            // Reset the cached path and validation promise before each test
            pythonUtils_1.state.cachedPythonPath = null;
            pythonUtils_1.state.validationPromise = null;
            ...
            jest.mocked(pythonUtils_1.tryPath).mockImplementation(async (path) => {
                // Simulate slow Windows process spawning and antivirus scanning
                await new Promise((resolve) => setTimeout(resolve, 50));
                return '/usr/bin/python3';
            });
            jest.mocked(pythonUtils_1.getSysExecutable).mockImplementation(async () => {
        ...
        it('should handle concurrent validatePythonPath calls without race conditions', async () => {
            ...
            const promises = Array.from({ length: concurrentCalls }, (_, i) => (0, pythonUtils_1.validatePythonPath)('python', false).then((result) => ({ testId: i, result })));
            ...
            expect(pythonUtils_1.state.cachedPythonPath).toBeTruthy();
        ...
        it('should handle mixed explicit/implicit validation calls consistently', async () => {
            ...
            const mixedPromises = Array.from({ length: 8 }, (_, i) => {
                ...
                return (0, pythonUtils_1.validatePythonPath)('python', isExplicit).then((result) => ({
                    testId: i,
                    result,
                    isExplicit,
                }));
        ...
        it('should handle rapid successive calls without race conditions', async () => {
            ...
            const rapidCalls = 20;
            const promises = Array.from({ length: rapidCalls }, (_, i) => (0, pythonUtils_1.validatePythonPath)('python', false).then((result) => ({ testId: i, result })));
            const results = await Promise.all(promises);
            // All calls should succeed
            ...
            expect(pythonUtils_1.state.cachedPythonPath).toBe(results[0].result);
        }, 10000);
    });
});
