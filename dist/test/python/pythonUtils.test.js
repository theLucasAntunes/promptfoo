...
const mockExecFileAsync = require('util').__mockExecFileAsync;
// Mock setup
jest.mock('fs', () => ({
...
describe('Python Utils', () => {
    ...
    describe('tryPath', () => {
        ...
        describe('failed path validation', () => {
            ...
            it('should return null if the command times out', async () => {
                ...
                mockExecFileAsync.mockReturnValue(execPromise);
    ...
    describe('validatePythonPath', () => {
        ...
        describe('fallback behavior', () => {
            ...
            it('should throw an error when no valid Python path is found', async () => {
                mockExecFileAsync.mockReset();
                mockExecFileAsync.mockRejectedValue(new Error('Command failed'));
                await expect(pythonUtils.validatePythonPath('python', false)).rejects.toThrow('Python 3 not found. Tried "python", sys.executable detection, and fallback commands.');
        ...
        describe('concurrent validation', () => {
            it('should share validation promise between concurrent calls', async () => {
                ...
                mockExecFileAsync.mockReturnValueOnce(firstPromise);
            ...
            it('should handle race conditions between concurrent validation attempts', async () => {
                ...
                mockExecFileAsync.mockReset();
                mockExecFileAsync.mockReturnValueOnce(controlledPromise);
        ...
        describe('promise cleanup', () => {
            it('should clear validation promise after failed validation', async () => {
                mockExecFileAsync.mockRejectedValue(new Error('Command failed'));
                await expect(pythonUtils.validatePythonPath('python', true)).rejects.toThrow('Python 3 not found. Tried "python"');
    ...
    describe('runPython', () => {
        ...
        it('should execute a Python script with proper arguments', async () => {
            ...
            jest
                .mocked(fs_1.default.readFileSync)
            ...
            expect(python_shell_1.PythonShell).toHaveBeenCalledWith('wrapper.py', expect.objectContaining({
                ...
                args: expect.arrayContaining([path_1.default.resolve(scriptPath), 'test_method']),
            }));
        });
        it('should throw an error if Python script returns invalid JSON', async () => {
            ...
            await expect(pythonUtils.runPython('/path/to/script.py', 'test_method', [])).rejects.toThrow('Invalid JSON');
        });
        it('should throw an error if Python script does not return final_result', async () => {
            jest.mocked(fs_1.default.writeFileSync).mockImplementation();
            jest.mocked(fs_1.default.readFileSync).mockReturnValue(JSON.stringify({ type: 'other', data: 42 }));
            jest.mocked(fs_1.default.unlinkSync).mockImplementation();
            mockExecFileAsync.mockResolvedValue({
                stdout: 'Python 3.8.10\n',
                stderr: '',
            ...
            await expect(pythonUtils.runPython('/path/to/script.py', 'test_method', [])).rejects.toThrow('The Python script `call_api` function must return a dict with an `output`');
        });
