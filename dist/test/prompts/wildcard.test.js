...
jest.mock('../../src/python/pythonUtils', () => ({
    runPython: jest.fn((filePath, functionName, args) => {
...
jest.mock('../../src/esm', () => ({
    importModule: jest.fn((filePath) => {
        return Promise.resolve(function (context) {
...
describe('Wildcard prompt support', () => {
    ...
    describe('Python wildcards', () => {
        it('should expand wildcard patterns and preserve function names', async () => {
            const prompts = await (0, index_1.processPrompts)(['file://test/prompts/**/*.py:get_prompt']);
        ...
        it('should work without function names', async () => {
            const prompts = await (0, index_1.processPrompts)(['file://test/prompts/**/*.py']);
            expect(prompts.length).toBe(3);
    ...
    describe('JavaScript wildcards', () => {
        it('should expand wildcard patterns for JavaScript files', async () => {
            const prompts = await (0, index_1.processPrompts)(['file://test/prompts/**/*.js']);
        ...
        it('should preserve function names for JavaScript', async () => {
            const prompts = await (0, index_1.processPrompts)(['file://test/prompts/**/*.js:myFunction']);
            expect(prompts.length).toBe(2);
            prompts.forEach((prompt) => {
    ...
    describe('Mixed patterns', () => {
        it('should handle both wildcards and explicit paths', async () => {
            ...
            const prompts = await (0, index_1.processPrompts)([
