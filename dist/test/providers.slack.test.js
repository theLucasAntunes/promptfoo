...
const slack_1 = require("../src/providers/slack");
...
describe('SlackProvider', () => {
    ...
    beforeEach(() => {
        ...
        web_api_1.WebClient.mockImplementation(() => mockWebClient);
    });
    afterEach(() => {
    ...
    describe('constructor', () => {
        it('should throw error if no token is provided', () => {
            delete process.env.SLACK_BOT_TOKEN;
            expect(() => new slack_1.SlackProvider({ config: { channel: 'C123' } })).toThrow('Slack provider requires a token');
        });
        it('should throw error if no channel is provided', () => {
            expect(() => new slack_1.SlackProvider({ config: { token: 'xoxb-test' } })).toThrow('Slack provider requires a channel ID');
        });
        it('should use token from config over environment variable', () => {
            const provider = new slack_1.SlackProvider({
                config: {
                    token: 'xoxb-config-token',
                    channel: 'C123',
                },
            });
            expect(web_api_1.WebClient).toHaveBeenCalledWith('xoxb-config-token');
        ...
        it('should use token from environment variable if not in config', () => {
            ...
            expect(web_api_1.WebClient).toHaveBeenCalledWith('xoxb-test-token');
            expect(provider).toBeDefined();
        });
    });
    describe('id()', () => {
        it('should return default id if not provided', () => {
            const provider = new slack_1.SlackProvider({ config: { channel: 'C123' } });
    ...
    describe('callApi()', () => {
        let provider;
        beforeEach(() => {
            provider = new slack_1.SlackProvider({
                config: {
                    channel: 'C123',
                    timeout: 1000,
                },
...
//# sourceMappingURL=providers.slack.test.js.map