...
describe('testCaseFromCsvRow', () => {
    ...
    it('should create a TestCase with assertions and options from a CSV row', () => {
        ...
        const result = (0, csv_1.testCaseFromCsvRow)(row);
    ...
    it('should log a warning for single underscore usage with reserved keys', () => {
        ...
        (0, csv_1.testCaseFromCsvRow)(row);
        (0, csv_1.testCaseFromCsvRow)(row);
        expect(logger_1.default.warn).toHaveBeenCalledWith('You used a single underscore for the key "_expected". Did you mean to use "__expected" instead?');
    ...
    it('should warn when __metadata has no key', () => {
        ...
        expect(result).toEqual(expectedTestCase);
        (0, csv_1.testCaseFromCsvRow)(row);
        expect(logger_1.default.warn).toHaveBeenCalledWith('The "__metadata" column requires a key, e.g. "__metadata:category". This column will be ignored.');
        expect(logger_1.default.warn).toHaveBeenCalledTimes(1);
    });
    ...
    describe('__config columns', () => {
        ...
        it('warns and ignores invalid __config column formats', () => {
            ...
            expect(logger_1.default.warn).toHaveBeenCalledWith('Invalid __config column format: "__config:__expected1". Expected format: __config:__expected:threshold or __config:__expected<N>:threshold');
        });
        it('throws on invalid expected key in __config column', () => {
            ...
            expect(() => (0, csv_1.testCaseFromCsvRow)(row)).toThrow('Invalid expected key "__expectedX" in __config column');
            expect(logger_1.default.error).toHaveBeenCalledWith('Invalid expected key "__expectedX" in __config column "__config:__expectedX:threshold". Must be __expected or __expected<N> where N is a positive integer.');
        });
        it('throws on invalid config key in __config column', () => {
            ...
            expect(() => (0, csv_1.testCaseFromCsvRow)(row)).toThrow('Invalid config key "bogus" in __config column');
            expect(logger_1.default.error).toHaveBeenCalledWith('Invalid config key "bogus" in __config column "__config:__expected1:bogus". Valid config keys include: threshold');
        });
        it('throws on non-numeric threshold values', () => {
            ...
            expect(() => (0, csv_1.testCaseFromCsvRow)(row)).toThrow('Invalid numeric value for threshold');
            expect(logger_1.default.error).toHaveBeenCalledWith('Invalid numeric value "not-a-number" for config key "threshold" in column "__config:__expected1:threshold"');
        });
    });
});
describe('assertionFromString', () => {
    ...
    it('should create an equality assertion', () => {
        ...
        const result = (0, csv_1.assertionFromString)(expected);
...
`);
    ...
    it('should parse python: assertion', () => {
        const assertion = (0, csv_1.assertionFromString)('python:some_python_code');
    ...
    it(`should parse file:// prefix assertion`, () => {
        const assertion = (0, csv_1.assertionFromString)('file://script.py');
        expect(assertion).toEqual({
            type: 'python',
            value: 'script.py',
        });
    ...
    it(`should parse file:// prefix assertion with function name`, () => {
        const assertion = (0, csv_1.assertionFromString)('file://script.py:function_name');
        expect(assertion).toEqual({
            type: 'python',
...
describe('serializeObjectArrayAsCSV', () => {
    it('should serialize an array of objects as a CSV string', () => {
        expect((0, csv_1.serializeObjectArrayAsCSV)([
            { name: 'John', age: 30 },
            { name: 'Jane', age: 25 },
        ])).toBe('name,age\n"John","30"\n"Jane","25"\n');
    });
    ...
    it('should handle multiline values', () => {
        expect((0, csv_1.serializeObjectArrayAsCSV)([{ name: 'John Smith\nSmithy', age: 30 }])).toBe('name,age\n"John Smith\nSmithy","30"\n');
    });
    it('should serialize vars to CSV format', () => {
        ...
        expect((0, csv_1.serializeObjectArrayAsCSV)(vars)).toBe(expected);
    });
    it('should handle empty vars array', () => {
        const vars = [];
        expect(() => (0, csv_1.serializeObjectArrayAsCSV)(vars)).toThrow('Invariant failed: No variables to serialize');
    });
