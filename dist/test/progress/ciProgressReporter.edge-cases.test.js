...
describe('CIProgressReporter - Edge Cases', () => {
    ...
    describe('Zero and Single Test Cases', () => {
        it('should handle zero tests gracefully', () => {
            const reporter = new ciProgressReporter_1.CIProgressReporter(0);
            reporter.start();
            expect(logger_1.default.info).toHaveBeenCalledWith('[Evaluation] Starting 1 test cases...');
            // Should not crash on update
            reporter.update(0);
            reporter.finish();
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[Evaluation] ✓ Complete! 0/1 tests in'));
        });
        it('should handle single test', () => {
            const reporter = new ciProgressReporter_1.CIProgressReporter(1);
            reporter.start();
            jest.clearAllMocks();
            reporter.update(1);
            // Should log 100% milestone
            expect(logger_1.default.info).toHaveBeenCalledTimes(0); // 100% is not in milestones array
            reporter.finish();
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[Evaluation] ✓ Complete! 1/1 tests in'));
        });
    });
    describe('Out of Order and Invalid Updates', () => {
        it('should handle updates out of order', () => {
            ...
            // Update to 50% first
            reporter.update(50);
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[Evaluation] ✓ 50% complete'));
            jest.clearAllMocks();
            // Then update to 25% - should not log again
            reporter.update(25);
            expect(logger_1.default.info).not.toHaveBeenCalledWith(expect.stringContaining('[Evaluation] ✓ 25% complete'));
        });
        it('should handle updates beyond total tests', () => {
            ...
            reporter.finish();
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[Evaluation] ✓ Complete! 150/100 tests in'));
        });
        it('should handle negative updates', () => {
            ...
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[CI Progress] Evaluation running for'));
        });
    ...
    describe('Timing Edge Cases', () => {
        it('should handle very fast completion (< 1 second)', () => {
            const reporter = new ciProgressReporter_1.CIProgressReporter(100, 100); // 100ms interval
            reporter.start();
            ...
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[CI Progress] Evaluation running for'));
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[CI Progress] Rate:'));
        });
        it('should handle very long running tests (> 24 hours)', () => {
            const reporter = new ciProgressReporter_1.CIProgressReporter(100000, 1000);
            reporter.start();
            ...
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('ETA: >24 hours'));
        });
        it('should handle zero progress after long time', () => {
            const reporter = new ciProgressReporter_1.CIProgressReporter(100, 1000);
            reporter.start();
            // Don't update progress
            jest.advanceTimersByTime(5000);
            // Should not log periodic update with 0 completed tests
            expect(logger_1.default.info).toHaveBeenCalledTimes(1); // Only start message
        });
    });
    describe('Robustness Tests', () => {
        ...
        it('should handle finish without start', () => {
            ...
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[Evaluation] ✓ Complete!'));
        });
    ...
    describe('Rate Calculation Edge Cases', () => {
        it('should handle very slow rate (< 0.1 tests/minute)', () => {
            const reporter = new ciProgressReporter_1.CIProgressReporter(1000, 1000);
            reporter.start();
            ...
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('ETA: calculating...'));
        });
        it('should handle non-integer test counts', () => {
            const reporter = new ciProgressReporter_1.CIProgressReporter(99);
            reporter.start();
            // 25% of 99 = 24.75
            reporter.update(24);
            expect(logger_1.default.info).not.toHaveBeenCalledWith(expect.stringContaining('[Evaluation] ✓ 25% complete'));
            reporter.update(25);
            expect(logger_1.default.info).toHaveBeenCalledWith(expect.stringContaining('[Evaluation] ✓ 25% complete'));
        });
    });
    ...
    describe('Memory and Performance', () => {
        it('should not accumulate memory with many updates', () => {
            const reporter = new ciProgressReporter_1.CIProgressReporter(10000);
            reporter.start();
            ...
            const milestoneLogs = logger_1.default.info.mock.calls.filter((call) => call[0].includes('[Evaluation] ✓'));
        ...
        it('should handle rapid periodic updates', () => {
            ...
            expect(logger_1.default.info).toHaveBeenCalledTimes(20); // 10 * 2 (progress + rate)
        });
    });
});
//# sourceMappingURL=ciProgressReporter.edge-cases.test.js.map