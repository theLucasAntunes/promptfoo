...
describe('langfuse integration', () => {
    ...
    describe('getPrompt', () => {
        it('should fetch a text prompt by version', async () => {
            ...
            const { getPrompt } = await Promise.resolve().then(() => __importStar(require('../../src/integrations/langfuse')));
        ...
        it('should fetch a text prompt by label', async () => {
            ...
            mockGetPrompt.mockResolvedValue(mockPrompt);
            const { getPrompt } = await Promise.resolve().then(() => __importStar(require('../../src/integrations/langfuse')));
            const result = await getPrompt('test-prompt', { name: 'test' }, 'text', undefined, 'production');
        ...
        it('should handle errors from Langfuse API', async () => {
            ...
            const { getPrompt } = await Promise.resolve().then(() => __importStar(require('../../src/integrations/langfuse')));
            await expect(getPrompt('non-existent', {}, 'text', 1)).rejects.toThrow('Failed to fetch Langfuse prompt "non-existent" version 1: API Error: Prompt not found');
        });
        it('should provide context in error messages for label-based fetching', async () => {
            ...
            const { getPrompt } = await Promise.resolve().then(() => __importStar(require('../../src/integrations/langfuse')));
            await expect(getPrompt('test-prompt', {}, 'text', undefined, 'non-existent-label')).rejects.toThrow('Failed to fetch Langfuse prompt "test-prompt" with label "non-existent-label": Label not found');
        });
        it('should provide context in error messages for prompts without version or label', async () => {
            ...
            const { getPrompt } = await Promise.resolve().then(() => __importStar(require('../../src/integrations/langfuse')));
            await expect(getPrompt('test-prompt', {}, 'text')).rejects.toThrow('Failed to fetch Langfuse prompt "test-prompt": Network error');
        });
...
//# sourceMappingURL=langfuse.test.js.map