...
describe('huggingfaceDatasets', () => {
    ...
    it('should fetch and parse dataset with default parameters', async () => {
        ...
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith('https://datasets-server.huggingface.co/rows?dataset=test%2Fdataset&split=test&config=default&offset=0&length=100', expect.objectContaining({
            headers: {},
        }));
    ...
    it('should fall back to HF_API_TOKEN when HF_TOKEN is empty', async () => {
        ...
        await (0, huggingfaceDatasets_1.fetchHuggingFaceDataset)('huggingface://datasets/test/dataset', 1);
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
            headers: {
    ...
    it('should handle custom query parameters', async () => {
        ...
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith('https://datasets-server.huggingface.co/rows?dataset=test%2Fdataset&split=train&config=custom&offset=0&length=100', expect.objectContaining({
            headers: {},
        }));
    });
    it('should handle pagination', async () => {
        ...
        const tests = await (0, huggingfaceDatasets_1.fetchHuggingFaceDataset)('huggingface://datasets/test/dataset');
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledTimes(2);
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenNthCalledWith(1, 'https://datasets-server.huggingface.co/rows?dataset=test%2Fdataset&split=test&config=default&offset=0&length=100', expect.objectContaining({
            headers: {},
        }));
        // Note: Second call might have different length due to concurrent fetching and remaining calculation
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenNthCalledWith(2, expect.stringContaining('dataset=test%2Fdataset&split=test&config=default&offset=2'), expect.objectContaining({
            headers: {},
        }));
    ...
    it('should handle API errors by throwing', async () => {
        ...
        await expect((0, huggingfaceDatasets_1.fetchHuggingFaceDataset)('huggingface://datasets/nonexistent/dataset')).rejects.toThrow('[HF Dataset] Failed to fetch dataset: Not Found');
    });
    it('should short-circuit and return [] when limit is 0', async () => {
        const tests = await (0, huggingfaceDatasets_1.fetchHuggingFaceDataset)('huggingface://datasets/test/dataset', 0);
        expect(jest.mocked(cache_1.fetchWithCache)).not.toHaveBeenCalled();
    ...
    it('should respect user-specified limit parameter (single request optimization)', async () => {
        ...
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith('https://datasets-server.huggingface.co/rows?dataset=test%2Fdataset&split=test&config=default&limit=2&offset=0&length=2', expect.objectContaining({
            headers: {},
        }));
        expect(tests).toHaveLength(2);
    ...
    it('should handle limit larger than page size', async () => {
        ...
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledTimes(2);
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenNthCalledWith(1, 'https://datasets-server.huggingface.co/rows?dataset=test%2Fdataset&split=test&config=default&limit=120&offset=0&length=100', expect.objectContaining({
            headers: {},
        }));
        expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenNthCalledWith(2, 'https://datasets-server.huggingface.co/rows?dataset=test%2Fdataset&split=test&config=default&limit=120&offset=100&length=20', expect.objectContaining({
            headers: {},
        }));
    ...
    describe('performance optimizations', () => {
        it('should use single request optimization for small limits', async () => {
            ...
            const tests = await (0, huggingfaceDatasets_1.fetchHuggingFaceDataset)('huggingface://datasets/test/dataset', 50);
            // Should only make one request for limits <= 100
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledTimes(1);
        ...
        it('should throw error on page fetch failure', async () => {
            ...
            await expect((0, huggingfaceDatasets_1.fetchHuggingFaceDataset)('huggingface://datasets/test/dataset', 200)).rejects.toThrow('[HF Dataset] Failed to fetch dataset: Internal Server Error');
        });
        it('should adapt page size based on row size', async () => {
            ...
            const tests = await (0, huggingfaceDatasets_1.fetchHuggingFaceDataset)('huggingface://datasets/test/dataset', 125);
            // Should have made at least one request
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalled();
            // First call should be normal page size
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenNthCalledWith(1, expect.stringContaining('length=100'), expect.anything());
        ...
        it('should handle authentication tokens correctly', async () => {
            ...
            await (0, huggingfaceDatasets_1.fetchHuggingFaceDataset)('huggingface://datasets/test/dataset', 5);
            expect(jest.mocked(cache_1.fetchWithCache)).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                headers: {
