    ...
    transform: jest.fn().mockImplementation(async (code, input, context, skipWrap, inputType) => {
        ...
        if (typeof code === 'string') {
            ...
            if (code.includes('context?.metadata')) {
                ...
                if (code.includes('Empty metadata') &&
                    context?.metadata &&
                ...
                if (code.includes('Missing context') &&
...
const mockReasoningApiProvider = {
...
describe('evaluator', () => {
    ...
    it('evaluate with vars from file', async () => {
        ...
        jest.spyOn(fs_1.default, 'readFileSync').mockImplementation((path) => {
        ...
        const evalHelpers = await Promise.resolve().then(() => __importStar(require('../src/evaluatorHelpers')));
        ...
        try {
            const processedTestSuite = await (0, fileReference_1.processConfigFileReferences)(testSuite);
            const evalRecord = await eval_1.default.create({}, processedTestSuite.prompts, { id: (0, crypto_1.randomUUID)() });
            await (0, evaluator_1.evaluate)(processedTestSuite, evalRecord, {});
            const summary = await evalRecord.toEvaluateSummary();
            expect(mockApiProvider.callApi).toHaveBeenCalledTimes(1);
            expect(mockApiProvider.callApi).toHaveBeenCalledWith('Test prompt <h1>Sample Report</h1><p>This is a test report with some data for the year 2023.</p>', expect.anything(), undefined);
            expect(summary.stats.successes).toBe(1);
            expect(summary.stats.failures).toBe(0);
    ...
    it('evaluate with metadata passed to test transform', async () => {
        const mockApiProviderWithMetadata = {
            ...
            tests: [
                {
                    ...
                    options: {
                        transform: 'context?.metadata ? `Output: ${output}, Metadata: ${JSON.stringify(context.metadata)}` : output',
                    },
                },
            ],
        };
        ...
        expect(summary.stats.successes).toBe(1);
        expect(summary.stats.failures).toBe(0);
        expect(summary.results[0].response?.output).toBe('Output: Test output, Metadata: {"responseTime":123,"modelVersion":"v1.0"}');
    });
    it('evaluate with metadata passed to test transform - no metadata case', async () => {
        const mockApiProviderNoMetadata = {
    ...
    it('evaluate with metadata passed to test transform - empty metadata', async () => {
        const mockApiProviderEmptyMetadata = {
            ...
            tests: [
                {
                    ...
                    options: {
                        transform: '(context?.metadata && Object.keys(context.metadata).length === 0) ? `Empty metadata: ${output}` : output',
                    },
                },
            ],
        };
            ...
            tests: [
                {
                    ...
                    options: {
                        transform: '(Boolean(context?.vars) && Boolean(context?.prompt) && Boolean(context?.metadata)) ? `All context: ${output}` : `Missing context: ${output}`',
                    },
                },
            ],
        };
    ...
    it('merges response metadata with test metadata', async () => {
        const mockProviderWithMetadata = {
        ...
        expect(results[0].metadata).toEqual({
            ...
            [constants_1.FILE_METADATA_KEY]: {},
        });
    });
    it('evaluate with _conversation variable', async () => {
        const mockApiProvider = {
            id: jest.fn().mockReturnValue('test-provider'),
            callApi: jest.fn().mockImplementation((prompt) => Promise.resolve({
                output: prompt,
                tokenUsage: { total: 10, prompt: 5, completion: 5, cached: 0, numRequests: 1 },
            })),
        };
    ...
    it('should apply prompt config to provider call', async () => {
        ...
        expect(summary.stats.failures).toBe(0);
        expect(mockApiProvider.callApi).toHaveBeenCalledTimes(1);
        expect(mockApiProvider.callApi).toHaveBeenCalledWith('You are a helpful math tutor. Solve 8x + 31 = 2', expect.objectContaining({
            prompt: expect.objectContaining({
                config: {
                    response_format: {
                        type: 'json_schema',
    ...
    it('should call runExtensionHook with correct parameters at appropriate times', async () => {
        ...
        expect(mockedRunExtensionHook).toHaveBeenNthCalledWith(1, [mockExtension], 'beforeAll', expect.objectContaining({ suite: testSuite }));
        // Check beforeEach call
        expect(mockedRunExtensionHook).toHaveBeenNthCalledWith(2, [mockExtension], 'beforeEach', expect.objectContaining({
        ...
        expect(mockedRunExtensionHook).toHaveBeenNthCalledWith(3, [mockExtension], 'afterEach', expect.objectContaining({
            test: expect.objectContaining({
                vars: { var1: 'value1' },
                assert: [{ type: 'equals', value: 'Test output' }],
            }),
        ...
        // Check afterAll call
        expect(mockedRunExtensionHook).toHaveBeenNthCalledWith(4, [mockExtension], 'afterAll', expect.objectContaining({
    ...
    it('should maintain separate conversation histories based on metadata.conversationId', async () => {
        ...
        expect(mockApiProvider.callApi).toHaveBeenNthCalledWith(1, expect.stringContaining('User: Question 1A'), expect.anything(), undefined);
        // First conversation, second question (should include history)
        expect(mockApiProvider.callApi).toHaveBeenNthCalledWith(2, expect.stringContaining('User: Question 1A\nAssistant: Test output\nUser: Question 1B'), expect.anything(), undefined);
        // Second conversation, first question (should NOT include first conversation)
        expect(mockApiProvider.callApi).toHaveBeenNthCalledWith(3, expect.stringContaining('User: Question 2A'), expect.anything(), undefined);
        // Second conversation, second question (should only include second conversation history)
        expect(mockApiProvider.callApi).toHaveBeenNthCalledWith(4, expect.stringContaining('User: Question 2A\nAssistant: Test output\nUser: Question 2B'), expect.anything(), undefined);
    });
    it('evaluates with provider delay', async () => {
        ...
        const evalRecord = await eval_1.default.create({}, testSuite.prompts, { id: (0, crypto_1.randomUUID)() });
        await (0, evaluator_1.evaluate)(testSuite, evalRecord, {});
        expect(time_1.sleep).toHaveBeenCalledWith(100);
        expect(mockApiProvider.callApi).toHaveBeenCalledTimes(1);
    });
    ...
    it('handles circular references when logging errors during result saving', async () => {
        ...
        const originalAddResult = eval_1.default.prototype.addResult;
        eval_1.default.prototype.addResult = mockAddResult;
        ...
        const evalRecord = await eval_1.default.create({}, testSuite.prompts, { id: (0, crypto_1.randomUUID)() });
        const errorSpy = jest.spyOn(logger_1.default, 'error');
        await (0, evaluator_1.evaluate)(testSuite, evalRecord, {});
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('Error saving result: Error: Mock save error'));
        eval_1.default.prototype.addResult = originalAddResult;
    ...
    it('evaluate with provider error response', async () => {
        const mockApiProviderWithError = {
    ...
    it('should handle evaluation timeout', async () => {
        ...
        try {
            const evalPromise = (0, evaluator_1.evaluate)(testSuite, mockEval, { timeoutMs: 100 });
            await evalPromise;
            expect(slowApiProvider.callApi).toHaveBeenCalledWith('Test prompt', expect.anything(), expect.objectContaining({
                abortSignal: expect.any(AbortSignal),
            }));
            expect(mockAddResult).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should abort when exceeding maxEvalTimeMs', async () => {
        ...
        try {
            const evalPromise = (0, evaluator_1.evaluate)(testSuite, mockEval, { maxEvalTimeMs: 100 });
            await evalPromise;
            expect(mockAddResult).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should accumulate token usage correctly', async () => {
        ...
        const results = await (0, evaluator_1.runEval)({
            ...mockOptions,
    ...
    it('should NOT include assertion tokens in main token totals', async () => {
        ...
        const gradingProviderWithTokens = {
...
describe('runEval', () => {
    ...
    it('should handle provider errors', async () => {
        ...
        expect(result.failureReason).toBe(index_1.ResultFailureReason.ERROR);
    });
    it('should handle null output differently for red team tests', async () => {
        const nullOutputProvider = {
        ...
        // Regular test
        const regularResults = await (0, evaluator_1.runEval)({
            ...defaultOptions,
        ...
        // Red team test
        const redTeamResults = await (0, evaluator_1.runEval)({
            ...defaultOptions,
            provider: nullOutputProvider,
            prompt: { raw: 'Test prompt', label: 'test-label' },
            test: {},
...
describe('formatVarsForDisplay', () => {
    it('should return empty string for empty or undefined vars', () => {
        expect((0, evaluator_1.formatVarsForDisplay)({}, 50)).toBe('');
        expect((0, evaluator_1.formatVarsForDisplay)(undefined, 50)).toBe('');
        expect((0, evaluator_1.formatVarsForDisplay)(null, 50)).toBe('');
    });
    it('should format simple variables correctly', () => {
        const vars = { name: 'John', age: 25, city: 'NYC' };
        const result = (0, evaluator_1.formatVarsForDisplay)(vars, 50);
    ...
    it('should handle different variable types', () => {
        ...
        const result = (0, evaluator_1.formatVarsForDisplay)(vars, 200);
    ...
    it('should handle extremely large vars without crashing', () => {
        ...
        expect(() => (0, evaluator_1.formatVarsForDisplay)(vars, 50)).not.toThrow();
        const result = (0, evaluator_1.formatVarsForDisplay)(vars, 50);
    ...
    it('should truncate final result to maxLength', () => {
        ...
        const result = (0, evaluator_1.formatVarsForDisplay)(vars, 20);
    ...
    it('should replace newlines with spaces', () => {
        ...
        const result = (0, evaluator_1.formatVarsForDisplay)(vars, 100);
    ...
    it('should return fallback message on any error', () => {
        ...
        const result = (0, evaluator_1.formatVarsForDisplay)(problematicVars, 50);
    ...
    it('should handle multiple variables with space distribution', () => {
        ...
        const result = (0, evaluator_1.formatVarsForDisplay)(vars, 30);
...
describe('Evaluator with external defaultTest', () => {
    ...
    it('should handle string defaultTest gracefully', async () => {
        const testSuite = {
            ...
            defaultTest: 'file://path/to/defaultTest.yaml', // String should have been resolved before reaching evaluator
        };
        const evalRecord = await eval_1.default.create({}, testSuite.prompts, { id: (0, crypto_1.randomUUID)() });
        await (0, evaluator_1.evaluate)(testSuite, evalRecord, {});
        const summary = await evalRecord.toEvaluateSummary();
        ...
        const testSuite = {
            ...
            tests: [
                { vars: { testVar: 'testValue' } },
                {
                    ...
                    assert: [{ type: 'contains', value: 'exp' }],
                    threshold: 0.9,
                },
            ],
            defaultTest,
    ...
    it('should handle invariant check for defaultTest.assert array', async () => {
        const testSuite = {
            ...
            tests: [{ vars: { var: 'value' } }],
            defaultTest: {
                assert: 'not-an-array', // Invalid type
            },
        };
        const evalRecord = await eval_1.default.create({}, testSuite.prompts, { id: (0, crypto_1.randomUUID)() });
        // Should throw or handle gracefully
        await expect((0, evaluator_1.evaluate)(testSuite, evalRecord, {})).rejects.toThrow('defaultTest.assert is not an array in test case #1');
    });
    ...
    it('should preserve metrics from existing prompts when resuming evaluation', async () => {
        ...
        try {
            ...
            const prompt2TotalTests = (evalRecord.prompts[1].metrics?.testPassCount || 0) +
