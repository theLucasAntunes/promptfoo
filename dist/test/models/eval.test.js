...
const eval_1 = __importStar(require("../../src/models/eval"));
...
describe('evaluator', () => {
    ...
    describe('summaryResults', () => {
        it('should return all evaluations', async () => {
            const eval1 = await evalFactory_1.default.create();
            const eval2 = await evalFactory_1.default.create();
            const eval3 = await evalFactory_1.default.createOldResult();
            const evaluations = await (0, eval_1.getEvalSummaries)();
        ...
        it('should return evaluations in descending order by createdAt', async () => {
            const eval1 = await evalFactory_1.default.create();
            const eval2 = await evalFactory_1.default.create();
            const eval3 = await evalFactory_1.default.create();
            const evaluations = await (0, eval_1.getEvalSummaries)();
            expect(evaluations).toHaveLength(3);
        ...
        it('should correctly deserialize all provider types', async () => {
            ...
            for (const testCase of testCases) {
                const evaluation = await eval_1.default.create({
                    providers: testCase.providers,
                    prompts: ['Test prompt'],
                    tests: [{ vars: { test: 'value' } }],
                }, [{ raw: 'Test prompt', label: 'Test prompt' }]);
                const summaries = await (0, eval_1.getEvalSummaries)(undefined, undefined, true);
    ...
    describe('delete', () => {
        it('should delete an evaluation', async () => {
            const eval1 = await evalFactory_1.default.create();
            const eval_ = await eval_1.default.findById(eval1.id);
            expect(eval_).toBeDefined();
            await eval1.delete();
            const eval_2 = await eval_1.default.findById(eval1.id);
    ...
    describe('create', () => {
        it('should use provided author when available', async () => {
            ...
            const evaluation = await eval_1.default.create(config, renderedPrompts, { author: providedAuthor });
            expect(evaluation.author).toBe(providedAuthor);
            const persistedEval = await eval_1.default.findById(evaluation.id);
        ...
        it('should use default author from getUserEmail when not provided', async () => {
            ...
            jest.mocked(accounts_1.getUserEmail).mockReturnValue(mockEmail);
            const config = { description: 'Test eval' };
            const renderedPrompts = [
                { raw: 'Test prompt', display: 'Test prompt', label: 'Test label' },
            ];
            const evaluation = await eval_1.default.create(config, renderedPrompts);
            const persistedEval = await eval_1.default.findById(evaluation.id);
    ...
    describe('findById', () => {
        it('should handle empty vars array', async () => {
            const eval1 = await evalFactory_1.default.create({ numResults: 0 });
            const persistedEval = await eval_1.default.findById(eval1.id);
    ...
    describe('getTablePage', () => {
        ...
        beforeEach(async () => {
            ...
            evalWithResults = await evalFactory_1.default.create({
                numResults: 20,
        ...
        it('should respect offset and limit parameters', async () => {
            ...
            const largeEval = await evalFactory_1.default.create({
                numResults: 12,
        ...
        it('should filter by errors', async () => {
            ...
            for (const row of result.body) {
                const hasError = row.outputs.some((output) => output.error?.includes('error') ||
        ...
        it('should filter by failures', async () => {
            ...
            for (const row of result.body) {
                const hasFailure = row.outputs.some((output) => !output.pass && !output.text.includes('error'));
        ...
        it('should filter by specific metrics', async () => {
            ...
            for (const row of result.body) {
                const hasMetric = row.outputs.some((output) => output.namedScores && output.namedScores['accuracy'] !== undefined);
        ...
        it('should be filterable by multiple metrics', async () => {
            ...
            for (const row of result.body) {
                const hasMetric1 = row.outputs.some((output) => output.namedScores && output.namedScores['accuracy'] !== undefined);
                const hasMetric2 = row.outputs.some((output) => output.namedScores && output.namedScores['relevance'] !== undefined);
        ...
        it('should handle empty result sets', async () => {
            ...
            const emptyEval = await evalFactory_1.default.create({ numResults: 0 });
    ...
    describe('EvalQueries.getMetadataKeysFromEval', () => {
        ...
        ('result3', '${eval_.id}', 0, 2, '{}', '{}', '{}', 1, 1.0, '{"key1": "value5", "key4": "value6"}')`);
            const keys = await eval_1.EvalQueries.getMetadataKeysFromEval(eval_.id);
    ...
    describe('queryTestIndices', () => {
        it('returns indices with default params and correct ordering', async () => {
            ...
            const { testIndices, filteredCount } = await eval_.queryTestIndices({});
        ...
        it('respects offset and limit', async () => {
            ...
            const page1 = await eval_.queryTestIndices({ offset: 0, limit: 5 });
            const page2 = await eval_.queryTestIndices({ offset: 5, limit: 5 });
            ...
            const overlap = page1.testIndices.filter((i) => page2.testIndices.includes(i));
        ...
        it('Mode Filtering: filters by errors, failures, and passes', async () => {
            ...
            const errors = await eval_.queryTestIndices({ filterMode: 'errors' });
            const failures = await eval_.queryTestIndices({ filterMode: 'failures' });
            const passes = await eval_.queryTestIndices({ filterMode: 'passes' });
        ...
        it('filters by metadata equals and contains', async () => {
            ...
            const db = (0, index_1.getDb)();
            await db.run(`UPDATE eval_results SET metadata = json('{"source":"unit","note":"hello world"}') WHERE eval_id = '${eval_.id}' AND test_idx = 1`);
            await db.run(`UPDATE eval_results SET metadata = json('{"source":"integration"}') WHERE eval_id = '${eval_.id}' AND test_idx = 2`);
            // equals on source=unit should return only test 1
            const eqRes = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            // contains on note
            const containsRes = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
        ...
        it('filters by metadata exists operator (non-empty values only)', async () => {
            ...
            await db.run(`UPDATE eval_results SET metadata = json('{"source":"unit","note":"hello"}') WHERE eval_id = '${eval_.id}' AND test_idx = 0`);
            await db.run(`UPDATE eval_results SET metadata = json('{"source":"","note":"hello"}') WHERE eval_id = '${eval_.id}' AND test_idx = 1`);
            await db.run(`UPDATE eval_results SET metadata = json('{"source":"  ","note":"hello"}') WHERE eval_id = '${eval_.id}' AND test_idx = 2`);
            await db.run(`UPDATE eval_results SET metadata = json('{"note":"hello"}') WHERE eval_id = '${eval_.id}' AND test_idx = 3`);
            await db.run(`UPDATE eval_results SET metadata = json('{"source":null,"note":"hello"}') WHERE eval_id = '${eval_.id}' AND test_idx = 4`);
            await db.run(`UPDATE eval_results SET metadata = json('{"source":"integration","note":"hello"}') WHERE eval_id = '${eval_.id}' AND test_idx = 5`);
            // exists should only return rows where source field has meaningful non-empty values
            const existsRes = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
        ...
        it('filters by metadata exists operator with various data types', async () => {
            ...
            await db.run(`UPDATE eval_results SET metadata = json('{"count":42}') WHERE eval_id = '${eval_.id}' AND test_idx = 0`);
            await db.run(`UPDATE eval_results SET metadata = json('{"count":0}') WHERE eval_id = '${eval_.id}' AND test_idx = 1`);
            await db.run(`UPDATE eval_results SET metadata = json('{"active":true}') WHERE eval_id = '${eval_.id}' AND test_idx = 2`);
            await db.run(`UPDATE eval_results SET metadata = json('{"active":false}') WHERE eval_id = '${eval_.id}' AND test_idx = 3`);
            await db.run(`UPDATE eval_results SET metadata = json('{"tags":["a","b"]}') WHERE eval_id = '${eval_.id}' AND test_idx = 4`);
            await db.run(`UPDATE eval_results SET metadata = json('{"config":{"key":"value"}}') WHERE eval_id = '${eval_.id}' AND test_idx = 5`);
            // Test exists for numeric field (should include both 42 and 0)
            const countExists = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            const activeExists = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            const tagsExists = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            const configExists = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
        ...
        it('filters by metadata with special characters in field names (security test)', async () => {
            ...
            await db.run(`UPDATE eval_results SET metadata = json('{"field\\"with\\"quotes":"value1"}') WHERE eval_id = '${eval_.id}' AND test_idx = 0`);
            await db.run(`UPDATE eval_results SET metadata = json('{"field\\\\with\\\\backslashes":"value2"}') WHERE eval_id = '${eval_.id}' AND test_idx = 1`);
            await db.run(`UPDATE eval_results SET metadata = json('{"normal_field":"value3"}') WHERE eval_id = '${eval_.id}' AND test_idx = 2`);
            // Test equals filter with quotes in field name
            const quotesResult = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            const backslashResult = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            const existsQuotesResult = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            const existsBackslashResult = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
        ...
        it('filters by metadata exists operator with empty arrays and objects', async () => {
            ...
            await db.run(`UPDATE eval_results SET metadata = json('{"arrayField":[]}') WHERE eval_id = '${eval_.id}' AND test_idx = 0`);
            // Test empty object - should match (not empty)
            await db.run(`UPDATE eval_results SET metadata = json('{"objectField":{}}') WHERE eval_id = '${eval_.id}' AND test_idx = 1`);
            // Test non-empty array - should match
            await db.run(`UPDATE eval_results SET metadata = json('{"arrayField":["item"]}') WHERE eval_id = '${eval_.id}' AND test_idx = 2`);
            // Test non-empty object - should match
            await db.run(`UPDATE eval_results SET metadata = json('{"objectField":{"key":"value"}}') WHERE eval_id = '${eval_.id}' AND test_idx = 3`);
            // Test null field - should not match
            await db.run(`UPDATE eval_results SET metadata = json('{"nullField":null}') WHERE eval_id = '${eval_.id}' AND test_idx = 4`);
            // Test missing field - should not match
            await db.run(`UPDATE eval_results SET metadata = json('{"otherField":"value"}') WHERE eval_id = '${eval_.id}' AND test_idx = 5`);
            // Test exists filter with empty array - should match because [] is not empty
            const emptyArrayResult = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            const emptyObjectResult = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
            ...
            const nullFieldResult = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'metadata',
        ...
        it('filters by plugin and strategy', async () => {
            ...
            await db.run(`UPDATE eval_results SET metadata = json('{"pluginId":"harmful:harassment"}') WHERE eval_id = '${eval_.id}' AND test_idx = 3`);
            await db.run(`UPDATE eval_results SET metadata = json('{"strategyId":"s1"}') WHERE eval_id = '${eval_.id}' AND test_idx = 5`);
            // Category filter should match pluginId that starts with harmful:
            const pluginCategory = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'plugin',
            ...
            const strategyEq = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'strategy',
        ...
        it('filters by explicit severity override', async () => {
            ...
            const db = (0, index_1.getDb)();
            await db.run(`UPDATE eval_results SET metadata = json('{"severity":"high"}') WHERE eval_id = '${eval_.id}' AND test_idx = 0`);
            const { testIndices, filteredCount } = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
        ...
        it('searches across response, grading, named scores, metadata, and vars', async () => {
            ...
            const limited = await eval_.queryTestIndices({ searchQuery: 'needle', limit: 2 });
