...
const evalPerformance_1 = require("../../src/models/evalPerformance");
...
describe('Highlights Filter Feature', () => {
    ...
    describe('Eval.queryTestIndices highlights filter', () => {
        it('should return only highlighted results when filterMode is highlights', async () => {
            const eval_ = await evalFactory_1.default.create({ numResults: 0 });
        ...
        it('should work with pagination when filtering highlights', async () => {
            ...
            // Test pagination
            const page1 = await eval_.queryTestIndices({
                filterMode: 'highlights',
                offset: 0,
                limit: 5,
            });
            const page2 = await eval_.queryTestIndices({
                filterMode: 'highlights',
                offset: 5,
                limit: 5,
            });
        ...
        it('should combine highlights filter with other filter modes correctly', async () => {
            ...
            for (const data of testData) {
                await eval_.addResult({
                    ...
                    error: null,
                    failureReason: data.success ? types_1.ResultFailureReason.NONE : types_1.ResultFailureReason.ASSERT,
            ...
            const highlights = await eval_.queryTestIndices({
                filterMode: 'highlights',
            });
            expect(highlights.testIndices).toEqual([0, 1]);
            // Verify other filters still work
            const passes = await eval_.queryTestIndices({
                filterMode: 'passes',
            });
            expect(passes.testIndices).toEqual([0, 2]);
            const failures = await eval_.queryTestIndices({
                filterMode: 'failures',
            });
    ...
    describe('evalPerformance.queryTestIndicesOptimized highlights filter', () => {
        it('should return only highlighted results with optimized query', async () => {
            ...
            const { testIndices, filteredCount } = await (0, evalPerformance_1.queryTestIndicesOptimized)(eval_.id, {
                filterMode: 'highlights',
            });
            expect(filteredCount).toBe(2);
        ...
        it('should work with search query in optimized mode', async () => {
            ...
            const withSearch = await (0, evalPerformance_1.queryTestIndicesOptimized)(eval_.id, {
                filterMode: 'highlights',
                searchQuery: 'target',
            });
            ...
            const withEmptyFilters = await (0, evalPerformance_1.queryTestIndicesOptimized)(eval_.id, {
                filterMode: 'highlights',
                searchQuery: 'target',
                filters: [],
            });
        ...
        it('should handle pagination correctly in optimized mode', async () => {
            ...
            // Get three pages
            const page1 = await (0, evalPerformance_1.queryTestIndicesOptimized)(eval_.id, {
                filterMode: 'highlights',
                offset: 0,
                limit: 5,
            });
            const page2 = await (0, evalPerformance_1.queryTestIndicesOptimized)(eval_.id, {
                filterMode: 'highlights',
                offset: 5,
                limit: 5,
            });
            const page3 = await (0, evalPerformance_1.queryTestIndicesOptimized)(eval_.id, {
                filterMode: 'highlights',
                offset: 10,
                limit: 5,
            });
        ...
        it('should verify SQL injection safety in optimized mode', async () => {
            ...
            const { filteredCount } = await (0, evalPerformance_1.queryTestIndicesOptimized)(eval_.id, {
                filterMode: 'highlights',
            ...
            const justHighlights = await (0, evalPerformance_1.queryTestIndicesOptimized)(eval_.id, {
                filterMode: 'highlights',
            });
    ...
    describe('Integration with getTablePage', () => {
        it('should filter highlights correctly through getTablePage', async () => {
            ...
            for (const row of result.body) {
                const hasHighlight = row.outputs.some((output) => output.gradingResult?.comment?.startsWith('!highlight'));
    ...
    describe('Edge cases and error handling', () => {
      ...
      `);
        ...
        it('should handle concurrent access to highlights filter', async () => {
            ...
            const promises = Array.from({ length: 5 }, () => eval_.queryTestIndices({ filterMode: 'highlights' }));
...
//# sourceMappingURL=evalHighlights.test.js.map