...
const index_2 = require("../../src/types/index");
...
describe('maybeLoadToolsFromExternalFile', () => {
    ...
    it('should process tool objects directly', () => {
        ...
        expect((0, index_3.maybeLoadToolsFromExternalFile)(tools, vars)).toEqual(tools);
    });
    it('should load tools from external file', () => {
        const tools = 'file://tools.json';
        expect((0, index_3.maybeLoadToolsFromExternalFile)(tools)).toEqual(JSON.parse(mockFileContent));
    });
    it('should render variables in tools object', () => {
        ...
        expect((0, index_3.maybeLoadToolsFromExternalFile)(tools, vars)).toEqual(expected);
    });
    it('should render variables and load from external file', () => {
        const tools = 'file://{{ file_path }}.json';
        const vars = { file_path: 'tools' };
        (0, index_3.maybeLoadToolsFromExternalFile)(tools, vars);
    ...
    it('should handle array of file paths', () => {
        ...
        (0, index_3.maybeLoadToolsFromExternalFile)(tools);
...
describe('util', () => {
    ...
    describe('writeOutput', () => {
        ...
        beforeEach(() => {
            ...
            // @ts-ignore
            jest.mocked(index_1.getDb).mockReturnValue({
        ...
        it('writeOutput with CSV output', async () => {
            ...
            await (0, index_3.writeOutput)(outputPath, eval_, shareableUrl);
            expect(fs.writeFileSync).toHaveBeenCalledTimes(1);
        });
        it('writeOutput with JSON output', async () => {
            const outputPath = 'output.json';
            const eval_ = new eval_1.default({});
            await (0, index_3.writeOutput)(outputPath, eval_, null);
            expect(fs.writeFileSync).toHaveBeenCalledTimes(1);
        });
        ...
        it('writeOutput with json and txt output', async () => {
            ...
            const eval_ = new eval_1.default({});
            await (0, index_3.writeMultipleOutputs)(outputPath, eval_, null);
    ...
    describe('readOutput', () => {
        it('reads JSON output', async () => {
            ...
            const output = await (0, index_3.readOutput)(outputPath);
            expect(output).toEqual({});
        });
        it('fails for csv output', async () => {
            await expect((0, index_3.readOutput)('output.csv')).rejects.toThrow('Unsupported output file format: csv currently only supports json');
        });
        it('fails for yaml output', async () => {
            await expect((0, index_3.readOutput)('output.yaml')).rejects.toThrow('Unsupported output file format: yaml currently only supports json');
            await expect((0, index_3.readOutput)('output.yml')).rejects.toThrow('Unsupported output file format: yml currently only supports json');
        });
        it('fails for xml output', async () => {
            await expect((0, index_3.readOutput)('output.xml')).rejects.toThrow('Unsupported output file format: xml currently only supports json');
        });
    });
    it('readFilters', async () => {
        ...
        const filters = await (0, index_3.readFilters)({ testFilter: 'filter.js' });
        expect(filters.testFilter).toBe(mockFilter);
    });
    describe('providerToIdentifier', () => {
        it('works with provider string', () => {
            expect((0, index_3.providerToIdentifier)('gpt-3.5-turbo')).toStrictEqual('gpt-3.5-turbo');
        });
        it('works with provider id undefined', () => {
            expect((0, index_3.providerToIdentifier)(undefined)).toBeUndefined();
        });
        it('works with ApiProvider', () => {
            ...
            expect((0, index_3.providerToIdentifier)(apiProvider)).toStrictEqual(providerId);
        });
        it('works with ProviderOptions', () => {
            ...
            expect((0, index_3.providerToIdentifier)(providerOptions)).toStrictEqual(providerId);
        });
        it('uses label when present on ProviderOptions', () => {
            ...
            expect((0, index_3.providerToIdentifier)(providerOptions)).toStrictEqual('my-provider');
        });
        it('canonicalizes relative file paths to absolute', () => {
            const originalCwd = process.cwd();
            expect((0, index_3.providerToIdentifier)('file://./provider.js')).toStrictEqual(`file://${path.join(originalCwd, 'provider.js')}`);
        });
        it('canonicalizes JavaScript files without file:// prefix', () => {
            const originalCwd = process.cwd();
            expect((0, index_3.providerToIdentifier)('./provider.js')).toStrictEqual(`file://${path.join(originalCwd, 'provider.js')}`);
        });
        it('preserves absolute file paths', () => {
            expect((0, index_3.providerToIdentifier)('file:///absolute/path/provider.js')).toStrictEqual('file:///absolute/path/provider.js');
        });
        it('canonicalizes exec: paths', () => {
            const originalCwd = process.cwd();
            expect((0, index_3.providerToIdentifier)('exec:./script.py')).toStrictEqual(`exec:${path.join(originalCwd, 'script.py')}`);
        });
        it('canonicalizes python: paths', () => {
            const originalCwd = process.cwd();
            expect((0, index_3.providerToIdentifier)('python:./provider.py')).toStrictEqual(`python:${path.join(originalCwd, 'provider.py')}`);
        });
    });
    describe('varsMatch', () => {
        it('true with both undefined', () => {
            expect((0, index_3.varsMatch)(undefined, undefined)).toBe(true);
        });
        it('false with one undefined', () => {
            expect((0, index_3.varsMatch)(undefined, {})).toBe(false);
            expect((0, index_3.varsMatch)({}, undefined)).toBe(false);
        });
    });
    describe('resultIsForTestCase', () => {
        ...
        it('is true', () => {
            expect((0, index_3.resultIsForTestCase)(result, testCase)).toBe(true);
        });
        it('is false if provider is different', () => {
            ...
            expect((0, index_3.resultIsForTestCase)(result, nonMatchTestCase)).toBe(false);
        });
        ...
        it('matches when test provider is label and result provider has label and id', () => {
            ...
            expect((0, index_3.resultIsForTestCase)(labelledResult, testCase)).toBe(true);
        });
        it('matches when test provider is relative path and result provider is absolute', () => {
            const relativePathTestCase = {
            ...
            expect((0, index_3.resultIsForTestCase)(absolutePathResult, relativePathTestCase)).toBe(true);
        });
        it('matches when test provider has no file:// prefix and result has absolute path', () => {
            const noPathTestCase = {
            ...
            expect((0, index_3.resultIsForTestCase)(absolutePathResult, noPathTestCase)).toBe(true);
        });
    ...
    describe('parsePathOrGlob', () => {
        ...
        it('should parse a simple file path with extension', () => {
            ...
            expect((0, index_3.parsePathOrGlob)('/base', 'file.txt')).toEqual({
                extension: '.txt',
        ...
        it('should parse a file path with function name', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', 'file.py:myFunction')).toEqual({
                extension: '.py',
        ...
        it('should parse a Go file path with function name', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', 'script.go:CallApi')).toEqual({
                extension: '.go',
        ...
        it('should parse a directory path', () => {
            ...
            expect((0, index_3.parsePathOrGlob)('/base', 'dir')).toEqual({
                extension: undefined,
                functionName: undefined,
                isPathPattern: true,
        ...
        it('should handle non-existent file path gracefully when PROMPTFOO_STRICT_FILES is false', () => {
            ...
            expect((0, index_3.parsePathOrGlob)('/base', 'nonexistent.js')).toEqual({
                extension: '.js',
                functionName: undefined,
                isPathPattern: false,
        ...
        it('should throw an error for non-existent file path when PROMPTFOO_STRICT_FILES is true', () => {
            ...
            expect(() => (0, index_3.parsePathOrGlob)('/base', 'nonexistent.js')).toThrow('File does not exist');
            delete process.env.PROMPTFOO_STRICT_FILES;
        });
        it('should properly test file existence when function name in the path', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            (0, index_3.parsePathOrGlob)('/base', 'script.py:myFunction');
        ...
        it('should return empty extension for files without extension', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', 'file')).toEqual({
                extension: '',
                functionName: undefined,
                isPathPattern: false,
        ...
        it('should handle paths with environment variables', () => {
            ...
            expect((0, index_3.parsePathOrGlob)('/base', process.env.FILE_PATH)).toEqual({
                extension: '.txt',
                functionName: undefined,
                isPathPattern: false,
                filePath: path.join('/base', 'file.txt'),
        ...
        it('should handle glob patterns in file path', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', '*.js')).toEqual({
                extension: undefined,
                functionName: undefined,
                isPathPattern: true,
        ...
        it('should handle complex file paths', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', 'dir/subdir/file.py:func')).toEqual({
                extension: '.py',
                functionName: 'func',
                isPathPattern: false,
        ...
        it('should handle non-standard file extensions', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', 'file.customext')).toEqual({
                extension: '.customext',
                functionName: undefined,
                isPathPattern: false,
        ...
        it('should handle deeply nested file paths', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', 'a/b/c/d/e/f/g/file.py:func')).toEqual({
                extension: '.py',
                functionName: 'func',
                isPathPattern: false,
        ...
        it('should handle complex directory paths', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => true });
            expect((0, index_3.parsePathOrGlob)('/base', 'a/b/c/d/e/f/g')).toEqual({
                extension: undefined,
                functionName: undefined,
                isPathPattern: true,
        ...
        it('should join basePath and safeFilename correctly', () => {
            ...
            expect((0, index_3.parsePathOrGlob)(basePath, relativePath)).toEqual({
                extension: '.txt',
                functionName: undefined,
                isPathPattern: false,
        ...
        it('should handle empty basePath', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('', 'file.txt')).toEqual({
                extension: '.txt',
                functionName: undefined,
                isPathPattern: false,
                filePath: 'file.txt',
        ...
        it('should handle file:// prefix', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('', 'file://file.txt')).toEqual({
                extension: '.txt',
                functionName: undefined,
                isPathPattern: false,
                filePath: 'file.txt',
        ...
        it('should handle file://./... with absolute base path', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/absolute/base', 'file://./prompts/file.txt')).toEqual({
                extension: '.txt',
                functionName: undefined,
                isPathPattern: false,
        ...
        it('should handle file://./... with relative base path', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('relative/base', 'file://file.txt')).toEqual({
                extension: '.txt',
                functionName: undefined,
                isPathPattern: false,
        ...
        it('should handle file:// prefix with Go function', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', 'file://script.go:CallApi')).toEqual({
                extension: '.go',
                functionName: 'CallApi',
                isPathPattern: false,
                filePath: path.join('/base', 'script.go'),
        ...
        it('should handle file:// prefix with absolute path and Go function', () => {
            jest.spyOn(fs, 'statSync').mockReturnValue({ isDirectory: () => false });
            expect((0, index_3.parsePathOrGlob)('/base', 'file:///absolute/path/script.go:CallApi')).toEqual({
                extension: '.go',
                functionName: 'CallApi',
                isPathPattern: false,
...
describe('setupEnv', () => {
    ...
    it('should call dotenv.config with path, override=true, and quiet=true when envPath is specified', () => {
        ...
        (0, index_3.setupEnv)(testEnvPath);
        expect(dotenvConfigSpy).toHaveBeenCalledTimes(1);
    ...
    it('should load environment variables with override when specified env file has conflicting values', () => {
        ...
        (0, index_3.setupEnv)('.env.production');
...
describe('renderVarsInObject', () => {
    ...
    it('should render environment variables in objects', () => {
        ...
        const rendered = (0, index_3.renderVarsInObject)(obj, {});
    ...
    it('should return object unchanged when PROMPTFOO_DISABLE_TEMPLATING is true', () => {
        ...
        const rendered = (0, index_3.renderVarsInObject)(obj, vars);
        expect(rendered).toEqual(obj);
    });
    ...
    it('should handle function objects by calling them with vars', () => {
        ...
        const rendered = (0, index_3.renderVarsInObject)(mockFunction, vars);
    ...
    it('should handle function that returns complex object structure', () => {
        ...
        const rendered = (0, index_3.renderVarsInObject)(complexFunction, vars);
...
describe('createOutputMetadata', () => {
    it('should create metadata with all fields when evalRecord has all data', () => {
        ...
        const metadata = (0, index_3.createOutputMetadata)(evalRecord);
