...
describe('sanitizeObject', () => {
    describe('primitives and basic types', () => {
        it('should handle null', () => {
            expect((0, sanitizer_1.sanitizeObject)(null)).toBeNull();
        });
        it('should handle undefined', () => {
            expect((0, sanitizer_1.sanitizeObject)(undefined)).toBeUndefined();
        });
        it('should handle strings', () => {
            expect((0, sanitizer_1.sanitizeObject)('test string')).toBe('test string');
        });
        it('should handle numbers', () => {
            expect((0, sanitizer_1.sanitizeObject)(42)).toBe(42);
            expect((0, sanitizer_1.sanitizeObject)(0)).toBe(0);
            expect((0, sanitizer_1.sanitizeObject)(-1)).toBe(-1);
            expect((0, sanitizer_1.sanitizeObject)(3.14)).toBe(3.14);
            expect((0, sanitizer_1.sanitizeObject)(Number.POSITIVE_INFINITY)).toBe(Number.POSITIVE_INFINITY);
            expect((0, sanitizer_1.sanitizeObject)(Number.NEGATIVE_INFINITY)).toBe(Number.NEGATIVE_INFINITY);
            expect((0, sanitizer_1.sanitizeObject)(Number.NaN)).toBeNaN();
        });
        it('should handle booleans', () => {
            expect((0, sanitizer_1.sanitizeObject)(true)).toBe(true);
            expect((0, sanitizer_1.sanitizeObject)(false)).toBe(false);
        });
        it('should handle empty string', () => {
            expect((0, sanitizer_1.sanitizeObject)('')).toBe('');
        });
        it('should parse and sanitize JSON strings', () => {
            const jsonString = JSON.stringify({ password: 'secret', data: 'public' });
            const result = (0, sanitizer_1.sanitizeObject)(jsonString);
        ...
        it('should return invalid JSON strings unchanged', () => {
            ...
            expect((0, sanitizer_1.sanitizeObject)(invalidJson)).toBe(invalidJson);
        });
    });
    describe('function handling', () => {
        it('should convert named functions to string representation', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)({ func: namedFunction });
        ...
        it('should convert anonymous functions to string representation', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)({ func: anonymousFunc });
            // Functions get lost during JSON.parse/stringify cycle
            expect(result.func).toBeUndefined();
        });
        it('should convert arrow functions to string representation', () => {
            const arrowFunc = () => 'test';
            const result = (0, sanitizer_1.sanitizeObject)({ func: arrowFunc });
            // Functions get lost during JSON.parse/stringify cycle
            expect(result.func).toBeUndefined();
        });
        it('should handle functions at multiple nesting levels', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)(input);
            // Functions get lost during JSON.parse/stringify cycle
    ...
    describe('sensitive field redaction', () => {
        describe('password variants', () => {
            it('should redact password', () => {
                expect((0, sanitizer_1.sanitizeObject)({ password: 'secret123' })).toEqual({ password: '[REDACTED]' });
            });
            it('should redact passwd', () => {
                expect((0, sanitizer_1.sanitizeObject)({ passwd: 'secret123' })).toEqual({ passwd: '[REDACTED]' });
            });
            it('should redact pwd', () => {
                expect((0, sanitizer_1.sanitizeObject)({ pwd: 'secret123' })).toEqual({ pwd: '[REDACTED]' });
            });
        ...
        describe('secret variants', () => {
            it('should redact secret', () => {
                expect((0, sanitizer_1.sanitizeObject)({ secret: 'hidden' })).toEqual({ secret: '[REDACTED]' });
            });
            it('should redact secrets', () => {
                expect((0, sanitizer_1.sanitizeObject)({ secrets: 'hidden' })).toEqual({ secrets: '[REDACTED]' });
            });
            it('should redact client_secret with underscore', () => {
                expect((0, sanitizer_1.sanitizeObject)({ client_secret: 'hidden' })).toEqual({
                    client_secret: '[REDACTED]',
                });
            });
            it('should redact client-secret with hyphen', () => {
                expect((0, sanitizer_1.sanitizeObject)({ 'client-secret': 'hidden' })).toEqual({
                    'client-secret': '[REDACTED]',
                });
            });
            it('should redact clientSecret in camelCase', () => {
                expect((0, sanitizer_1.sanitizeObject)({ clientSecret: 'hidden' })).toEqual({ clientSecret: '[REDACTED]' });
            });
            it('should redact webhook_secret', () => {
                expect((0, sanitizer_1.sanitizeObject)({ webhook_secret: 'hidden' })).toEqual({
        ...
        describe('api keys and tokens', () => {
            it('should redact apiKey', () => {
                expect((0, sanitizer_1.sanitizeObject)({ apiKey: 'key123' })).toEqual({ apiKey: '[REDACTED]' });
            });
            it('should redact api_key', () => {
                expect((0, sanitizer_1.sanitizeObject)({ api_key: 'key123' })).toEqual({ api_key: '[REDACTED]' });
            });
            it('should redact api-key', () => {
                expect((0, sanitizer_1.sanitizeObject)({ 'api-key': 'key123' })).toEqual({ 'api-key': '[REDACTED]' });
            });
            it('should redact token', () => {
                expect((0, sanitizer_1.sanitizeObject)({ token: 'token123' })).toEqual({ token: '[REDACTED]' });
            });
            it('should redact accessToken', () => {
                expect((0, sanitizer_1.sanitizeObject)({ accessToken: 'token123' })).toEqual({ accessToken: '[REDACTED]' });
            });
            it('should redact access_token', () => {
                expect((0, sanitizer_1.sanitizeObject)({ access_token: 'token123' })).toEqual({
                    access_token: '[REDACTED]',
                });
            });
            it('should redact refreshToken', () => {
                expect((0, sanitizer_1.sanitizeObject)({ refreshToken: 'token123' })).toEqual({
                    refreshToken: '[REDACTED]',
                });
            });
            it('should redact idToken', () => {
                expect((0, sanitizer_1.sanitizeObject)({ idToken: 'token123' })).toEqual({ idToken: '[REDACTED]' });
            });
            it('should redact bearerToken', () => {
                expect((0, sanitizer_1.sanitizeObject)({ bearerToken: 'token123' })).toEqual({ bearerToken: '[REDACTED]' });
            });
            it('should redact authToken', () => {
                expect((0, sanitizer_1.sanitizeObject)({ authToken: 'token123' })).toEqual({ authToken: '[REDACTED]' });
            });
        });
        describe('authorization and auth variants', () => {
            it('should redact authorization', () => {
                expect((0, sanitizer_1.sanitizeObject)({ authorization: 'Bearer token' })).toEqual({
                    authorization: '[REDACTED]',
                });
            });
            it('should redact auth', () => {
                expect((0, sanitizer_1.sanitizeObject)({ auth: 'token' })).toEqual({ auth: '[REDACTED]' });
            });
            it('should redact bearer', () => {
                expect((0, sanitizer_1.sanitizeObject)({ bearer: 'token' })).toEqual({ bearer: '[REDACTED]' });
            });
        });
        describe('header-specific patterns', () => {
            it('should redact x-api-key', () => {
                expect((0, sanitizer_1.sanitizeObject)({ 'x-api-key': 'key123' })).toEqual({ 'x-api-key': '[REDACTED]' });
            });
            it('should redact x-auth-token', () => {
                expect((0, sanitizer_1.sanitizeObject)({ 'x-auth-token': 'token123' })).toEqual({
                    'x-auth-token': '[REDACTED]',
                });
            });
            it('should redact x-access-token', () => {
                expect((0, sanitizer_1.sanitizeObject)({ 'x-access-token': 'token123' })).toEqual({
                    'x-access-token': '[REDACTED]',
                });
            });
            it('should redact x-auth', () => {
                expect((0, sanitizer_1.sanitizeObject)({ 'x-auth': 'token123' })).toEqual({ 'x-auth': '[REDACTED]' });
            });
            it('should redact cookie', () => {
                expect((0, sanitizer_1.sanitizeObject)({ cookie: 'session=abc123' })).toEqual({ cookie: '[REDACTED]' });
            });
            it('should redact set-cookie', () => {
                expect((0, sanitizer_1.sanitizeObject)({ 'set-cookie': 'session=abc123' })).toEqual({
                    'set-cookie': '[REDACTED]',
                });
            });
        });
        describe('certificate and encryption keys', () => {
            it('should redact privateKey', () => {
                expect((0, sanitizer_1.sanitizeObject)({ privateKey: '-----BEGIN' })).toEqual({ privateKey: '[REDACTED]' });
            });
            it('should redact private_key', () => {
                expect((0, sanitizer_1.sanitizeObject)({ private_key: '-----BEGIN' })).toEqual({
                    private_key: '[REDACTED]',
                });
            });
            it('should redact certificatePassword', () => {
                expect((0, sanitizer_1.sanitizeObject)({ certificatePassword: 'pass' })).toEqual({
                    certificatePassword: '[REDACTED]',
                });
            });
            it('should redact pfxPassword', () => {
                expect((0, sanitizer_1.sanitizeObject)({ pfxPassword: 'pass' })).toEqual({ pfxPassword: '[REDACTED]' });
            });
            it('should redact keystorePassword', () => {
                expect((0, sanitizer_1.sanitizeObject)({ keystorePassword: 'pass' })).toEqual({
                    keystorePassword: '[REDACTED]',
                });
            });
            it('should redact encryptionKey', () => {
                expect((0, sanitizer_1.sanitizeObject)({ encryptionKey: 'key' })).toEqual({ encryptionKey: '[REDACTED]' });
            });
            it('should redact signingKey', () => {
                expect((0, sanitizer_1.sanitizeObject)({ signingKey: 'key' })).toEqual({ signingKey: '[REDACTED]' });
            });
            it('should redact signature', () => {
                expect((0, sanitizer_1.sanitizeObject)({ signature: 'sig' })).toEqual({ signature: '[REDACTED]' });
            });
            it('should redact sig', () => {
                expect((0, sanitizer_1.sanitizeObject)({ sig: 'sig' })).toEqual({ sig: '[REDACTED]' });
            });
            it('should redact passphrase', () => {
                expect((0, sanitizer_1.sanitizeObject)({ passphrase: 'phrase' })).toEqual({ passphrase: '[REDACTED]' });
            });
            it('should redact certificateContent', () => {
                expect((0, sanitizer_1.sanitizeObject)({ certificateContent: 'content' })).toEqual({
                    certificateContent: '[REDACTED]',
                });
            });
            it('should redact pfx', () => {
                expect((0, sanitizer_1.sanitizeObject)({ pfx: 'content' })).toEqual({ pfx: '[REDACTED]' });
            });
            it('should redact pfxContent', () => {
                expect((0, sanitizer_1.sanitizeObject)({ pfxContent: 'content' })).toEqual({ pfxContent: '[REDACTED]' });
            });
            it('should redact certKey', () => {
                expect((0, sanitizer_1.sanitizeObject)({ certKey: 'key' })).toEqual({ certKey: '[REDACTED]' });
            });
        });
    ...
    describe('array handling', () => {
        ...
        it('should handle empty arrays', () => {
            expect((0, sanitizer_1.sanitizeObject)([])).toEqual([]);
        });
    ...
    describe('class instances and prototypes', () => {
        it('should convert class instances to plain objects via JSON', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)({ obj: instance });
        ...
        it('should convert Date objects to ISO strings via JSON', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)({ date });
        ...
        it('should convert RegExp objects to empty objects via JSON', () => {
            const regex = /test/gi;
            const result = (0, sanitizer_1.sanitizeObject)({ regex });
        ...
        it('should convert Error objects to empty objects via JSON', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)({ error });
        ...
        it('should handle objects with null prototype', () => {
            ...
            obj.password = 'secret';
            obj.data = 'public';
            const result = (0, sanitizer_1.sanitizeObject)(obj);
            expect(result.password).toBe('[REDACTED]');
    ...
    describe('circular references', () => {
        it('should handle circular references', () => {
            const obj = { name: 'test', data: 'value' };
            obj.self = obj;
            const result = (0, sanitizer_1.sanitizeObject)(obj);
        ...
        it('should handle deep circular references', () => {
            const obj = { level1: { level2: { data: 'test' } } };
        ...
        it('should handle multiple circular references', () => {
            ...
            obj1.ref = obj2;
            obj2.ref = obj1;
            const result = (0, sanitizer_1.sanitizeObject)({ obj1, obj2 });
    ...
    describe('error handling', () => {
        it('should handle errors gracefully with throwOnError false', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)(input, { throwOnError: false });
            expect(result).toEqual(input);
        });
        it('should throw errors when throwOnError is true', () => {
            ...
            expect(() => (0, sanitizer_1.sanitizeObject)(input, { throwOnError: true })).toThrow('Parse error');
        });
        it('should log context in error messages', () => {
            ...
            (0, sanitizer_1.sanitizeObject)(input, { context: 'test context', throwOnError: false });
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('test context'), expect.any(Error));
        });
    });
    describe('real-world scenarios', () => {
        it('should sanitize HTTP request config', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)(requestConfig);
        ...
        it('should sanitize URLs with sensitive query parameters in url field', () => {
            ...
            expect(result.url).toBe('https://api.example.com/v1/resource?api_key=%5BREDACTED%5D&token=%5BREDACTED%5D&data=public');
        ...
        it('should sanitize database connection config', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)(dbConfig);
        ...
        it('should sanitize OAuth token response', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)(tokenResponse);
        ...
        it('should sanitize AWS credentials', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)(awsConfig);
        ...
        it('should sanitize provider response with metadata', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)(providerResponse);
        ...
        it('should sanitize JSON string request body with apiKey', () => {
            ...
            const result = (0, sanitizer_1.sanitizeObject)(logContext);
...
describe('sanitizeBody', () => {
    it('should be an alias for sanitizeObject', () => {
        const input = { password: 'secret', data: 'public' };
        expect((0, sanitizer_1.sanitizeBody)(input)).toEqual((0, sanitizer_1.sanitizeObject)(input));
    });
});
describe('sanitizeUrl', () => {
    describe('invalid inputs', () => {
        it('should handle non-string inputs', () => {
            ...
            expect((0, sanitizer_1.sanitizeUrl)(123)).toBe(123);
            expect((0, sanitizer_1.sanitizeUrl)({})).toEqual({});
            expect((0, sanitizer_1.sanitizeUrl)([])).toEqual([]);
        });
        it('should handle empty string', () => {
            expect((0, sanitizer_1.sanitizeUrl)('')).toBe('');
        });
        ...
        it('should handle malformed URLs gracefully', () => {
            ...
            expect((0, sanitizer_1.sanitizeUrl)(malformedUrl)).toBe(malformedUrl);
        });
        it('should handle protocol-relative URLs', () => {
            const url = '//example.com/api?api_key=secret123';
            expect((0, sanitizer_1.sanitizeUrl)(url)).toBe(url);
        });
    ...
    describe('basic authentication', () => {
        it('should redact username and password', () => {
            ...
            const result = (0, sanitizer_1.sanitizeUrl)(url);
    ...
    describe('query parameter sanitization', () => {
        ...
        it('should be case insensitive for parameter matching', () => {
            ...
            const result = (0, sanitizer_1.sanitizeUrl)(url);
            expect(result).toBe('https://example.com/api?API_KEY=%5BREDACTED%5D&Token=%5BREDACTED%5D&SECRET=%5BREDACTED%5D');
        });
        it('should redact multiple sensitive parameters', () => {
            const url = 'https://example.com/api?api_key=secret123&token=token456&data=public&password=pass789';
            const result = (0, sanitizer_1.sanitizeUrl)(url);
            expect(result).toBe('https://example.com/api?api_key=%5BREDACTED%5D&token=%5BREDACTED%5D&data=public&password=%5BREDACTED%5D');
        });
    ...
    describe('edge cases', () => {
        ...
        it('should handle URLs with all components', () => {
            ...
            const result = (0, sanitizer_1.sanitizeUrl)(url);
            expect(result).toBe('https://***:***@example.com:8080/path/to/resource?api_key=%5BREDACTED%5D#section');
        });
    ...
    describe('error handling', () => {
        it('should handle URL parsing errors gracefully', () => {
            ...
            const result = (0, sanitizer_1.sanitizeUrl)(invalidUrl);
            expect(result).toBe(invalidUrl);
            expect(consoleWarnSpy).toHaveBeenCalled();
        });
        it('should log warning on URL parsing failure', () => {
            const invalidUrl = 'totally-invalid-url';
            (0, sanitizer_1.sanitizeUrl)(invalidUrl);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to sanitize URL'));
        });
    });
});
