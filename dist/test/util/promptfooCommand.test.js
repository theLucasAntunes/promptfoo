...
const promptfooCommand_1 = require("../../src/util/promptfooCommand");
describe('nextCommand', () => {
    ...
    describe('detectInstaller', () => {
        it('should detect npx from npm_execpath', () => {
            ...
            expect((0, promptfooCommand_1.detectInstaller)()).toBe('npx');
        });
        ...
        it('should detect brew from npm_config_prefix', () => {
            ...
            expect((0, promptfooCommand_1.detectInstaller)()).toBe('brew');
        });
        ...
        it('should detect npm-global from npm_config_user_agent', () => {
            ...
            expect((0, promptfooCommand_1.detectInstaller)()).toBe('npm-global');
        });
        it('should return unknown for unrecognized patterns', () => {
            process.env.npm_config_user_agent = 'yarn/1.22.0';
            expect((0, promptfooCommand_1.detectInstaller)()).toBe('unknown');
        });
    ...
    describe('promptfooCommand', () => {
        it('should return npx command for npx installer', () => {
            process.env.npm_execpath = '/path/to/npx';
            expect((0, promptfooCommand_1.promptfooCommand)('eval')).toBe('npx promptfoo@latest eval');
        });
        it('should return regular command for brew installer', () => {
            process.env.npm_config_prefix = '/usr/local/Homebrew/Cellar/node/20.0.0/bin';
            expect((0, promptfooCommand_1.promptfooCommand)('eval')).toBe('promptfoo eval');
        });
        ...
        it('should handle complex subcommands', () => {
            process.env.npm_execpath = '/path/to/npx';
            expect((0, promptfooCommand_1.promptfooCommand)('redteam init')).toBe('npx promptfoo@latest redteam init');
        });
        it('should handle subcommands with flags', () => {
            process.env.npm_execpath = '/path/to/npx';
            expect((0, promptfooCommand_1.promptfooCommand)('eval -c config.yaml')).toBe('npx promptfoo@latest eval -c config.yaml');
        });
        it('should handle empty subcommand for npx', () => {
            process.env.npm_execpath = '/path/to/npx';
            expect((0, promptfooCommand_1.promptfooCommand)('')).toBe('npx promptfoo@latest');
        });
        it('should handle empty subcommand for non-npx', () => {
            process.env.npm_config_prefix = '/usr/local/Homebrew/Cellar/node/20.0.0/bin';
            expect((0, promptfooCommand_1.promptfooCommand)('')).toBe('promptfoo');
        });
    });
    describe('isRunningUnderNpx (legacy compatibility)', () => {
        it('should return true for npx installer', () => {
            process.env.npm_execpath = '/path/to/npx';
            expect((0, promptfooCommand_1.isRunningUnderNpx)()).toBe(true);
        });
        it('should return false for non-npx installers', () => {
            process.env.npm_config_prefix = '/usr/local/Homebrew/Cellar/node/20.0.0/bin';
            expect((0, promptfooCommand_1.isRunningUnderNpx)()).toBe(false);
        });
    });
    describe('edge cases', () => {
        it('should handle partial matches in strings', () => {
            process.env.npm_execpath = '/some/path/with-npx-in-middle/but-not-npx';
            expect((0, promptfooCommand_1.detectInstaller)()).toBe('npx'); // Should still match because contains 'npx'
        });
        it('should handle npm_config_user_agent with multiple patterns', () => {
            process.env.npm_config_user_agent = 'npx/10.5.0 npm/10.8.2 node/v18.20.4';
            expect((0, promptfooCommand_1.detectInstaller)()).toBe('npx'); // Should detect npx, not npm
        });
        it('should handle undefined environment variables gracefully', () => {
            ...
            delete process.env.npm_config_prefix;
            delete process.env.npm_config_user_agent;
            expect(() => (0, promptfooCommand_1.detectInstaller)()).not.toThrow();
            expect((0, promptfooCommand_1.detectInstaller)()).toBe('unknown');
        });
    });
});
//# sourceMappingURL=promptfooCommand.test.js.map