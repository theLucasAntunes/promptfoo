...
describe('combineConfigs', () => {
    ...
    it('should load defaultTest from external file when string starts with file://', async () => {
        ...
        jest.mocked(fs.existsSync).mockReturnValue(true);
        jest.mocked(fs.readFileSync).mockReturnValue(js_yaml_1.default.dump(externalDefaultTest));
        jest.mocked(file_1.maybeLoadFromExternalFile).mockResolvedValue(externalDefaultTest);
        const config = {
            providers: ['provider1'],
            prompts: ['prompt1'],
            tests: ['test1'],
...
describe('resolveConfigs', () => {
    ...
    it('should set cliState.basePath', async () => {
        ...
        jest.mocked(glob_1.globSync).mockReturnValueOnce(['config.json']);
    ...
    it('should load scenarios and tests from external files', async () => {
        ...
        jest.mocked(index_1.readPrompts).mockResolvedValue([
            {
                raw: prompt,
                label: prompt,
                config: {},
        ...
        jest.mocked(index_2.loadApiProviders).mockResolvedValue([
            {
                id: () => 'openai:gpt-4',
                label: 'openai:gpt-4',
                modelName: 'gpt-4',
    ...
    it('should warn and exit when no config file, no prompts, no providers, and not in CI', async () => {
        ...
        expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining('npx promptfoo@latest eval -c'));
        expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining('npx promptfoo@latest init'));
    });
    it('should throw an error if no providers are provided', async () => {
        ...
        jest.mocked(glob_1.globSync).mockReturnValueOnce(['config.json']);
        await expect((0, load_1.resolveConfigs)(cmdObj, defaultConfig)).rejects.toThrow('Process exited with code 1');
        expect(logger_1.default.error).toHaveBeenCalledWith(expect.stringContaining('You must specify at least 1 provider (for example, openai:gpt-4.1)'));
    });
    ...
    it('should load defaultTest with embedding provider configuration', async () => {
        ...
        jest.mocked(fs.readFileSync).mockImplementation((path) => {
            if (typeof path === 'string' && path.endsWith('config.yaml')) {
                return js_yaml_1.default.dump(configWithDefaultTest);
            }
            return Buffer.from('');
        });
        jest.mocked(glob_1.globSync).mockReturnValue(['config.yaml']);
        jest.mocked(envars_1.isCI).mockReturnValue(true); // Avoid triggering exit
        ...
        const result = await (0, load_1.resolveConfigs)(cmdObj, {});
...
describe('readConfig', () => {
    ...
    it('should resolve YAML references before validation', async () => {
        ...
        jest.mocked(fs.readFileSync).mockReturnValueOnce(js_yaml_1.default.dump(mockConfig));
        jest.spyOn(json_schema_ref_parser_1.default.prototype, 'dereference').mockResolvedValueOnce(dereferencedConfig);
        const result = await (0, load_1.readConfig)('config.yaml');
...
describe('resolveConfigs with external defaultTest', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        jest.restoreAllMocks();
        cliState_1.default.basePath = '/mock/base';
    });
    it('should resolve defaultTest from external file in resolveConfigs', async () => {
        ...
        jest.mocked(file_1.maybeLoadFromExternalFile).mockResolvedValue(externalDefaultTest);
        jest.mocked(testCaseReader_1.readTests).mockResolvedValue([{ vars: { test: 'value' } }]);
        const result = await (0, load_1.resolveConfigs)({ config: ['config.json'] }, {});
        expect(file_1.maybeLoadFromExternalFile).toHaveBeenCalledWith('file://shared/defaultTest.yaml');
        expect(result.testSuite.defaultTest).toEqual(expect.objectContaining({
    ...
    it('should handle inline defaultTest object in resolveConfigs', async () => {
        ...
        expect(file_1.maybeLoadFromExternalFile).toHaveBeenCalledTimes(0);
        expect(result.testSuite.defaultTest).toEqual(expect.objectContaining({
